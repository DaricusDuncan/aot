# nix/nixosModules.nix — NixOS module for aot-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: aot runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $AOT_HOME/.env
# and read by aot at startup — no container recreation needed for env changes.
#
# Tool resolution: the aot wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    aot-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-aot-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "aot-config-attrs";
      description = "Aot YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "aot-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "aot-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'AOT_DOC_EOF'\n${value}\nAOT_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "aot-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/aot";
    ...
    # aot user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "aot-container-entrypoint" ''
      set -eu

      AOT_UID="''${AOT_UID:?AOT_UID must be set}"
      AOT_GID="''${AOT_GID:?AOT_GID must be set}"

      # ── Group: ensure a group with GID=$AOT_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$AOT_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="aot"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$AOT_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$AOT_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$AOT_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$AOT_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="aot"
        TARGET_HOME="/home/aot"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$AOT_UID" -g "$AOT_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$AOT_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$AOT_UID:$AOT_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure AOT_HOME is owned by the target user.
      ...
      if [ -n "''${AOT_HOME:-}" ] && [ -d "$AOT_HOME" ]; then
        find "$AOT_HOME" \! -user "$AOT_UID" -exec chown "$AOT_UID:$AOT_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/aot-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/aot-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/aot ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/aot
        chmod 0440 /etc/sudoers.d/aot
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$AOT_UID" --regid="$AOT_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $AOT_HOME/.env.
    ...
    # Default: /var/lib/aot/workspace → /data/workspace.
  ...
  in {
    options.services.aot-agent = with lib; {
      enable = mkEnableOption "Aot Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = aot-agent;
        description = "The aot-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/aot";
        description = "State directory. Contains .aot/ subdir (AOT_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Aot config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $AOT_HOME/.env at activation time.
          Aot reads this file on every startup via load_aot_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $AOT_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $AOT_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `aot gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the aot user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the aot plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Aot discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          aot_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as aot (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in aot-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the aot CLI to environment.systemPackages and export
          AOT_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.aot symlink to the service
            stateDir. These users are automatically added to the aot group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.aot-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the aot CLI to system PATH and export AOT_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.aot/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.AOT_HOME = "${cfg.stateDir}/.aot";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.aot-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the aot user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.aot-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed aot
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure aot is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.aot        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aot/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aot/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aot/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aot/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.aot/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."aot-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.aot
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.aot ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.aot ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.aot -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.aot/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.aot/$_subdir"
            chmod 2770 "${cfg.stateDir}/.aot/$_subdir"
            find "${cfg.stateDir}/.aot/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.aot/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.aot/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aot/config.yaml
            chmod 0640 ${cfg.stateDir}/.aot/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.aot/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aot/.managed
          chmod 0644 ${cfg.stateDir}/.aot/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.aot/.container-mode <<'AOT_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    aot_bin=${containerDataDir}/current-package/bin/aot
    AOT_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aot/.container-mode
            chmod 0644 ${cfg.stateDir}/.aot/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.aot/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.aot";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.aot" ]; then
                  rm -f "${symlinkPath}"
                  echo "aot-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.aot -> stateDir/.aot for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.aot";
                target = "${cfg.stateDir}/.aot";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "aot-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.aot/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.aot/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.aot/auth.json
              fi
            ''}
          ''}
          ...
          # Aot reads $AOT_HOME/.env at startup via load_aot_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.aot/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'AOT_NIX_ENV_EOF'
    ${envFileContent}
    AOT_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.aot/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.aot/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.aot/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.aot-agent = {
          description = "Aot Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $AOT_HOME/.env by the activation script. load_aot_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/aot"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the aot group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.aot-agent = {
          description = "Aot Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              AOT_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              AOT_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env AOT_UID="$AOT_UID" \
                --env AOT_GID="$AOT_GID" \
                --env AOT_HOME=${containerDataDir}/.aot \
                --env AOT_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/aot gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
