import type { RunExternalProcess } from '@aot/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`aot ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching aot: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`aot ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
