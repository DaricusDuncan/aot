import { useApp, useHasSelection, useSelection, useStdout, useTerminalTitle, type ScrollBoxHandle } from '@aot/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(model ? `${marker} ${model}${tabCwd ? ` · ${shortCwd(tabCwd, 24)}` : ''}` : 'Aot')
  ...
  const cwd = ui.info?.cwd || process.env.AOT_CWD || process.cwd()
