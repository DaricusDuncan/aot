...
// Mirrors @aot/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @aot/ink is bundled separately from packages/.
...
// Terminal. Explicit AOT_TUI_THEME / COLORFGBG signals above still win,
...
// `AOT_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `AOT_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `AOT_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `AOT_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Aot palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.AOT_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.AOT_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.AOT_TUI_BACKGROUND ?? '')
