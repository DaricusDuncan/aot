...
export const STARTUP_RESUME_ID = (process.env.AOT_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.AOT_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.AOT_TUI_IMAGE ?? '').trim()

const mouseTrackingOverride = parseToggle(process.env.AOT_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.AOT_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.AOT_TUI_NO_CONFIRM)

const inlineOverride = parseToggle(process.env.AOT_TUI_INLINE)
...
// less fragile. Override explicitly with AOT_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.AOT_TUI_FPS)
