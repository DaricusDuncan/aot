...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.AOT_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Aot on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via AOT_TUI_TRUECOLOR=1.
