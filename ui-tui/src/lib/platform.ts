 ...
 * aot-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``aot_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but aot-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS aot-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on aot-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is aot-ink's Alt signal and accepting it would
