...
await build({
  ...
  // Skip the prebuilt @aot/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@aot/ink': resolve(root, 'packages/aot-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The aot_cli launcher
