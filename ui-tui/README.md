...
React + Ink terminal UI for Aot. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `AOT_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.aot/.aot_history` or under `AOT_HOME`.
...
```text
ui-tui/
  packages/aot-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      aot-ink.d.ts    type declarations for @aot/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AotCLI subprocess for slash commands
```
