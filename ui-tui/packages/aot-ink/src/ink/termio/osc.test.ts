  ...
  it('honors explicit env override', () => {
    expect(
      shouldEmitClipboardSequence({
        AOT_TUI_CLIPBOARD_OSC52: '1',
        TMUX: '/tmp/tmux-1/default,1,0'
      } as NodeJS.ProcessEnv)
    ).toBe(true)
    expect(
      shouldEmitClipboardSequence({ AOT_TUI_COPY_OSC52: '0', TERM: 'xterm-256color' } as NodeJS.ProcessEnv)
    ).toBe(false)
  })

  it('AOT_TUI_FORCE_OSC52 takes precedence over TMUX suppression', () => {
    ...
    expect(
      shouldEmitClipboardSequence({
        AOT_TUI_FORCE_OSC52: '1',
        TMUX: '/tmp/t,1,0'
      } as NodeJS.ProcessEnv)
    ).toBe(true)
  })

  it('AOT_TUI_FORCE_OSC52=0 suppresses OSC 52 even for remote or plain terminals', () => {
    expect(
      shouldEmitClipboardSequence({
        AOT_TUI_FORCE_OSC52: '0',
        SSH_CONNECTION: '1'
      } as NodeJS.ProcessEnv)
    ).toBe(false)
  })
...
describe('shouldUseNativeClipboard', () => {
  ...
  it('returns true when OSC 52 emission is disabled via AOT_TUI_FORCE_OSC52=0', () => {
    ...
    expect(shouldUseNativeClipboard({ AOT_TUI_FORCE_OSC52: '0' } as NodeJS.ProcessEnv, 'ghostty')).toBe(true)
    expect(shouldUseNativeClipboard({ AOT_TUI_FORCE_OSC52: '0' } as NodeJS.ProcessEnv, 'kitty')).toBe(true)
    expect(shouldUseNativeClipboard({ AOT_TUI_CLIPBOARD_OSC52: '0' } as NodeJS.ProcessEnv, 'WezTerm')).toBe(true)
    expect(shouldUseNativeClipboard({ AOT_TUI_COPY_OSC52: 'no' } as NodeJS.ProcessEnv, 'vscode')).toBe(true)
  })

  it('returns true under TMUX even with AOT_TUI_FORCE_OSC52=1 on an allowlisted terminal (tmux load-buffer path)', () => {
    ...
    expect(
      shouldUseNativeClipboard({ AOT_TUI_FORCE_OSC52: '1', TMUX: '/tmp/t,1,0' } as NodeJS.ProcessEnv, 'ghostty')
