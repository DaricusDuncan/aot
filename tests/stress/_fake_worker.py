...
Reads AOT_KANBAN_TASK from env, heartbeats periodically, does short
work, completes via the CLI. Designed to be spawned by the dispatcher
exactly the way `aot chat -q` would be, minus the LLM cost.
"""
...
def main():
    tid = os.environ["AOT_KANBAN_TASK"]
    workspace = os.environ.get("AOT_KANBAN_WORKSPACE", "")

    # Announce via CLI (goes through real argparse + init_db + etc)
    subprocess.run(
        ["aot", "kanban", "heartbeat", tid, "--note", "started"],
    ...
    for i in range(3):
        time.sleep(0.3)
        subprocess.run(
            ["aot", "kanban", "heartbeat", tid, "--note", f"progress {i+1}/3"],
            check=True, capture_output=True,
        )
