...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        aot_home.mkdir()
        (aot_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(aot_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.aot/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        (aot_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        (aot_home / "secret.key").write_text("key")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.aot/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        aot_home = tmp_path / ".aot"
        (aot_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"AOT_HOME": str(aot_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.aot")

        assert mounts[0]["container_path"] == "/home/user/.aot/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.aot/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.aot/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape AOT_HOME."""
        monkeypatch.setenv("AOT_HOME", str(tmp_path / ".aot"))
        (tmp_path / ".aot").mkdir()

        # Create a sensitive file one level above aot_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside aot_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside AOT_HOME must still be registered."""
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        monkeypatch.setenv("AOT_HOME", str(aot_home))
        (aot_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_aot_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of AOT_HOME must be allowed."""
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        subdir = aot_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside AOT_HOME pointing outside must be rejected."""
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        monkeypatch.setenv("AOT_HOME", str(aot_home))

        # Create a sensitive file outside aot_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside aot_home pointing outside
        symlink = aot_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes AOT_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, aot_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape AOT_HOME."""
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        monkeypatch.setenv("AOT_HOME", str(aot_home))
        ...
        sensitive.write_text("{}")
        self._write_config(aot_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(aot_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside AOT_HOME via config must still mount."""
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        monkeypatch.setenv("AOT_HOME", str(aot_home))

        (aot_home / "oauth.json").write_text("{}")
        self._write_config(aot_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        (aot_home / "cache" / "documents").mkdir(parents=True)
        (aot_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("AOT_HOME", str(aot_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.aot/cache/documents" in paths
        assert "/root/.aot/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.aot/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        # Use legacy dir name — get_aot_dir prefers old if it exists
        (aot_home / "document_cache").mkdir()
        (aot_home / "image_cache").mkdir()
        monkeypatch.setenv("AOT_HOME", str(aot_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(aot_home / "document_cache") in host_paths
        assert str(aot_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.aot/cache/documents" in container_paths
        assert "/root/.aot/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        aot_home = tmp_path / ".aot"
        doc_dir = aot_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        aot_home = tmp_path / ".aot"
        ss_dir = aot_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.aot/cache/screenshots/session_abc/screen1.png"
