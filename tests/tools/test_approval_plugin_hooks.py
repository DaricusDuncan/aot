...
def isolated_session(monkeypatch):
    ...
    monkeypatch.setenv("AOT_SESSION_KEY", session_key)
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: AOT_INTERACTIVE is set, the
    ...
    def test_pre_and_post_fire_with_expected_kwargs(
        ...
        with patch("aot_cli.plugins.invoke_hook", side_effect=fake_invoke_hook):
    ...
    def test_plugin_hook_crash_does_not_break_approval(
        ...
        with patch("aot_cli.plugins.invoke_hook", side_effect=boom):
            result = check_all_command_guards(
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: AOT_GATEWAY_SESSION is set and a
