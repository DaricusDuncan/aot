...
def _restore_tool_modules():
    original_aot_home = os.environ.get("AOT_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_aot_home is None:
            os.environ.pop("AOT_HOME", None)
        else:
            os.environ["AOT_HOME"] = original_aot_home
        _reset_modules(("tools", "aot_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    aot_cli = types.ModuleType("aot_cli")
    aot_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["aot_cli"] = aot_cli
    aot_home = tmp_path / "aot-home"
    os.environ["AOT_HOME"] = str(aot_home)
    sys.modules["aot_cli.config"] = types.SimpleNamespace(
        get_aot_home=lambda: aot_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="aot-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": aot_home / "modal_snapshots.json",
