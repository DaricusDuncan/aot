...
``~/.aot/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.aot/.env`` would be told
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self):
        """Importing STT while aot_cli.config.get_env_value is patched must
    ...
    def test_xai_resolver_import_after_config_env_patch_uses_restored_dotenv_loader(self):
        ...
        try:
            with patch(
                ...
                "aot_cli.auth.resolve_xai_oauth_runtime_credentials",
                return_value={},
            ), patch(
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``aot_cli.config.load_env`` to simulate
    ``~/.aot/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch("aot_cli.config.load_env",
                   return_value={"XAI_API_KEY": "dotenv-secret"}):
