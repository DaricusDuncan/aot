...
Covers:
- :class:`AotTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.AotMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth import AotTokenStorage
from tools.mcp_oauth_manager import _AOT_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# AotTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = AotTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("AOT_HOME", str(tmp_path))
        storage = AotTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (AotMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: AotTokenStorage, **context_attrs):
    ...
    """
    if _AOT_PROVIDER_CLS is None:
        pytest.skip("MCP SDK auth not available")
    provider = _AOT_PROVIDER_CLS.__new__(_AOT_PROVIDER_CLS)
    provider._aot_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AOT_HOME", str(tmp_path))
        storage = AotTokenStorage("mgr-srv")
        ...
        with patch.object(
            _AOT_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AOT_HOME", str(tmp_path))
        storage = AotTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AOT_HOME", str(tmp_path))
        storage = AotTokenStorage("persist-srv")
        assert storage.load_oauth_metadata() is None
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AOT_HOME", str(tmp_path))
        storage = AotTokenStorage("noop-srv")
        ...
        with patch.object(
            AotTokenStorage, "save_oauth_metadata"
        ) as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AOT_HOME", str(tmp_path))
        storage = AotTokenStorage("flow-srv")
