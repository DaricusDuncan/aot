...
and the running Aot session picks up the new tokens on the next auth
...
async def test_external_refresh_picked_up_without_restart(tmp_path, monkeypatch):
    ...
    1. A running Aot session has OAuth tokens loaded in memory.
    ...
    # automatically via the AotMCPOAuthProvider.async_auth_flow
