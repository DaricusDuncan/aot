...
def _clean_state():
    ...
    saved = {}
    for k in ("AOT_INTERACTIVE", "AOT_GATEWAY_SESSION", "AOT_EXEC_ASK", "AOT_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["AOT_INTERACTIVE"] = "1"
        session_key = os.getenv("AOT_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No AOT_INTERACTIVE or AOT_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
