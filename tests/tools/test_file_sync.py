...
def _make_get_files(tmp_files, remote_base="/root/.aot"):
...
def _make_manager(tmp_files, remote_base="/root/.aot", upload=None, delete=None):
...
class TestEdgeCases:
    ...
    def test_file_disappears_between_list_and_upload(self, tmp_path):
        ...
        upload = MagicMock()
        mgr = FileSyncManager(
            get_files_fn=lambda: [(str(f), "/root/.aot/ephemeral.txt")],
            upload_fn=upload,
            delete_fn=MagicMock(),
        )
