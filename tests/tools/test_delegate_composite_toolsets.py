...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_aot_cli_expands_web(self):
        """aot-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"aot-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("aot-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['aot-cli'] yields ['web']."""
        parent_toolsets = {"aot-cli"}
