...
def test_aot_provider_subclass_exists():
    """AotMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _AOT_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _AOT_PROVIDER_CLS is not None
    assert issubclass(_AOT_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_aot_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns AotMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _AOT_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("AOT_HOME", str(tmp_path))
    ...
    assert _AOT_PROVIDER_CLS is not None
    assert isinstance(provider, _AOT_PROVIDER_CLS)
    assert provider._aot_server_name == "srv"
