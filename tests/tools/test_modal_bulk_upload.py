...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.aot/credentials/a.json"),
            (str(src_b), "/root/.aot/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.aot/credentials/a.json" in names
            assert "root/.aot/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.aot/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.aot/skills/b.py").read()
    ...
    def test_mkdir_includes_all_parents(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src), "/root/.aot/credentials/f.txt"),
            (str(src), "/root/.aot/skills/deep/nested/f.txt"),
        ]
        ...
        cmd = exec_calls[0][2]
        assert "/root/.aot/credentials" in cmd
        assert "/root/.aot/skills/deep/nested" in cmd
    ...
    def test_single_exec_call(self, monkeypatch, tmp_path):
        ...
        for i in range(20):
            ...
            files.append((str(src), f"/root/.aot/cache/file_{i}.txt"))
    ...
    def test_bulk_upload_wired_in_filesyncmanager(self, monkeypatch):
        ...
        env._sync_manager = modal_env.FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.aot"),
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.aot/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.aot/large.bin" in names
