...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because aot uses an internal scheduler, not system crontab.
    ...
    def test_accepts_gateway_session(self, monkeypatch):
        ...
        monkeypatch.setenv("AOT_GATEWAY_SESSION", "1")
        monkeypatch.delenv("AOT_EXEC_ASK", raising=False)
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("AOT_INTERACTIVE", raising=False)
        monkeypatch.delenv("AOT_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("AOT_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("AOT_INTERACTIVE", false_like_value)
        monkeypatch.delenv("AOT_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("AOT_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["AOT_INTERACTIVE", "AOT_GATEWAY_SESSION", "AOT_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("AOT_INTERACTIVE", "AOT_GATEWAY_SESSION", "AOT_EXEC_ASK"):
