...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. aot_cli/config.py:save_config_value
                       -> ``_config_to_env_sync`` dict (one-shot when the
                          user runs ``aot config set …``)
...
dicts) keeps the test independent of the user's ~/.aot/config.yaml and
mirrors the pattern used in tests/aot_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    """terminal config keys bridged by ``aot config set foo bar``."""
    from aot_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    """``aot config set terminal.X true`` must propagate to .env for
    ...
    assert not missing, (
        f"`aot config set terminal.X` doesn't sync these load-bearing "
        f"keys to .env: {sorted(missing)}.  Add them to _config_to_env_sync "
        f"in aot_cli/config.py:set_config_value."
    )
