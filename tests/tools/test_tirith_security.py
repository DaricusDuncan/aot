...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._aot_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_aot_bin(self):
        """After _INSTALL_FAILED, manual install in AOT_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        aot_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(aot_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(aot_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._aot_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == aot_bin
            assert _tirith_mod._resolved_path == aot_bin
            mock_clear.assert_called_once()
...
class TestAotHomeIsolation:
    def test_aot_bin_dir_respects_aot_home(self):
        """_aot_bin_dir must use AOT_HOME, not hardcoded ~/.aot."""
        from tools.tirith_security import _aot_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"AOT_HOME": tmpdir}):
            result = _aot_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_aot_home(self):
        """_failure_marker_path must use AOT_HOME, not hardcoded ~/.aot."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"AOT_HOME": "/custom/aot"}):
            result = _failure_marker_path()
        assert result == "/custom/aot/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets AOT_HOME; verify it's active."""
        aot_home = os.getenv("AOT_HOME")
        assert aot_home is not None, "AOT_HOME should be set by conftest"
        assert "aot_test" in aot_home, "Should point to test temp dir"

    def test_get_aot_home_fallback(self):
        """Without AOT_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_aot_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove AOT_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".aot")
            result = _get_aot_home()
