...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_aot_home_override(self, tmp_path):
        """Auth lookup should read from AOT_HOME/auth.json, not ~/.aot/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".aot").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``aot tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
