...
AlexKucera reported on Discord (2026-04-16) that after ``aot update`` pulled
...
Opt-in — not part of default CI:
    AOT_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.aot/.env).
"""
...
LIVE = os.environ.get("AOT_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set AOT_LIVE_TESTS=1"),
