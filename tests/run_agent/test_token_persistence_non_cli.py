...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    aot_state = ModuleType("aot_state")
    aot_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "aot_state", aot_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Aot"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Aot"
