...
COMMON_AOT_TOOLS = ["read_file", "search_files", "terminal", "patch", "write_file", "process"]


class TestToolKindMap:
    def test_all_aot_tools_have_kind(self):
        """Every common aot tool should appear in TOOL_KIND_MAP."""
        for tool in COMMON_AOT_TOOLS:
...
class TestBuildToolTitle:
    ...
    def test_execute_code_title_includes_first_code_line(self):
        ...
        assert title == "python: from aot_tools import terminal"

    def test_skill_manage_title_includes_action_and_target(self):
        title = build_tool_title(
            "skill_manage",
            {"action": "patch", "name": "aot-agent-operations", "file_path": "references/acp.md"},
        )
        assert title == "skill patch: aot-agent-operations/references/acp.md"
...
class TestBuildToolStart:
    ...
    def test_build_tool_start_for_skill_manage_patch_shows_diff(self):
        ...
        assert result.kind == "edit"
        assert result.title == "skill patch: aot-agent-operations/references/acp.md"
        assert isinstance(result.content[0], FileEditToolCallContent)
        assert result.content[0].path == "skills/aot-agent-operations/references/acp.md"
...
class TestBuildToolComplete:
    ...
    def test_build_tool_complete_for_skill_manage_summarizes_without_raw_json(self):
        result = build_tool_complete(
            "tc-skill-manage",
            "skill_manage",
            '{"success":true,"message":"Patched references/aot-acp-zed-rendering.md in skill \'aot-agent-operations\' (1 replacement)."}',
            function_args={
                "action": "patch",
                "name": "aot-agent-operations",
                "file_path": "references/aot-acp-zed-rendering.md",
            },
        )
        text = result.content[0].content.text
        assert "**✅ Skill updated**" in text
        assert "`patch`" in text
        assert "`aot-agent-operations`" in text
        assert "references/aot-acp-zed-rendering.md" in text
    ...
    def test_build_tool_complete_for_search_files_files_only_formats_file_list(self):
        result = build_tool_complete(
            "tc-search-files",
            "search_files",
            '{"total_count":36,"files":["/home/nour/.aot/config.yaml","/home/nour/.aot/profiles/recall-test/config.yaml"],"truncated":true}',
        )
        text = result.content[0].content.text
        assert "File search results" in text
        assert "Found 36 files; showing 2." in text
        assert "/home/nour/.aot/config.yaml" in text
    ...
    def test_build_tool_complete_for_write_file_summarizes_without_repeating_diff(self, tmp_path):
        ...
        target.write_text("hello from aot\n", encoding="utf-8")

        result = build_tool_complete(
            "tc-wf1",
            "write_file",
            '{"bytes_written": 18, "dirs_created": false}',
            function_args={"path": str(target), "content": "hello from aot\n"},
            snapshot=snapshot,
        )
        assert isinstance(result, ToolCallProgress)
        assert len(result.content) == 1
