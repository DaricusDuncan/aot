...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("aot/aot-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned aot/* and pr-* branches."""

    def test_prunes_orphaned_aot_branch(self, git_repo):
        """aot/aot-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "aot/aot-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "aot/aot-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "aot/aot-deadbeef" in result.stdout
        ...
        assert "aot/aot-deadbeef" in orphaned
        ...
        assert "aot/aot-deadbeef" not in result.stdout
