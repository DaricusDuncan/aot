"""Tests for cli.AotCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full AotCLI (which requires extensive setup).
"""
...
def test_gate_off_returns_once_without_prompting():
    ...
    with patch(
        ...
        result = _bound(AotCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
...
def test_slash_confirm_modal_number_selection_submits_without_raw_input():
    ...
    _bound(AotCLI._submit_slash_confirm_response, self_)("always")
...
def test_slash_confirm_display_fragments_include_choice_mapping():
    ...
    fragments = _bound(AotCLI._get_slash_confirm_display_fragments, self_)()
