...
def session_db(tmp_path):
    ...
    os.environ["AOT_HOME"] = str(tmp_path / ".aot")
    os.makedirs(tmp_path / ".aot", exist_ok=True)
    from aot_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".aot" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal AotCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        AotCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        AotCLI._handle_branch_command(cli_instance, "/branch refactor approach")
