...
Verifies that ``AotCLI._handle_update_command`` correctly:
...
Also verifies that ``aot_cli.main._launch_tui`` correctly handles exit
...
def _make_self(modal_response):
    ...
    so we don't need a full ``AotCLI`` construction.
    ...
    self_._normalize_slash_confirm_choice = _bound(
        AotCLI._normalize_slash_confirm_choice, self_
    )
    return self_
...
def _call(self_):
    ...
    return AotCLI._handle_update_command(self_)
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        patch("aot_cli.config.is_managed", return_value=True),
        patch(
            "aot_cli.config.format_managed_message",
            return_value="Use `brew upgrade aot-agent` to update.",
        ),
    ):
        result = _call(self_)

    out = capsys.readouterr().out
    assert "brew upgrade aot-agent" in out
...
def test_affirmative_answer_sets_pending_relaunch_and_returns_true(answer, capsys):
    ...
    with patch("aot_cli.config.is_managed", return_value=False):
        result = _call(self_)
