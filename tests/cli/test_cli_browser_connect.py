...
class TestChromeDebugLaunch:
    def test_windows_launch_uses_browser_found_on_path(self):
        ...
        with patch("aot_cli.browser_connect.shutil.which", side_effect=lambda name: r"C:\Chrome\chrome.exe" if name == "chrome.exe" else None), \
             patch("aot_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == r"C:\Chrome\chrome.exe"), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert AotCLI._try_launch_chrome_debug(9333, "Windows") is True
    ...
    def test_windows_launch_falls_back_to_common_install_dirs(self, monkeypatch):
        ...
        with patch("aot_cli.browser_connect.shutil.which", return_value=None), \
             patch("aot_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == installed), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert AotCLI._try_launch_chrome_debug(9222, "Windows") is True

        _assert_chrome_debug_cmd(captured["cmd"], installed, 9222)

    def test_manual_command_uses_detected_linux_browser(self):
        with patch("aot_cli.browser_connect.shutil.which", side_effect=lambda name: "/usr/bin/chromium" if name == "chromium" else None), \
             patch("aot_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == "/usr/bin/chromium"):
    ...
    def test_manual_command_uses_wsl_windows_chrome_when_available(self):
        ...
        with patch("aot_cli.browser_connect.shutil.which", return_value=None), \
             patch("aot_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == chrome):
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_manual_command_uses_windows_quoting_on_windows(self):
        ...
        with patch("aot_cli.browser_connect.shutil.which", side_effect=lambda name: chrome if name == "chrome.exe" else None), \
             patch("aot_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == chrome):
    ...
    def test_manual_command_returns_none_when_linux_browser_missing(self):
        with patch("aot_cli.browser_connect.shutil.which", return_value=None), \
             patch("aot_cli.browser_connect.os.path.isfile", return_value=False):
