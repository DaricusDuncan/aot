...
def _isolate(tmp_path, monkeypatch):
    """Isolate AOT_HOME so tests don't touch real config."""
    home = tmp_path / ".aot"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal AotCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = AotCLI.__new__(AotCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["aot-core"]
        obj.compact = False
