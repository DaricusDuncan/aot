...
class TestToolsetConsistency:
    ...
    def test_aot_platforms_share_core_tools(self):
        """All aot-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on aot-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["aot-cli", "aot-telegram", "aot-discord", "aot-whatsapp", "aot-slack", "aot-signal", "aot-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_aot_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("aot-whatsapp")

    def test_aot_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("aot-api-server")
