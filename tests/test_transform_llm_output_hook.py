...
from aot_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(aot_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <aot_home>/plugins/<name> and opt it in."""
    plugin_dir = aot_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    aot_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        aot_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AOT_HOME", str(tmp_path / "aot_empty"))
