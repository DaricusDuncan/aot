...
from aot_cli.copilot_auth import _try_gh_cli_token
...
PROVIDER_ENV_VARS = (
    ...
    "OPENAI_BASE_URL", "AOT_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "AOT_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("aot_cli.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("aot_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        monkeypatch.setenv("AOT_COPILOT_ACP_ARGS", "--acp --stdio --debug")
        monkeypatch.setattr("aot_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("aot_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr("aot_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("aot_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("aot_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "aot_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "aot_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("aot_cli.copilot_auth.subprocess.run", _fake_run)
    ...
    def test_resolve_copilot_acp_with_local_cli(self, monkeypatch):
        monkeypatch.setenv("AOT_COPILOT_ACP_ARGS", "--acp --stdio")
        monkeypatch.setattr("aot_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("aot_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "aot_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "aot_cli.models.fetch_github_model_catalog",
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from aot_cli import config as config_module
        monkeypatch.setenv("GLM_API_KEY", "test-key")
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: aot_home / ".env")
        monkeypatch.setattr(config_module, "get_aot_home", lambda: aot_home)
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Aot is unconfigured."""
        from aot_cli import config as config_module
        ...
        monkeypatch.setattr(config_module, "get_env_path", lambda: aot_home / ".env")
        monkeypatch.setattr(config_module, "get_aot_home", lambda: aot_home)
        monkeypatch.setattr("aot_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("aot_cli.auth.get_auth_status", lambda _pid: {})
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        config_file = aot_home / "config.yaml"
    ...
    def test_claude_code_creds_counted_when_aot_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Aot has been explicitly configured."""
        import yaml
        from aot_cli import config as config_module
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "aot_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("aot_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from aot_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from aot_cli.main import _PROVIDER_MODELS as main_models
        from aot_cli.models import _PROVIDER_MODELS as models_models
...
class TestNovitaProvider:
    ...
    def test_novita_pricing_cache(self, monkeypatch):
        ...
        from aot_cli import models as models_mod
...
class TestMinimaxOAuthProvider:
    ...
    def test_minimax_oauth_listed_in_canonical_providers(self):
        from aot_cli.models import CANONICAL_PROVIDERS
