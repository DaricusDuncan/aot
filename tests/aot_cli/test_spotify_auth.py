...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from aot_cli.auth_commands import auth_status_command
...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes AOT_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("AOT_HOME", str(tmp_path))
    ...
    assert "AOT_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "AOT_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "AOT_SPOTIFY_CLIENT_ID" not in env_path.read_text()
