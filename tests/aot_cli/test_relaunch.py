"""Tests for aot_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from aot_cli import relaunch as relaunch_mod


class TestResolveAotBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/aot"
        ...
        assert relaunch_mod.resolve_aot_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'aot' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_aot_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/aot" if name == "aot" else None
        )
        assert relaunch_mod.resolve_aot_bin() == "/usr/bin/aot"

    def test_returns_none_when_unresolvable(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_aot_bin() is None
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_aot_bin", lambda: "/usr/bin/aot")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/aot"

    def test_falls_back_to_python_module(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_aot_bin", lambda: None)
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv == [sys.executable, "-m", "aot_cli.main", "--resume", "abc"]
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/aot", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/aot", ["/usr/bin/aot", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        aot).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_aot_bin", lambda: r"C:\Users\test\aot.exe")
        ...
        assert execvp_calls == []
        assert captured_argv == [[r"C:\Users\test\aot.exe", "chat"]]
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_aot_bin", lambda: r"C:\aot.exe")
    ...
    def test_windows_surfaces_oserror_with_help(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_aot_bin", lambda: r"C:\missing.exe")
...
class TestResolveAotBinWindowsPyGuard:
    """On Windows, resolve_aot_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a aot.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\aot.exe" if name == "aot" else None,
        )

        bin_path = relaunch_mod.resolve_aot_bin()
        # Must NOT be the .py — must be the aot.exe PATH entry.
        assert bin_path == r"C:\venv\Scripts\aot.exe"
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_aot_bin() == str(script)

    def test_windows_py_argv0_with_no_aot_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND aot.exe
        isn't on PATH, return None so the caller falls back to
        python -m aot_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
