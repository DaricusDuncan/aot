...
board into `AOT_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`aot kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `aot kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `AOT_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("AOT_KANBAN_BOARD")
    ...
    try:
        ...
        if prev is None:
            ...
            os.environ["AOT_KANBAN_BOARD"] = prev


def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    main_mod = importlib.import_module("aot_cli.main")

    import aot_cli.kanban_db as kdb
    monkeypatch.setattr(kdb, "get_current_board", lambda: "space")

    main_mod._pin_kanban_board_env()

    assert main_mod.os.environ.get("AOT_KANBAN_BOARD") == "space"


def test_pin_does_not_overwrite_existing_env(monkeypatch):
    monkeypatch.setenv("AOT_KANBAN_BOARD", "preset")
    main_mod = importlib.import_module("aot_cli.main")
    ...
    assert main_mod.os.environ.get("AOT_KANBAN_BOARD") == "preset"
...
def test_pin_swallows_resolution_failures(monkeypatch):
    ...
    assert "AOT_KANBAN_BOARD" not in main_mod.os.environ
