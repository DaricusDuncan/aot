...
The terminal `aot model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import aot_cli.providers as providers_mod
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "AOT_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_named_custom_provider(monkeypatch):
    ...
    monkeypatch.setattr("aot_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("aot_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("aot_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Aot's own writer stores configured models as a dict
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("aot_cli.models.fetch_lmstudio_models", _fake_fetch)
