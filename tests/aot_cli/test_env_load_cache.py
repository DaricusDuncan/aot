...
The cache exists to keep `aot tools` → "All Platforms" fast: every
...
def test_load_env_caches_on_repeat_calls():
    ...
    from aot_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("aot_cli.config.get_env_path", return_value=env_path):
            first = load_env()
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from aot_cli import config as config_mod
    from aot_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_aot_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "aot-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("aot_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
