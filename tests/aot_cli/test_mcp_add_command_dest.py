"""Regression test: ``aot mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``aot_cli/_parser.py``).  The dispatcher in ``aot_cli/main.py``
...
result, ``aot mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the aot parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`aot mcp add foo --url ...` must keep args.command == "mcp".
