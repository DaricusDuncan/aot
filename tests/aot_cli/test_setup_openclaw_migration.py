...
class TestOfferOpenclawMigration:
    ...
    def test_skips_when_no_openclaw_dir(self, tmp_path):
        ...
        with patch("aot_cli.setup.Path.home", return_value=tmp_path):
            assert setup_mod._offer_openclaw_migration(tmp_path / ".aot") is False
    ...
    def test_skips_when_migration_script_missing(self, tmp_path):
        ...
        openclaw_dir.mkdir()
        with (
            patch("aot_cli.setup.Path.home", return_value=tmp_path),
    ...
    def test_skips_when_user_declines(self, tmp_path):
        ...
        openclaw_dir = tmp_path / ".openclaw"
        openclaw_dir.mkdir()
        script = tmp_path / "openclaw_to_aot.py"
    ...
    def test_runs_migration_when_user_accepts(self, tmp_path):
        ...
        # Create a fake aot home with config
        ...
        fake_mod = ModuleType("openclaw_to_aot")
        ...
        fake_migrator.migrate.return_value = {
            ...
            "output_dir": str(aot_home / "migration"),
        }
        ...
        with (
            ...
            result = setup_mod._offer_openclaw_migration(aot_home)
    ...
    def test_creates_config_if_missing(self, tmp_path):
        ...
        with (
            ...
            setup_mod._offer_openclaw_migration(aot_home)
...
class TestSetupWizardOpenclawIntegration:
    ...
    def test_migration_offered_during_first_time_setup(self, tmp_path):
        ...
        with (
            patch.object(setup_mod, "ensure_aot_home"),
            patch.object(setup_mod, "load_config", return_value={}),
            patch.object(setup_mod, "get_aot_home", return_value=tmp_path),
            patch.object(setup_mod, "get_env_value", return_value=""),
            patch.object(setup_mod, "is_interactive_stdin", return_value=True),
            patch("aot_cli.auth.get_active_provider", return_value=None),
...
class TestGetSectionConfigSummary:
    ...
    def test_gateway_returns_none_without_tokens(self):
        # _platform_status reads via aot_cli.gateway.get_env_value, not
    ...
    def test_gateway_lists_platforms(self):
        ...
        # reads from aot_cli.gateway.get_env_value after the setup
    ...
    def test_gateway_matches_platform_registry(self):
        ...
        from aot_cli.gateway import _PLATFORMS
...
class TestSetupWizardSkipsConfiguredSections:
    ...
    def test_sections_skipped_when_migration_imported_settings(self, tmp_path):
        ...
        # vars via aot_cli.gateway.get_env_value, NOT setup_mod's. Patch
