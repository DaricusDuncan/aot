"""Shared fixtures for the aot-agent test suite.
...
2. **Isolated AOT_HOME.** AOT_HOME points to a per-test tempdir so
   code reading ``~/.aot/*`` via ``get_aot_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".aot"`` instead of the canonical
   ``get_aot_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No AOT_SESSION_* inheritance** — the agent's current gateway
...
# AOT_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_AOT_BEHAVIORAL_VARS = frozenset({
    "AOT_YOLO_MODE",
    "AOT_INTERACTIVE",
    "AOT_QUIET",
    "AOT_TOOL_PROGRESS",
    ...
    "AOT_SESSION_CHAT_ID",
    "AOT_SESSION_CHAT_NAME",
    "AOT_SESSION_THREAD_ID",
    "AOT_SESSION_SOURCE",
    "AOT_SESSION_KEY",
    "AOT_GATEWAY_SESSION",
    "AOT_PLATFORM",
    ...
    "AOT_TIMEZONE",
    "AOT_REDACT_SECRETS",
    "AOT_BACKGROUND_NOTIFICATIONS",
    "AOT_EXEC_ASK",
    "AOT_HOME_MODE",
    "AOT_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.aot/kanban.db instead of the per-test AOT_HOME.
    "AOT_KANBAN_DB",
    "AOT_KANBAN_BOARD",
    "AOT_KANBAN_HOME",
    "AOT_KANBAN_WORKSPACES_ROOT",
    "AOT_KANBAN_LOGS_ROOT",
    "AOT_KANBAN_TASK",
    "AOT_KANBAN_WORKSPACE",
    "AOT_KANBAN_RUN_ID",
    "AOT_KANBAN_CLAIM_LOCK",
    "AOT_KANBAN_DISPATCH_IN_GATEWAY",
    "AOT_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and AOT_HOME to per-test tempdirs so code that
    reads ``~/.aot/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral AOT_* vars that could change test semantics.
    for name in _AOT_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect AOT_HOME to a per-test tempdir. Code that reads
    #    ``~/.aot/*`` via ``get_aot_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.aot/*`` via
    #    ``Path.home() / ".aot"`` instead of ``get_aot_home()``
    #    is a bug to fix at the callsite.
    fake_aot_home = tmp_path / "aot_test"
    fake_aot_home.mkdir()
    (fake_aot_home / "sessions").mkdir()
    (fake_aot_home / "cron").mkdir()
    (fake_aot_home / "memories").mkdir()
    (fake_aot_home / "skills").mkdir()
    monkeypatch.setenv("AOT_HOME", str(fake_aot_home))
    ...
    #    ~/.aot/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import aot_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_aot_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "aot_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal aot config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``aot-gateway`` process
...
# correlated with a ``tests/aot_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> aot-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart aot-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting aot/python patterns are also blocked.
    """
    ...
    def _matches_aot_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _AOT_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_aot_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch aot-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m aot_cli.main".
                if (
                    "aot" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live aot-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting aot/python could hit the live gateway. "
