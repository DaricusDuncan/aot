"""
Tests for mcp_serve — Aot MCP server.
...
def _isolate_aot_home(tmp_path, monkeypatch):
    """Redirect AOT_HOME to a temp directory."""
    monkeypatch.setenv("AOT_HOME", str(tmp_path))
    try:
        import aot_constants
        monkeypatch.setattr(aot_constants, "get_aot_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking aot_state schema."""
