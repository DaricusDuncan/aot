...
See: https://github.com/NousResearch/aot-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for aot_constants.get_subprocess_home()."""

    def test_returns_none_when_aot_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("AOT_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("AOT_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from aot_constants import (
            get_aot_home,
            reset_aot_home_override,
            set_aot_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_aot_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_aot_home_override(profile)
        try:
            assert get_aot_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_aot_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_aot_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(aot_home / "home")
    ...
    def test_no_injection_when_aot_home_unset(self, monkeypatch):
        monkeypatch.delenv("AOT_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["AOT_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from aot_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from aot_cli.profiles import create_profile
