...
def server():
    with patch.dict(
        "sys.modules",
        {
            "aot_constants": MagicMock(
                get_aot_home=MagicMock(return_value="/tmp/aot_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'aot-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'aot-release' patched"
    }
