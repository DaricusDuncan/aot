...
Tests the new --gateway mode for aot update, including:
...
def _make_runner(aot_home=None):
...
class TestGatewayPrompt:
    ...
    def test_writes_prompt_file_and_reads_response(self, tmp_path):
        ...
        def write_response():
            time.sleep(0.3)
            (aot_home / ".update_response").write_text("y")

        thread = threading.Thread(target=write_response)
        thread.start()

        with patch.dict(os.environ, {"AOT_HOME": str(aot_home)}):
            from aot_cli.main import _gateway_prompt
            result = _gateway_prompt("Restore? [Y/n]", "y", timeout=5.0)

        thread.join()
        assert result == "y"
        # Both files should be cleaned up
        assert not (aot_home / ".update_prompt.json").exists()
        assert not (aot_home / ".update_response").exists()
    ...
    def test_prompt_file_content(self, tmp_path):
        ...
        def capture_and_respond():
            ...
            prompt_path = aot_home / ".update_prompt.json"
            for _ in range(20):
                if prompt_path.exists():
                    prompt_data = json.loads(prompt_path.read_text())
                    (aot_home / ".update_response").write_text("n")
                    return
                time.sleep(0.1)
    ...
    def test_empty_response_returns_default(self, tmp_path):
        ...
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        (aot_home / ".update_response").write_text("")
...
class TestRestoreStashWithInputFn:
    ...
    def test_uses_input_fn_when_provided(self, tmp_path):
        ...
        from aot_cli.main import _restore_stashed_changes
...
class TestUpdateCommandGatewayFlag:
    """Verify the gateway spawns aot update --gateway."""
...
class TestWatchUpdateProgress:
    ...
    async def test_streams_output_to_adapter(self, tmp_path):
        ...
        # Write output
        (aot_home / ".update_output.txt").write_text("→ Fetching updates...\n", encoding="utf-8")
    ...
    async def test_detects_and_forwards_prompt(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (aot_home / ".update_pending.json").write_text(json.dumps(pending))
        (aot_home / ".update_output.txt").write_text("output\n")
        ...
        async def simulate_prompt_cycle():
            ...
            (aot_home / ".update_prompt.json").write_text(json.dumps(prompt))
            # Simulate user responding
            await asyncio.sleep(0.5)
            (aot_home / ".update_response").write_text("y")
            (aot_home / ".update_prompt.json").unlink(missing_ok=True)
            await asyncio.sleep(0.3)
            (aot_home / ".update_exit_code").write_text("0")
    ...
    async def test_prompt_forwarding_preserves_thread_metadata(self, tmp_path):
        ...
        (aot_home / ".update_pending.json").write_text(json.dumps(pending))
        (aot_home / ".update_output.txt").write_text("")
        (aot_home / ".update_prompt.json").write_text(json.dumps({
    ...
    async def test_failure_exit_code(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222",
                   "session_key": "agent:main:telegram:dm:111"}
        (aot_home / ".update_pending.json").write_text(json.dumps(pending))
        (aot_home / ".update_output.txt").write_text("error occurred\n")
    ...
    async def test_prompt_is_recovered_after_watcher_restart(self, tmp_path):
        ...
        with patch("gateway.run._aot_home", aot_home):
            ...
            assert (aot_home / ".update_prompt.json").exists()
...
class TestUpdatePromptInterception:
    ...
    async def test_intercepts_response_when_prompt_pending(self, tmp_path):
        ...
        (aot_home / ".update_prompt.json").write_text(json.dumps({"prompt": "test"}))
        ...
        response_path = aot_home / ".update_response"
