...
class TestTelegramYamlConfigLoading:
    ...
    def _write_config(self, tmp_path, content: str):
        ...
        aot_home.mkdir()
        (aot_home / "config.yaml").write_text(content, encoding="utf-8")
        return aot_home
    ...
    def test_top_level_reply_to_mode_off(self, tmp_path, monkeypatch):
        ...
        aot_home = self._write_config(tmp_path, "telegram:\n  reply_to_mode: off\n")
...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/NousResearch/aot-agent/issues/23994:
    reply_to_mode 'off' was ignored when sending via Aot-created DM topic
