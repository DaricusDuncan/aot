...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        # Required env vars are the user-facing AOT_ prefixed keys.
        assert "AOT_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "AOT_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        # Isolated AOT_HOME so we don't read the developer's config.yaml.
        home = tmp_path / ".aot"
        home.mkdir()
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_returns_none_without_credentials(self, monkeypatch):
        for k in (
            "AOT_LANGFUSE_PUBLIC_KEY", "AOT_LANGFUSE_SECRET_KEY",
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("AOT_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
    ...
    def test_get_langfuse_does_not_import_aot_config(self, monkeypatch):
        ...
        # Drop any cached import of aot_cli.config.
        sys.modules.pop("aot_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "aot_cli.config" not in sys.modules, (
            "langfuse plugin imported aot_cli.config — regression toward "
...
# AOT_LANGFUSE_PUBLIC_KEY / SECRET_KEY at a template value like
...
# Aot logs.  The fix in `_get_langfuse()` validates the documented
...
class TestPlaceholderKeyDetection:
    ...
    def test_validate_langfuse_key_accepts_documented_prefix(self, monkeypatch):
        ...
        assert plugin._validate_langfuse_key(
            "AOT_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz"
        ) is None
        assert plugin._validate_langfuse_key(
            "AOT_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz"
        ) is None
    ...
    def test_validate_langfuse_key_rejects_wrong_prefix(self, monkeypatch):
        ...
        msg = plugin._validate_langfuse_key(
            "AOT_LANGFUSE_PUBLIC_KEY", "placeholder"
        )
        assert msg is not None
        assert "AOT_LANGFUSE_PUBLIC_KEY" in msg
        assert "pk-lf-" in msg
    ...
    def test_validate_langfuse_key_unknown_name_passes(self, monkeypatch):
        ...
        self._clear_env(monkeypatch)
        plugin = self._fresh_plugin()
        assert plugin._validate_langfuse_key("AOT_LANGFUSE_BASE_URL", "anything") is None
    ...
    def test_placeholder_public_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("AOT_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("AOT_LANGFUSE_SECRET_KEY", "sk-lf-real-secret-xyz")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "AOT_LANGFUSE_PUBLIC_KEY" in text
    ...
    def test_placeholder_secret_key_warns_and_skips(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("AOT_LANGFUSE_PUBLIC_KEY", "pk-lf-real-public-xyz")
        monkeypatch.setenv("AOT_LANGFUSE_SECRET_KEY", "test-key")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        text = caplog.text
        assert "AOT_LANGFUSE_SECRET_KEY" in text
    ...
    def test_both_placeholders_one_warning_with_both_keys(self, monkeypatch, caplog):
        self._clear_env(monkeypatch)
        monkeypatch.setenv("AOT_LANGFUSE_PUBLIC_KEY", "placeholder")
        monkeypatch.setenv("AOT_LANGFUSE_SECRET_KEY", "placeholder")
        plugin = self._fresh_plugin(monkeypatch)
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
    ...
    def test_common_placeholders_detected(self, monkeypatch, caplog, placeholder):
        ...
        with caplog.at_level(logging.WARNING, logger=self.LOGGER_NAME):
            assert plugin._get_langfuse() is None
        assert "AOT_LANGFUSE_PUBLIC_KEY" in caplog.text

    def test_legacy_LANGFUSE_PUBLIC_KEY_also_validated(self, monkeypatch, caplog):
        """The plugin reads both the canonical AOT_-prefixed env var and
        ...
        # AOT_-prefixed one — operators set the AOT_-prefixed one).
        assert "AOT_LANGFUSE_PUBLIC_KEY" in caplog.text
