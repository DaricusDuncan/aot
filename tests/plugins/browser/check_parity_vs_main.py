...
Run from the PR worktree:
    ...
    cd ~/.aot/aot-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.aot/aot-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.aot/aot-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a aot-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a aot-agent checkout"
)
...
# Isolated AOT_HOME for the config write.
