...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate AOT_HOME for each test.

    The global hermetic fixture already redirects AOT_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    AOT_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "aot_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "aot_plugins" not in sys.modules:
        ns = types.ModuleType("aot_plugins")
        ns.__path__ = []
        sys.modules["aot_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "aot_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["aot_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_aot_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_aot_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_aot_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/aot-abc/x.log")) is True
...
class TestBundledDiscovery:
    def _write_enabled_config(self, aot_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from aot_cli import plugins as pmod
