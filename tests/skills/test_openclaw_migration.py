...
"""
    ...
    assert "Tyler Williams > Active Projects: Aot Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `aot cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # aot_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.aot/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "aot_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Aot``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Aot prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Aot to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Aot config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Aot uses tools well"
    # All-lowercase matches → lowercase ``aot``; this preserves the
    # real filesystem path ``~/.aot`` (Aot home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "aot should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Aot remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "aot prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Aot was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "aot uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Aot said to ask Aot about Aot settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.aot`` (the real Aot home), not the broken
    ``.Aot``.
    ...
    rewritten to ``~/.Aot/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.aot/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .aot directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.aot'"
