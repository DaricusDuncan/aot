...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Aot passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("AOT_HOME", "/users/alice/.aot/profiles/backend-worker")
        monkeypatch.setenv("AOT_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "AOT_KANBAN_DB",
            "/users/alice/.aot/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.aot/kanban/boards/smoke"]'
            in cmd
        )
