...
def test_aot_client_tag_includes_current_version():
    """The client tag must reflect aot_cli.__version__ verbatim."""
    from aot_cli import __version__
    from agent.portal_tags import aot_client_tag

    assert aot_client_tag() == f"client=aot-client-v{__version__}"


def test_aot_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import aot_client_tag

    tag = aot_client_tag()
    assert tag.startswith("client=aot-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import aot_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=aot-agent" in tags
    assert aot_client_tag() in tags
    assert len(tags) == 2
