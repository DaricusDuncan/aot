...
@pytest.fixture
def aot_home(monkeypatch):
    """Isolate AOT_HOME for ``reload_skills`` tests.
    ...
    module-level ``AOT_HOME`` / ``SKILLS_DIR`` constants in place so the
    isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="aot-reload-skills-")
    monkeypatch.setenv("AOT_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "AOT_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_returns_expected_keys(self, aot_home):
    ...
    def test_detects_newly_added_skill_with_description(self, aot_home):
        from agent.skill_commands import reload_skills, get_skill_commands

        # Prime the cache so subsequent diff is meaningful
        get_skill_commands()

        _write_skill(aot_home / "skills", "demo", "a demo skill")
        result = reload_skills()
    ...
    def test_detects_removed_skill_carries_description(self, aot_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(aot_home / "skills", "demo", "soon to be gone")
    ...
    def test_description_passes_through_verbatim(self, aot_home):
        ...
        long_desc = "x" * 200
        _write_skill(aot_home / "skills", "longdesc", long_desc)

        result = reload_skills()
        assert len(result["added"]) == 1
        assert result["added"][0]["description"] == long_desc

    def test_unchanged_skills_appear_in_unchanged_list(self, aot_home):
        from agent.skill_commands import reload_skills, get_skill_commands

        _write_skill(aot_home / "skills", "alpha")
        # Prime cache
        get_skill_commands()
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, aot_home):
