...
""",
        ...
        from aot_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        aot_home = tmp_path / ".aot"
        aot_home.mkdir()
        (aot_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("AOT_HOME", str(aot_home))
        from aot_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from aot_cli.plugins import PluginContext, PluginManifest, PluginManager
