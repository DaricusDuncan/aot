...
class TestSlackNativeSlashes:
    ...
    def test_aot_catchall_is_first(self):
        """``/aot`` must be reserved as the first slot so the legacy
        ``/aot <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "aot"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /aot <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `aot slack manifest`)."""
...
from aot_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch aot_cli.plugins.get_plugin_commands() to a fixed dict."""
        from aot_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/aot metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
