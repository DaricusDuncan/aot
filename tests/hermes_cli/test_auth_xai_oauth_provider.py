"""Tests for xAI Grok OAuth — tokens stored in Aot auth store (~/.aot/auth.json)."""
...
def _setup_aot_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Aot auth store at the given root."""
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("aot_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_aot_agent():
    """Attribution: xAI's OAuth server can identify Aot-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "aot-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "aot auth add xai-oauth" in body
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    aot_home.mkdir(parents=True, exist_ok=True)
    (aot_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    aot_home = tmp_path / "aot"
    _setup_aot_auth(aot_home, access_token="")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    aot_home = tmp_path / "aot"
    _setup_aot_auth(aot_home, refresh_token="")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    aot_home = tmp_path / "aot"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_aot_auth(aot_home, access_token=fresh)
    monkeypatch.setenv("AOT_HOME", str(aot_home))
    monkeypatch.delenv("AOT_XAI_BASE_URL", raising=False)
    ...
    assert creds["source"] == "aot-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("aot_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_aot_auth(aot_home, access_token=fresh)
    monkeypatch.setenv("AOT_HOME", str(aot_home))
    monkeypatch.setenv("AOT_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
def _seed_xai_oauth_state(
    aot_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    aot_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (aot_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    aot_home = tmp_path / "aot"
    _seed_xai_oauth_state(aot_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
    ...
    monkeypatch.setattr("aot_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((aot_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    aot_home = tmp_path / "aot"
    _seed_xai_oauth_state(aot_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("AOT_HOME", str(aot_home))
    ...
    monkeypatch.setattr("aot_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``aot model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``aot model``
    ...
    assert "aot model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from aot_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Aot that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("aot_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `aot model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `aot auth list` reflects truth and
    refreshes route through the pool consistently with codex."""
    from agent.credential_pool import load_pool

    aot_home = tmp_path / "aot"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_aot_auth(aot_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`aot auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `aot auth remove`.
    from aot_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``aot auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `aot auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((aot_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Aot invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_aot_auth(aot_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
    ...
    monkeypatch.setattr("aot_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from aot_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("aot_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    aot_home = tmp_path / "aot"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_aot_auth(aot_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Aot process refreshed the singleton
    ...
    profiles + Aot processes."""
    from agent.credential_pool import load_pool

    aot_home = tmp_path / "aot"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_aot_auth(aot_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
    ...
    (aot_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_aot_auth(aot_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``aot model`` -> xAI Grok OAuth (or another process
    ...
    _setup_aot_auth(aot_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
    ...
    # Simulate the user re-running `aot model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``aot auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_aot_auth(aot_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`aot auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``aot auth status``, ``aot setup``, and the
    ``aot`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    aot_home = tmp_path / "aot"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_aot_auth(aot_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("AOT_HOME", str(aot_home))
