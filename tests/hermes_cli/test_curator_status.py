"""Tests for `aot curator status` output.
...
def test_status_uses_last_activity_not_only_last_used(monkeypatch, capsys):
    ...
    import aot_cli.curator as curator_cli
...
def curator_status_env(tmp_path, monkeypatch):
    """Isolated AOT_HOME with real agent-created skills on disk."""
    home = tmp_path / ".aot"
    skills = home / "skills"
    ...
    from aot_cli import curator as curator_cli
