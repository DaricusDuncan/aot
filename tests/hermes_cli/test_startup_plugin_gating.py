...
``aot_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``aot --help``, ``aot version``,
``aot logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``aot --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from aot_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["aot", "--tui", "chat"], "chat"),
        (["aot", "-w", "logs"], "logs"),
        (["aot", "chat", "hello world"], "chat"),
        (["aot", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["aot", "-m", "gpt5", "chat"], "chat"),
        (["aot", "--model", "gpt5", "chat", "hi"], "chat"),
        (["aot", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["aot", "-z", "hello world"], None),
        (["aot", "-z", "hello", "chat"], "chat"),
        (["aot", "--model=gpt5", "chat"], "chat"),     # inline form
        (["aot", "--", "chat"], "chat"),               # -- terminator
        (["aot", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["aot", "some-plugin-cmd"], "some-plugin-cmd"),
        (["aot", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["aot"],                          # bare → chat
        ["aot", "--help"],                # top-level help
        ["aot", "-h"],
        ["aot", "version"],               # known built-in
        ["aot", "logs"],
        ["aot", "gateway", "run"],
        ["aot", "--tui"],
        ["aot", "-w", "--tui"],
        ["aot", "chat", "hi"],
        ["aot", "help"],                  # accepted built-in-ish
        ["aot", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["aot", "meet", "join"],          # potential google_meet plugin
        ["aot", "honcho", "status"],      # potential memory plugin
        ["aot", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``aot help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"aot_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
