"""Tests for the `aot memory reset` CLI command.
...
- Profile-scoped reset (uses AOT_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake AOT_HOME with memory files."""
    aot_home = tmp_path / ".aot"
    memories = aot_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("AOT_HOME", str(aot_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAot repo is at ~/.aot/aot-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `aot memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        aot_home = tmp_path / ".aot"
        (aot_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("AOT_HOME", str(aot_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's AOT_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        aot_home = tmp_path / ".aot"
        aot_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("AOT_HOME", str(aot_home))

        # The memories dir won't exist; get_aot_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
