"""Tests for aot_cli.security_advisories.
...
import aot_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect AOT_HOME so banner cache and config writes are sandboxed."""
    home = tmp_path / ".aot"
    home.mkdir()
    (home / "cache").mkdir()
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full aot_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "aot_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "aot_cli.config.save_config",
