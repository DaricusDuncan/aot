...
def container_env(tmp_path, monkeypatch):
    """Set up a fake AOT_HOME with .container-mode file."""
    aot_home = tmp_path / ".aot"
    aot_home.mkdir()
    monkeypatch.setenv("AOT_HOME", str(aot_home))
    monkeypatch.delenv("AOT_DEV", raising=False)

    container_mode = aot_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=aot-agent\n"
        "exec_user=aot\n"
        "aot_bin=/data/current-package/bin/aot\n"
    )
    return aot_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("aot_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "aot-agent"
    assert info["exec_user"] == "aot"
    assert info["aot_bin"] == "/data/current-package/bin/aot"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("aot_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_aot_dev(container_env, monkeypatch):
    """Returns None when AOT_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("AOT_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_aot_dev_zero(container_env, monkeypatch):
    """AOT_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("AOT_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        aot_home = Path(tmpdir) / ".aot"
        aot_home.mkdir()
        (aot_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("aot_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_aot_home": lambda: aot_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("AOT_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=aot-custom\n"
        "exec_user=myuser\n"
        "aot_bin=/opt/aot/bin/aot\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "aot-custom"
    assert info["exec_user"] == "myuser"
    assert info["aot_bin"] == "/opt/aot/bin/aot"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from aot_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "aot"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "aot-agent" in cmd
    assert "/data/current-package/bin/aot" in cmd
    assert "chat" in cmd
