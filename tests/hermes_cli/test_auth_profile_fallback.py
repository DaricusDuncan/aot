...
When ``AOT_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.aot/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.aot            (has its own auth.json fixture)
    * Profile     -> tmp_path/.aot/profiles/coder   (active, AOT_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (AOT_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.aot as the "real home" — which is NOT equal
    # to the AOT_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    aot_home = tmp_path / "classic"
    aot_home.mkdir()
    monkeypatch.setenv("AOT_HOME", str(aot_home))

    _write(aot_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from aot_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: AOT_HOME is set to a custom path that is NOT under
    # ~/.aot/profiles/ — get_default_aot_root() returns AOT_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from aot_cli.auth import read_credential_pool, write_credential_pool
