...
Covers:
 - Normal path (tokens already in Aot auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Aot
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `aot auth openai-codex`. The old
...
@pytest.fixture()
def aot_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Aot auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(aot_auth_only_env):
    """openai-codex appears when tokens are already in Aot auth store."""
...
def test_codex_picker_uses_live_codex_catalog(aot_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Aot
    auth store."""
    aot_home = tmp_path / ".aot"
    aot_home.mkdir()
