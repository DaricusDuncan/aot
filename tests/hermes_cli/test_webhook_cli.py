"""Tests for aot_cli/webhook.py — webhook subscription CLI."""
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "aot_cli.webhook._is_webhook_enabled", lambda: True
    )
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("aot_cli.webhook._is_webhook_enabled", lambda: False)
        webhook_command(_make_args(webhook_action="subscribe", name="blocked"))
        out = capsys.readouterr().out
        assert "not enabled" in out.lower()
        assert "aot gateway setup" in out
        assert _load_subscriptions() == {}
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "aot_cli.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "aot_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import aot_cli.webhook as wh_mod
