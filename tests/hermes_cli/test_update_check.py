"""Tests for the update check mechanism in aot_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("aot_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AOT_HOME", str(tmp_path))
    with patch("aot_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import aot_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "aot_cli" / "banner.py"
    ...
    monkeypatch.setenv("AOT_HOME", str(tmp_path))
    with patch("aot_cli.banner.subprocess.run") as mock_run:
        with patch("aot_cli.banner.check_via_pypi", return_value=0):
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when AOT_HOME has no git repo."""
    import aot_cli.banner as banner
    ...
    # Point AOT_HOME at a temp dir with no aot-agent/.git
    monkeypatch.setenv("AOT_HOME", str(tmp_path))
    with patch("aot_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from aot_cli.main import _invalidate_update_cache

    # Build a fake ~/.aot with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"AOT_HOME": str(default_home)}):
