"""Tests for the Kanban DB layer (aot_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated AOT_HOME with an empty kanban DB."""
    home = tmp_path / ".aot"
    home.mkdir()
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("AOT_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import aot_cli.kanban_db as _kb
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Aot profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="aot-task", assignee="daily")
...
def test_scratch_workspace_created_under_aot_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `aot -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's AOT_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's AOT_HOME."""

    def _set_home(self, monkeypatch, tmp_path, aot_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AOT_HOME", str(aot_home))
        monkeypatch.delenv("AOT_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_aot(
        self, tmp_path, monkeypatch
    ):
        # Standard install: AOT_HOME == ~/.aot, no profile active.
        default_home = tmp_path / ".aot"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.aot/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.aot/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.aot/kanban.db.
        default_home = tmp_path / ".aot"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # AOT_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `aot -p coder`).
        monkeypatch.setenv("AOT_HOME", str(profile_home))
    ...
    def test_docker_custom_aot_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: AOT_HOME points outside ~/.aot.
        # `get_default_aot_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".aot"`.
        custom_root = tmp_path / "opt" / "aot"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: AOT_HOME=/opt/aot/profiles/coder;
        # `get_default_aot_root()` walks up to /opt/aot because
    ...
    def test_explicit_override_via_aot_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: AOT_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".aot"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AOT_HOME", str(profile_home))
        monkeypatch.setenv("AOT_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AOT_HOME", str(default_home))
        monkeypatch.setenv("AOT_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # AOT_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile AOT_HOME and reads.
        monkeypatch.setenv("AOT_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_aot_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # AOT_KANBAN_DB pins the file path directly and beats both
        # AOT_KANBAN_HOME and the `get_default_aot_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AOT_HOME", str(default_home))
        monkeypatch.setenv("AOT_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("AOT_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows AOT_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_aot_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # AOT_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".aot"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("AOT_HOME", str(default_home))
        monkeypatch.setenv("AOT_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("AOT_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows AOT_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("AOT_HOME", str(default_home))
        monkeypatch.setenv("AOT_KANBAN_DB", "   ")
        monkeypatch.setenv("AOT_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject AOT_KANBAN_DB
        # and AOT_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites AOT_HOME.
        default_home = tmp_path / ".aot"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["AOT_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["AOT_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["AOT_KANBAN_TASK"] == "t_dispatch_env"
        assert env["AOT_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see aot_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("aot_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="aot_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'aot kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_aot_argv()
#
# Workers spawned by the dispatcher must use a `aot` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["aot", ...]`
...
def test_resolve_aot_argv_prefers_path_shim(monkeypatch):
    """When `aot` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import aot_cli.kanban_db as kb

    monkeypatch.delenv("AOT_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/aot")
    argv = kb._resolve_aot_argv()
    assert argv == ["/usr/local/bin/aot"]


def test_resolve_aot_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import aot_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("AOT_BIN", ".\\aot.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_aot_argv() == [os.path.abspath(".\\aot.exe")]


def test_resolve_aot_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "aot.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("AOT_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_aot_argv() == [sys.executable, "-m", "aot_cli.main"]


def test_resolve_aot_argv_honors_aot_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like AOT_BIN lets service managers pin the executable."""
    import shutil
    import aot_cli.kanban_db as kb

    shim = tmp_path / "bin" / "aot"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("AOT_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_aot_argv() == [str(shim)]


def test_resolve_aot_argv_aot_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare AOT_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import aot_cli.kanban_db as kb

    cwd_aot = tmp_path / "aot"
    cwd_aot.write_text("wrong\n", encoding="utf-8")
    cwd_aot.chmod(cwd_aot.stat().st_mode | stat.S_IXUSR)
    path_aot = tmp_path / "bin" / "aot"
    path_aot.parent.mkdir()
    path_aot.write_text("right\n", encoding="utf-8")
    path_aot.chmod(path_aot.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_aot.parent))
    monkeypatch.setenv("AOT_BIN", "aot")

    assert kb._resolve_aot_argv() == [str(path_aot)]


def test_resolve_aot_argv_aot_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare AOT_BIN does not accept current-directory shadow executables."""
    import sys
    import aot_cli.kanban_db as kb

    (tmp_path / "aot.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_aot_argv_aot_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved AOT_BIN batch shim is not used as worker argv[0]."""
    import sys
    import aot_cli.kanban_db as kb
...
def test_resolve_aot_argv_aot_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved AOT_BIN command names do not delegate cwd search to Popen."""
    import sys
    import aot_cli.kanban_db as kb
...
def test_resolve_aot_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m aot_cli.main`.

    Pins the correct module name (NOT `aot` — there is no top-level
    `aot` package). Regression for #23198: the original PR shipped
    `python -m aot` which fails with `No module named aot` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_aot_argv()
    assert argv == [sys.executable, "-m", "aot_cli.main"]


def test_resolve_aot_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `aot_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m aot_cli.main --version`
    ...
    assert "Aot Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
