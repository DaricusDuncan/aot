...
class TestApply:
    ...
    def test_enable_triggers_mcp_migration(self):
        """Enabling codex_app_server should auto-migrate Aot mcp_servers
        ...
        with patch.object(crs, "check_codex_binary_ok",
                          return_value=(True, "0.130.0")), \
             patch("aot_cli.codex_runtime_plugin_migration.migrate") as mig:
            mig.return_value.migrated = ["filesystem", "aot-tools"]
        ...
        # User MCP servers are reported (excluding internal aot-tools)
        ...
        # Aot tool callback announcement
    ...
    def test_disable_does_not_trigger_migration(self):
        ...
        with patch("aot_cli.codex_runtime_plugin_migration.migrate") as mig:
            r = crs.apply(cfg, "auto")
        assert r.success
