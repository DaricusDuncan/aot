...
Before the fix, ``aot whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``aot gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AOT_HOME", str(aot))
...
def _env_value(aot_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("aot_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``aot whatsapp`` with an existing paired session and
