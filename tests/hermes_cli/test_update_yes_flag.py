"""Tests for `aot update --yes / -y` — assume yes for interactive prompts.
...
class TestUpdateYesConfigMigration:
    ...
    @patch("aot_cli.config.migrate_config")
    @patch("aot_cli.config.check_config_version", return_value=(1, 2))
    @patch("aot_cli.config.get_missing_config_fields", return_value=[])
    @patch("aot_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``aot_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``aot_cli.main`` first could leave
