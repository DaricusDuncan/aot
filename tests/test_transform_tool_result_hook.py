...
def _run_handle_function_call(
    ...
    *,
    ...
    if invoke_hook is not _UNSET:
        ...
        monkeypatch.setattr("aot_cli.plugins.invoke_hook", invoke_hook)
...
def test_result_unchanged_when_no_hook_registered(monkeypatch):
    ...
    monkeypatch.setenv("AOT_HOME", "/tmp/aot_no_plugins")
...
def test_transform_tool_result_integration_with_real_plugin(monkeypatch, tmp_path):
    """End-to-end: load a real plugin from AOT_HOME and verify it rewrites results."""
    import yaml

    aot_home = Path(os.environ["AOT_HOME"])
