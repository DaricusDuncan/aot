...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("aot_cli.config.get_config_path", return_value=config_path), \
             patch("aot_cli.config.ensure_aot_home"):
    ...
    def test_ensure_aot_home_sets_0700(self):
        home = Path(self.tmpdir) / ".aot"
        with patch("aot_cli.config.get_aot_home", return_value=home):
