...
AOT_HOME scoping, and tick() serialization for profile jobs.
"""
...
def isolated_cron_profile_home(tmp_path, monkeypatch):
    """Create an isolated Aot root with a named profile and temp cron store."""
    root = tmp_path / "aot-root"
...
class TestCronjobToolProfile:
    ...
    def test_schema_advertises_profile(self):
        ...
        assert "aot profile" in desc_lower
        assert "context-local" in desc_lower
        assert "subprocess" in desc_lower
        assert "temporarily sets aot_home" not in desc_lower
...
class TestRunJobProfileContext:
    ...
    def _install_agent_stubs(monkeypatch, observed: dict):
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["env_home_during_init"] = os.environ.get("AOT_HOME")
                observed["profile_env_only_during_init"] = os.environ.get(
                    "AOT_PROFILE_TEST_ONLY"
                )
                observed["profile_env_shared_during_init"] = os.environ.get(
                    "AOT_PROFILE_TEST_SHARED"
                )
                observed["aot_home_during_init"] = str(get_aot_home())
                observed["scheduler_home_during_init"] = str(sched._get_aot_home())
                observed["skip_context_files"] = kwargs.get("skip_context_files")

            def run_conversation(self, *_a, **_kw):
                from aot_constants import get_aot_home

                observed["env_home_during_run"] = os.environ.get("AOT_HOME")
                ...
                observed["aot_home_during_run"] = str(get_aot_home())
                observed["scheduler_home_during_run"] = str(sched._get_aot_home())
        ...
        from aot_cli import runtime_provider as runtime_provider
        ...
        monkeypatch.setattr(sched, "_aot_home", None)
        monkeypatch.setenv("AOT_CRON_TIMEOUT", "0")
    ...
    def test_run_job_sets_and_restores_profile_home(
        ...
        assert observed["aot_home_during_init"] == str(profile_home.resolve())
        assert observed["aot_home_during_run"] == str(profile_home.resolve())
        ...
        assert os.environ["AOT_HOME"] == str(root)
        assert sched._get_aot_home() == root
    ...
    def test_profile_dotenv_environment_is_restored(
        ...
        observed: dict = {}
        self._install_agent_stubs(monkeypatch, observed)
        monkeypatch.setenv("AOT_PROFILE_TEST_SHARED", "outer")
        monkeypatch.delenv("AOT_PROFILE_TEST_ONLY", raising=False)

        def fake_load_dotenv(path, *_a, **_kw):
            observed.setdefault("dotenv_paths", []).append(str(path))
            os.environ["AOT_PROFILE_TEST_SHARED"] = "profile-value"
            os.environ["AOT_PROFILE_TEST_ONLY"] = "profile-only"
            os.environ["AOT_CRON_TIMEOUT"] = "123"
            return True
        ...
        assert os.environ["AOT_PROFILE_TEST_SHARED"] == "outer"
        assert "AOT_PROFILE_TEST_ONLY" not in os.environ
        assert os.environ["AOT_CRON_TIMEOUT"] == "0"
        assert os.environ["AOT_HOME"] == str(root)
        assert sched._get_aot_home() == root
    ...
    def test_run_job_without_profile_leaves_aot_home_untouched(
        self, isolated_cron_profile_home, monkeypatch
    ):
        import cron.scheduler as sched
        ...
        assert observed["aot_home_during_init"] == str(root)
        assert os.environ["AOT_HOME"] == str(root)
