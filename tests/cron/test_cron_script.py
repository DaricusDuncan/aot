...
- Path resolution (absolute, relative to AOT_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp AOT_HOME."""
    aot_home = tmp_path / ".aot"
    aot_home.mkdir()
    (aot_home / "cron").mkdir()
    (aot_home / "cron" / "output").mkdir()
    (aot_home / "scripts").mkdir()
    monkeypatch.setenv("AOT_HOME", str(aot_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "AOT_DIR", aot_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", aot_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", aot_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", aot_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.aot/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("AOT_SESSION_PLATFORM") is None
        assert os.environ.get("AOT_SESSION_CHAT_ID") is None
        assert os.environ.get("AOT_SESSION_CHAT_NAME") is None
