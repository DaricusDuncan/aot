...
Aot Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `aot plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/docs/developer-guide/context-engine-plugin).

## Dual Compression System

Aot has two separate compression layers that operate independently:
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Aot
