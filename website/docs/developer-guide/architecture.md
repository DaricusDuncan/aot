...
sidebar_position: 1
title: "Architecture"
description: "Aot Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Aot Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ aot_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # AotCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── aot_state.py           # SQLite session/state database with FTS5
├── aot_constants.py       # AOT_HOME, profile-aware paths
...
│
├── aot_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `aot` subcommands (large file)
...
│   ├── skills_config.py      # aot skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # aot tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # aot gateway start/stop
│
...
```text
User input → AotCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .aot.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.aot/plugins/` (user), `.aot/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `aot plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-aot-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Aot as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`aot -p <name>`) gets its own AOT_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
