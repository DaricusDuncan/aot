...
[PR #12550](https://github.com/NousResearch/aot-agent/pull/12550) proposed a
...
(`aot-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Aot `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `aot-acp`, `aot-api-server`, core toolsets (gated on CDP reachability)
...
- `aot_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
