...
The ACP adapter wraps Aot' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
aot acp / aot-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.aot/.env
  -> configure stderr logging
  -> construct AotACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'aot-agent[acp]==<version>' aot-acp`, pointed at the `aot-agent` PyPI release.
...
- `allow_once` -> Aot `once`
- `allow_always` -> Aot `always`
- reject options -> Aot `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Aot tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["aot-acp"])
...
Instead it reuses Aot' runtime resolver:

- `acp_adapter/auth.py`
- `aot_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Aot provider/credentials. It also always advertises a terminal setup auth method (`aot-setup`, args `--setup`) so first-run registry clients can open Aot' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.aot/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `aot-acp` toolset definition
- `aot_cli/main.py` — `aot acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `aot-acp` script
