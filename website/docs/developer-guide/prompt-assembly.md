...
description: "How Aot builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Aot deliberately separates:
...
1. agent identity — `SOUL.md` from `AOT_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.aot/SOUL.md)
You are Aot, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.aot/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_aot_md(cwd_path)       # 1. .aot.md / AOT.md (walks to git root)
    ...
    # SOUL.md from AOT_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.aot.md`, `AOT.md` | CWD up to git root | Aot-native project config |
...
- **YAML frontmatter stripped** — `.aot.md` frontmatter is removed (reserved for future config overrides)
...
1. `.aot.md` / `AOT.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Aot already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.aot/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.aot/MEMORY.md` and `~/.aot/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.aot.md`, `AOT.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Aot.
- Ephemeral overlays such as `AOT_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Aot assembles prompts for everyone, change Python and treat it as a code contribution
