...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Aot Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Aot tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Aot core, use the plugin route instead:

- [Plugins](/docs/user-guide/features/plugins)
- [Build a Aot Plugin](/docs/guides/build-a-aot-plugin)
...
2. **`toolsets.py`** — add tool name to `_AOT_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `aot_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `aot_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `aot chat -q "Use the weather tool for London"`
