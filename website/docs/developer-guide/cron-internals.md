...
description: "How Aot stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `aot_cli/cron.py` | CLI `aot cron` subcommands |
...
Jobs are stored in `~/.aot/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `aot cron` commands are run or during active CLI sessions.
...
```python
# ~/.aot/scripts/check_competitors.py
...
2. **Environment variable** — `AOT_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.aot/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `aot cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `aot cron` CLI provides direct job management:

```bash
aot cron list                    # Show all jobs
aot cron create                  # Interactive job creation (alias: add)
aot cron edit <job_id>           # Edit job configuration
aot cron pause <job_id>          # Pause a running job
aot cron resume <job_id>         # Resume a paused job
aot cron run <job_id>            # Trigger immediate execution
aot cron remove <job_id>         # Delete a job
```
