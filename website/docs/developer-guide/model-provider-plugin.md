...
description: "How to build a model provider (inference backend) plugin for Aot Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that Aot can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$AOT_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with Aot
2. **User plugins** — `$AOT_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** — `<repo>/providers/<name>.py` — back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$AOT_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `aot plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `aot_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `aot_cli/main.py` | Accepts `acme-inference` |
| `aot model` picker | `aot_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `aot doctor` | `aot_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `aot setup` | `aot_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `aot_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| Field | Type | Purpose |
|---|---|---|
| `name` | str | Canonical id — matches `--provider` choices and `AOT_INFERENCE_PROVIDER` |
| `aliases` | `tuple[str, ...]` | Alternative names resolved by `get_provider_profile()` (e.g. `grok` → `xai`) |
| `api_mode` | str | `chat_completions` \| `codex_responses` \| `anthropic_messages` \| `bedrock_converse` |
| `display_name` | str | Human label shown in `aot model` picker |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.aot/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. Aot picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but Aot' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `AOT_HOME` at a temp directory so you don't pollute your real config:

```bash
export AOT_HOME=/tmp/aot-plugin-test
mkdir -p $AOT_HOME/plugins/model-providers/my-provider
cat > $AOT_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
aot -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `aot plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$AOT_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Like any Aot plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:

```toml
[project.entry-points."aot.plugins"]
acme-inference = "acme_aot_plugin:register"
```

…where `acme_aot_plugin:register` is a function that calls `register_provider(profile)`. The general PluginManager picks up entry-point plugins during `discover_and_load()`. For `kind: model-provider` pip plugins, you still need to declare the kind in your manifest (or rely on the source-text heuristic).

See [Building a Aot Plugin](/docs/guides/build-a-aot-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a Aot Plugin](/docs/guides/build-a-aot-plugin) — general plugin authoring
