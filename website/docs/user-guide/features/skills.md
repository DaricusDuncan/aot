...
All skills live in **`~/.aot/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Aot at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Aot to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.aot/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
aot chat --toolsets skills -q "What skills do you have?"
aot chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.aot/cache/chart-q4-2025.png
...
When a missing value is encountered, Aot asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `aot setup` or `~/.aot/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `aot config migrate` prompts for unconfigured settings, and `aot config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.aot/skills/                  # Single source of truth
...
If you maintain skills outside of Aot — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Aot to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.aot/config.yaml`:
...
- **Read-only**: External dirs are only scanned for skill discovery. When the agent creates or edits a skill, it always writes to `~/.aot/skills/`.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Aot ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.aot/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.aot/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `aot bundles list`.
...
aot bundles create research

# Overwrite an existing bundle
aot bundles create backend-dev --skill ... --force

# Delete a bundle
aot bundles delete backend-dev

# Re-scan ~/.aot/skill-bundles/ and report changes
aot bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.aot/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.aot/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
aot skills browse                              # Browse all hub skills (official first)
aot skills browse --source official            # Browse only official optional skills
aot skills search kubernetes                   # Search all sources
aot skills search react --source skills-sh     # Search the skills.sh directory
aot skills search https://mintlify.com/docs --source well-known
aot skills inspect openai/skills/k8s           # Preview before installing
aot skills install openai/skills/k8s           # Install with security scan
aot skills install official/security/1password
aot skills install skills-sh/vercel-labs/json-render/json-render-react --force
aot skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
aot skills list --source hub                   # List hub-installed skills
aot skills check                               # Check installed hub skills for upstream updates
aot skills update                              # Reinstall hub skills with upstream changes when needed
aot skills audit                               # Re-scan all hub skills for security
aot skills uninstall k8s                       # Remove a hub skill
aot skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
aot skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
aot skills publish skills/my-skill --to github --repo owner/repo
aot skills snapshot export setup.json          # Export skill config
aot skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Aot. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `aot skills search <query> --source skills-sh`. Aot resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Aot currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Aot repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Aot can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
aot skills search https://mintlify.com/docs --source well-known
aot skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
aot skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Aot can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
aot skills install openai/skills/k8s
aot skills tap add myorg/skills-repo
```
...
- Aot source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Aot supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Aot source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Aot can search and convert agent entries from LobeHub's public catalog into installable Aot skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Aot source id: `lobehub`

#### 8. Direct URL (`url`)

Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Aot fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
aot skills install https://sharethis.chat/SKILL.md
aot skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
aot skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `aot skills update` re-fetches from the same URL automatically when you want to refresh.
...
`aot skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Aot | Always trusted |
...
```bash
aot skills check          # Report which installed hub skills changed upstream
aot skills update         # Reinstall only the skills with updates available
aot skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Aot users add with `aot skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.aot.tags`, `version`, `author`, `platforms`, `metadata.aot.config`).
...
Aot discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Aot user can subscribe and install:

```bash
aot skills tap add my-org/aot-skills
aot skills search deploy
aot skills install my-org/aot-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.aot/.hub/taps.json`:
...
The `aot skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `aot skills tap list` shows the effective path per tap.
...
```bash
aot skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.aot/.hub/taps.json` (created on demand).

## Bundled skill updates (`aot skills reset`)

Aot ships with a set of bundled skills in `skills/` inside the repo. On install and on every `aot update`, a sync pass copies those into `~/.aot/skills/` and records a manifest at `~/.aot/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Aot recomputes the hash of your local copy and compares it to the origin hash:
...
aot skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `AOT_HOME`, so `aot -p coder skills reset <name>` only affects that profile.
:::
