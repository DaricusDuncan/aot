...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Aot CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Aot CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.aot/skins/mytheme.yaml
```

Or set the default skin in `~/.aot/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Aot — gold and kawaii | `Aot Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Aot Agent` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Aot Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Aot Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Aot Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Aot Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Aot Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Aot ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default AOT_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.aot/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.aot/skins/mytheme.yaml
...
## Aot Mod — Visual Skin Editor

[Aot Mod](https://github.com/cocktailpeanut/aot-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Aot Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/aot-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Aot skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.aot/skins/`
- Activates a skin by updating `~/.aot/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/aot-mod.git
cd aot-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.aot/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Aot Mod respects the `AOT_HOME` environment variable, so it works with [profiles](/docs/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `aot_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.aot/skins/` take precedence over built-in skins with the same name.
