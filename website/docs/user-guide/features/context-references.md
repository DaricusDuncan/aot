...
Type `@` followed by a reference to inject content directly into your message. Aot expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Aot env: `$AOT_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$AOT_HOME/skills/.hub/`
