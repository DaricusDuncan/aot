...
Aot Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Aot can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`aot fallback` reuses the provider picker from `aot model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.

If you'd rather edit the YAML directly, add a `fallback_model` section to `~/.aot/config.yaml`:
...
`fallback_model` (singular) is the legacy single-fallback key — Aot still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `aot fallback` writes to this key. When both are set, Aot merges them with `fallback_providers` taking priority.
:::
...
| Nous Portal | `nous` | `aot auth` (OAuth) |
| OpenAI Codex | `openai-codex` | `aot model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `aot model` (Google OAuth; optional: `AOT_GEMINI_PROJECT_ID`) |
| Google AI Studio | `gemini` | `GOOGLE_API_KEY` (alias: `GEMINI_API_KEY`) |
| xAI (Grok) | `xai` (alias `grok`) | `XAI_API_KEY` (optional: `XAI_BASE_URL`) |
| xAI Grok OAuth (SuperGrok) | `xai-oauth` (alias `grok-oauth`) | `aot model` → xAI Grok OAuth (browser login; SuperGrok subscription) |
| AWS Bedrock | `bedrock` | Standard boto3 auth (`AWS_REGION` + `AWS_PROFILE` or `AWS_ACCESS_KEY_ID`) |
| Qwen Portal (OAuth) | `qwen-oauth` | `aot model` (Qwen Portal OAuth; optional: `AOT_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `aot model` (MiniMax portal OAuth) |
...
Aot uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `aot kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), Aot tries providers in order until one works:
...
If the resolved provider fails at call time, Aot also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
| `"codex"` | Force Codex OAuth | `aot model` → Codex |
...
`base_url` takes precedence over `provider`. Aot uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.

---

## Auxiliary Capacity-Error Fallback

When you set an explicit auxiliary provider (e.g. `auxiliary.vision.provider: glm`), Aot treats that as your preferred choice — but if the provider literally cannot serve the request because of a **capacity error** (HTTP 402 payment required, HTTP 429 daily-quota exhaustion, connection failure), Aot falls back through a layered chain instead of failing silently:
...
4. **Warn + re-raise** — if every layer fails, Aot logs `Auxiliary <task>: ... all fallbacks exhausted` at WARNING level and re-raises the original error
...
Aot recognizes these as capacity-equivalent to 402 credit exhaustion (not transient rate limits):
...
If your provider returns a different phrase for daily-quota exhaustion and Aot doesn't trigger fallback, that's a bug — open an issue with the exact error string.
...
If no provider is available for compression, Aot drops middle conversation turns without generating a summary rather than failing the session.
