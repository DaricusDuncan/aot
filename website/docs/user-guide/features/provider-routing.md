...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Aot Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.aot/config.yaml`:
...
- **CLI mode** — configured in `~/.aot/config.yaml`, loaded at startup
