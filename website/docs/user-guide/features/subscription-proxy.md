...
chat completions — use your Aot-managed provider subscription as their
...
| Use case | "Use Aot as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Aot stores
the refresh token in `~/.aot/auth.json` — the same place all Aot
...
```
Starting Aot proxy for Nous Portal
...
Model:      Aot-4-70B    # or Aot-4.3-36B, Aot-4-405B
```
...
```
Aot proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `aot login nous`. If you see
...
INFERENCE_TEXT_MODEL=Aot-4-70B
```
...
```bash
aot proxy start --host 0.0.0.0 --port 8645
```
...
`aot_cli/proxy/adapters/<provider>.py` and registering it in
