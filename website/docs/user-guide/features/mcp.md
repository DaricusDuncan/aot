...
description: "Connect Aot Agent to external tool servers via MCP — and control exactly which MCP tools Aot loads"
---

# MCP (Model Context Protocol)

MCP lets Aot Agent connect to external tool servers so the agent can use tools that live outside Aot itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Aot to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Aot tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Aot to see
...
2. Add an MCP server to `~/.aot/config.yaml`:
...
4. Ask Aot to use the MCP-backed capability.
...
Aot will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Aot connects to directly.
...
- you do not want Aot spawning a local subprocess for that integration

## Basic configuration reference

Aot reads MCP config from `~/.aot/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Aot skips the server entirely |
...
For well-known MCP servers, `aot mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
aot mcp add codex --preset codex
```
...
You can pick any local name (`aot mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Aot registers MCP tools

Aot prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Aot sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Aot also registers utility tools around MCP resources and prompts:
...
- Aot only registers resource utilities if the MCP session actually supports resource operations
- Aot only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Aot, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Aot skips the server completely and does not even attempt a connection.
...
You can also separately disable Aot-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Aot does not create an empty runtime MCP toolset for that server.
...
Aot discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Aot when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Aot receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Aot does not blindly pass your full shell environment.
...
Then verify your config and restart Aot.
...
Because Aot now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Aot may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Aot via the `sampling/createMessage` protocol. This allows an MCP server to ask Aot to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Aot as an MCP server

In addition to connecting **to** MCP servers, Aot can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Aot's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Aot
- You want a single MCP server that bridges to all of Aot's connected messaging platforms at once
- You already have a running Aot gateway with connected platforms
...
Add Aot to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Aot in a specific location:

```json
{
  "mcpServers": {
    "aot": {
      "command": "/home/user/.aot/aot-agent/venv/bin/aot",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Aot-specific channel browser:
...
The MCP server includes a live event bridge that polls Aot's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
aot mcp serve              # Normal mode
aot mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Aot's session store (`~/.aot/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Aot agent itself.
...
- The embedded `aot mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Aot, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
