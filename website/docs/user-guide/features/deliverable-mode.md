...
When Aot Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.aot/config.yaml` under `agent.custom_instructions`.
...
If you use Aot' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.aot/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Aot Agent's deliverable
