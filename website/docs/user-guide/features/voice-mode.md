...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Aot Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Aot Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Aot](/docs/guides/use-voice-mode-with-aot).

## Prerequisites

Before using voice features, make sure you have:

1. **Aot Agent installed** — `pip install aot-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `aot model` or set your preferred provider credentials in `~/.aot/.env`
3. **A working base setup** — run `aot` to verify the agent responds to text before enabling voice

:::tip
The `~/.aot/` directory and default `config.yaml` are created automatically the first time you run `aot`. You only need to create `~/.aot/.env` manually for API keys.
:::
...
pip install "aot-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "aot-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "aot-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "aot-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`aot chat`) and the **TUI** (`aot --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.aot/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
1. Start the CLI with `aot` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.aot/config.yaml` (default: `ctrl+b`).
:::
...
```bash
aot gateway        # Start the gateway (connects to configured platforms)
aot gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@aotbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.aot/.env`:
```bash
...
```bash
aot gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.aot/.env`
...
- Review gateway logs: `tail -f ~/.aot/logs/gateway.log`
