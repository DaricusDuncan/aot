...
Aot Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
```
aot computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `aot computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `aot tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ```
   aot -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.aot/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Aot refreshes the binary in two
places so you don't get stuck on a stale release:

- **`aot update`** — when you update Aot itself, if `cua-driver` is
...
- **`aot computer-use install --upgrade`** — manual force-refresh.
...
`aot computer-use status` shows the installed version next to the
binary path.
...
Aot applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.aot/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Aot applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `AOT_CUA_DRIVER_VERSION`
...
```
AOT_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
AOT_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
AOT_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`aot computer-use install` to fetch the cua-driver binary, or run
`aot tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/aot-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
