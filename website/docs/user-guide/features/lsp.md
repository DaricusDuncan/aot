...
Aot runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Aot
...
1. Aot captures a baseline of current diagnostics for the file.
...
…). Aot auto-detects the binary on PATH or in
`<AOT_HOME>/lsp/bin/`.
...
`node_modules` tree — Aot installs both packages together when you
run `aot lsp install typescript` or auto-install fires on first
use.

## CLI

```
aot lsp status          # service state + per-server install status
aot lsp list            # registry, optionally --installed-only
aot lsp install <id>    # eagerly install one server
aot lsp install-all     # try every server with a known recipe
aot lsp restart         # tear down running clients
aot lsp which <id>      # print resolved binary path
```

`aot lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <AOT_HOME>/lsp/bin
...
When `install_strategy: auto`, Aot installs binaries into
`<AOT_HOME>/lsp/bin/`. NPM packages land in
`<AOT_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Aot-owned and is
...
Servers are kept alive for the life of the Aot process. There's
...
**`aot lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<AOT_HOME>/lsp/bin/`. Run
`aot lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `aot lsp status`**
...
When `aot lsp status` shows a `Backend warnings` section, install
...
Check `~/.aot/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `aot lsp restart` to clear the set;
