...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Aot's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Aot instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
aot memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$AOT_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `aot` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Aot syncs them back at session init.
...
The `aot honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `aot memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
aot honcho status          # Connection status, config, and key settings
aot honcho setup           # Redirects to `aot memory setup`
aot honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
aot honcho peer            # Show or update peer names + dialectic reasoning level
aot honcho mode            # Show or set recall mode (hybrid/context/tools)
aot honcho tokens          # Show or set token budget for context and dialectic
aot honcho identity        # Seed or show the AI peer's Honcho identity
aot honcho sync            # Sync Honcho config to all existing profiles
aot honcho peers           # Show peer identities across all profiles
aot honcho sessions        # List known Honcho session mappings
aot honcho map             # Map current directory to a Honcho session name
aot honcho enable          # Enable Honcho for the active profile
aot honcho disable         # Disable Honcho for the active profile
aot honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `aot honcho`

If you previously used the standalone `aot honcho setup`:
...
No re-login or re-setup needed. Run `aot memory setup` and select "honcho" — the wizard detects your existing config.
