...
Aot Agent includes a full browser automation toolset with multiple backend options:
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. Run `aot model` or `aot tools` to enable it.
:::
...
When a cloud provider is configured, Aot auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`aot setup tools → Browser Automation`
...
Or configure via `aot tools` → Browser Automation → Camofox.
...
By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.aot/config.yaml`:
...
Then fully restart Aot so the new config is picked up.

:::warning Nested path matters
Aot reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Aot ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Aot silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Aot profile, so different Aot profiles get different browser profiles (profile isolation).

##### What Aot does not do
- It does not force persistence on the Camofox server. Aot only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Aot cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Aot and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Aot after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Aot derives the stable `userId` from the profile-scoped directory `~/.aot/browser_auth/camofox/` (or the equivalent under `$AOT_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Aot profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Aot to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Aot uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Aot calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Aot skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Aot does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Aot issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Aot adopts the most recently created one in that group.
3. Otherwise, Aot adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Aot falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — Aot does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Aot to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Aot generates a per-task `session_key` (`task_<id>`) — Aot will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Aot can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Aot runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Aot automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.

### Local Chrome via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Aot browser tools to your own running Chrome instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Aot from the terminal (`aot` or `aot chat`) and issue `/browser connect` there.
:::
...
If Chrome isn't already running with remote debugging, Aot will attempt to auto-launch it with `--remote-debugging-port=9222`.
...
google-chrome \
  ...
  --user-data-dir=$HOME/.aot/chrome-debug \
  --no-first-run \
...
Then launch the Aot CLI and run `/browser connect`.
...
If Aot runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Aot itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Aot talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Aot MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Aot can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Extra Chromium launch flags (comma- or newline-separated). Aot auto-injects
...
# Aot doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `aot config set toolsets '["aot-cli", "browser"]'`.
:::
...
```
Type "aot agent" into the search field @e3
```
...
Screenshots are stored in `~/.aot/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.aot/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Aot automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
