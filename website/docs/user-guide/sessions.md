...
Aot Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.aot/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.aot/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
Aot stores session history so it can resume conversations, but it does not
...
Aot explicitly injects for that turn.
...
For example, if a user sends an image and asks Aot to make a meme from it,
Aot may inspect that image once with vision and run an image-processing
...
`aot sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`aot` or `aot chat`) |
...
aot -c "my project"   # → resumes "my project #3"
```
...
aot --resume 20250305_091523_a1b2c3d4
aot -r 20250305_091523_a1b2c3d4

# Resume by title
aot --resume "refactoring auth"

# Or with the chat subcommand
aot chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `aot sessions list`.

### Conversation Recap on Resume

When you resume a session, Aot displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Aot session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.aot/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `aot -r "<title>"` from the shell) and pick up where the platform left off.
...
Aot automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `aot sessions list` or `aot sessions browse`.
...
```bash
aot sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Aot creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`aot -c "my project"`), it automatically picks the most recent session in the lineage.
...
Aot provides a full set of session management commands via `aot sessions`:
...
# Filter by platform
aot sessions list --source telegram

# Show more sessions
aot sessions list --limit 50
```
...
aot sessions export backup.jsonl

# Export sessions from a specific platform
aot sessions export telegram-history.jsonl --source telegram

# Export a single session
aot sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
aot sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
aot sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
aot sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
aot sessions prune --older-than 30

# Only prune sessions from a specific platform
aot sessions prune --source telegram --older-than 60

# Skip confirmation
aot sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`aot insights`](/docs/reference/cli-commands#aot-insights).
...
When Aot cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Aot uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Aot in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.aot/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.aot/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.aot/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Aot process in the same `AOT_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.aot/config.yaml`:
...
aot sessions delete <session_id>
