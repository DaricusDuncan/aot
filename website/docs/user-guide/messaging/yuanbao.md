...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Aot Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Aot to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.aot/.env`:
...
All standard Aot commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.aot/logs/gateway.log`
...
4. Consider enabling verbose logging: `AOT_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Aot automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
AOT_LOG_LEVEL=debug aot gateway
```
...
```bash
aot chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
