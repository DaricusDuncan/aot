...
description: "Chat with Aot from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Aot from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Aot](/docs/guides/use-voice-mode-with-aot).
...
```bash
aot gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
aot gateway              # Run in foreground
aot gateway setup        # Configure messaging platforms interactively
aot gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo aot gateway install --system   # Linux only: install a boot-time system service
aot gateway start        # Start the default service
aot gateway stop         # Stop the default service
aot gateway status       # Check default service status
aot gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Aot Agent to the latest version |
...
Configure per-platform overrides in `~/.aot/gateway.json`:
...
aot pairing approve telegram XKGH5N7P

# Other pairing commands:
aot pairing list          # View pending + approved users
aot pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Aot appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.aot/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.aot/config.yaml`:
...
```bash
AOT_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
aot gateway install               # Install as user service
aot gateway start                 # Start the service
aot gateway stop                  # Stop the service
aot gateway status                # Check status
journalctl --user -u aot-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo aot gateway install --system
sudo aot gateway start --system
sudo aot gateway status --system
journalctl -u aot-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Aot will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Aot installations on the same machine (with different `AOT_HOME` directories), each gets its own systemd service name. The default `~/.aot` uses `aot-gateway`; other installations use `aot-gateway-<hash>`. The `aot gateway` commands automatically target the correct service for your current `AOT_HOME`.
:::

### macOS (launchd)

```bash
aot gateway install               # Install as launchd agent
aot gateway start                 # Start the service
aot gateway stop                  # Stop the service
aot gateway status                # Check status
tail -f ~/.aot/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.aot.gateway.plist`. It includes three environment variables:
...
- **AOT_HOME** — scopes the gateway to your Aot installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `aot gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `AOT_HOME` directory gets its own launchd label. The default `~/.aot` uses `ai.aot.gateway`; other installations use `ai.aot.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `aot-cli` | Full access |
| Telegram | `aot-telegram` | Full tools including terminal |
| Discord | `aot-discord` | Full tools including terminal |
| WhatsApp | `aot-whatsapp` | Full tools including terminal |
| Slack | `aot-slack` | Full tools including terminal |
| Google Chat | `aot-google_chat` | Full tools including terminal |
| Signal | `aot-signal` | Full tools including terminal |
| SMS | `aot-sms` | Full tools including terminal |
| Email | `aot-email` | Full tools including terminal |
| Home Assistant | `aot-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `aot-mattermost` | Full tools including terminal |
| Matrix | `aot-matrix` | Full tools including terminal |
| DingTalk | `aot-dingtalk` | Full tools including terminal |
| Feishu/Lark | `aot-feishu` | Full tools including terminal |
| WeCom | `aot-wecom` | Full tools including terminal |
| WeCom Callback | `aot-wecom-callback` | Full tools including terminal |
| Weixin | `aot-weixin` | Full tools including terminal |
| BlueBubbles | `aot-bluebubbles` | Full tools including terminal |
| QQBot | `aot-qqbot` | Full tools including terminal |
| Yuanbao | `aot-yuanbao` | Full tools including terminal |
| Microsoft Teams | `aot-teams` | Full tools including terminal |
| API Server | `aot-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `aot-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.aot/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
