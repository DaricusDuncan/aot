...
sidebar_position: 10
title: "DingTalk"
description: "Set up Aot Agent as a DingTalk chatbot"
---

# DingTalk Setup

Aot Agent integrates with DingTalk (钉钉) as a chatbot, letting you chat with your AI assistant through direct messages or group chats. The bot connects via DingTalk's Stream Mode — a long-lived WebSocket connection that requires no public URL or webhook server — and replies using markdown-formatted messages through DingTalk's session webhook API.

Before setup, here's the part most people want to know: how Aot behaves once it's in your DingTalk workspace.

## How Aot Behaves

| Context | Behavior |
|---------|----------|
| **DMs (1:1 chat)** | Aot responds to every message. No `@mention` needed. Each DM has its own session. |
| **Group chats** | Aot responds when you `@mention` it. Without a mention, Aot ignores the message. |
| **Shared groups with multiple users** | By default, Aot isolates session history per user inside the group. Two people talking in the same group do not share one transcript unless you explicitly disable that. |
...
```bash
pip install "aot-agent[dingtalk]"
```
...
Aot Agent uses your DingTalk User ID to control who can interact with the bot. DingTalk User IDs are alphanumeric strings set by your organization's admin.
...
- **QR-code device flow (recommended).** Scan the QR that prints in your terminal with the DingTalk mobile app — your Client ID and Client Secret are returned automatically and written to `~/.aot/.env`. No developer-console trip needed.
...
Aot can reply using DingTalk AI Cards instead of plain markdown messages. Cards provide a richer, more structured display and support streaming updates as the agent generates its response.
...
Aot automatically adds emoji reactions to your messages to show processing status:
...
**Fix**: Verify `DINGTALK_CLIENT_ID` and `DINGTALK_CLIENT_SECRET` are set correctly in `~/.aot/.env`. The Client ID is your AppKey, and the Client Secret is your AppSecret from the DingTalk Developer Console.
...
**Fix**: Check that `aot gateway` is running. Look at the terminal output for error messages. Common issues: wrong credentials, app deactivated, `dingtalk-stream` or `httpx` not installed.
