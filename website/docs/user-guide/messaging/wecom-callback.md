...
Connect Aot to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Aot supports two WeCom integration modes:
...
3. Aot decrypts the message, queues it for the agent
...
(Use `aot gateway start` only after `aot gateway install` has registered the systemd/launchd service.)
