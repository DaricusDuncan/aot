...
sidebar_position: 12
title: "Google Chat"
description: "Set up Aot Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Aot Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Aot
...
download the file. Save it somewhere only Aot can read (e.g.,
`~/.aot/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `aot-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `aot-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a aot restart)
...
- Principal: `aot-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Aot calls
...
- **App name**: whatever you want users to see ("Aot" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/aot-chat-events`.
...
`ADDED_TO_SPACE` event that Aot uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Aot

Add the Google Chat section to `~/.aot/.env`:

```bash
# Required
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/aot-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.aot/google-chat-sa.json
...
Install the dependencies the Google Chat adapter needs (no Aot extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Aot is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Aot detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Aot session.
...
3. Download the JSON. Move it onto the host that runs Aot.
4. On the host, register the client with Aot:
...
That writes `~/.aot/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.aot/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.aot/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Aot isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `aot gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Aot will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.aot/google_chat_user_tokens/<sanitized_email>.json` (filesystem
