...
- Give it a title like "aot-agent-&lt;machine-name>"
...
elif [ -f ~/.aot/.env ] && grep -q "^GITHUB_TOKEN=" ~/.aot/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.aot/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
