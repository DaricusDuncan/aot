...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.aot/.env ] && grep -q "^GITHUB_TOKEN=" ~/.aot/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.aot/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Aot Agent\",
    \"comments\": [
...
```bash
source "${AOT_HOME:-$HOME/.aot}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Aot Agent. Code looks clean — good test coverage, no security concerns."
