...
description: "Operate the Teams meeting summary pipeline via Aot CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions"
---
...
Operate the Teams meeting summary pipeline via Aot CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions.
...
Everything operator-facing is a `aot teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `~/.aot/.env`:
...
```bash
aot teams-pipeline validate              # config snapshot — run first after any change
aot teams-pipeline token-health          # Graph token status
aot teams-pipeline token-health --force-refresh   # force a fresh token acquisition
aot teams-pipeline list                  # recent meeting jobs
aot teams-pipeline list --status failed  # only failed jobs
aot teams-pipeline show <job-id>         # full detail of one job
aot teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
aot teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
aot teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
aot teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
aot teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

aot teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
aot teams-pipeline delete-subscription <sub-id>
aot teams-pipeline maintain-subscriptions            # renew near-expiry ones
aot teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `aot teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `aot cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
