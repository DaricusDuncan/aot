---
title: "Debugging Aot Tui Commands — Debug Aot TUI slash commands: Python, gateway, Ink UI"
sidebar_label: "Debugging Aot Tui Commands"
description: "Debug Aot TUI slash commands: Python, gateway, Ink UI"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Debugging Aot Tui Commands

Debug Aot TUI slash commands: Python, gateway, Ink UI.
...
| Path | `skills/software-development/debugging-aot-tui-commands` |
| Version | `1.0.0` |
| Author | Aot Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `debugging`, `aot-agent`, `tui`, `slash-commands`, `typescript`, `python` |
...
# Debugging Aot TUI Slash Commands

## Overview

Aot slash commands span three layers — Python command registry, tui_gateway JSON-RPC bridge, and the Ink/TypeScript frontend. When a command misbehaves (missing from autocomplete, works in CLI but not TUI, config persists but UI doesn't update), the bug is almost always one layer being out of sync with another.

Use this skill when you encounter issues with slash commands in the Aot TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
<!-- ascii-guard-ignore -->
```
Python backend (aot_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path aot_cli/
   search_files --pattern "commandname" --path aot_cli/commands.py --context 3
   ```
...
4. If the command runs server-side, add a handler in `AotCLI.process_command()` in `cli.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `aot_cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/aot-agent && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.aot/config.yaml`)
