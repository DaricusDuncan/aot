...
| | |
|---|---|
| Source | Optional — install with `aot skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Aot Agent |
| License | MIT |
...
import upstream classes into Aot itself.
...
```bash
mkdir -p ~/.aot/cache/darwinian-evolver && cd ~/.aot/cache/darwinian-evolver
...
```bash
cd ~/.aot/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.aot/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.aot/skills/research/darwinian-evolver
DE_DIR=~/.aot/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Aot core.
   Custom driver scripts under `~/.aot/skills/...` are user-side and fine.
