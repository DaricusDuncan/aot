...
| | |
|---|---|
| Source | Optional — install with `aot skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Aot core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Aot native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Aot native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Aot native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Aot usage patterns
...
- Do not silently choose Parallel when Aot native tools are already sufficient.
