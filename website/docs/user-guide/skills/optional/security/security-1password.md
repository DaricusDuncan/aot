...
| | |
|---|---|
| Source | Optional — install with `aot skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Aot Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Aot terminal calls (desktop app flow only)
...
### Service Account (recommended for Aot)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.aot/.env` (the skill will prompt for this on first load).
...
## Aot Execution Pattern (desktop app flow)

Aot terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/aot-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/aot-op.sock"
