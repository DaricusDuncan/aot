...
| | |
|---|---|
| Source | Optional — install with `aot skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Aot or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Aot. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.aot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.aot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Aot integration, either:

- configure the server in `~/.aot/config.yaml` using the `native-mcp` skill, or
...
### Aot cannot see the deployed server

The server-building part may be correct while the Aot config is not. Load the `native-mcp` skill and configure the server in `~/.aot/config.yaml`, then restart Aot.
