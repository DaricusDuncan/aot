---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Aot Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Aot Agent"
---
...
Migrate a user's OpenClaw customization footprint into Aot Agent. Imports Aot-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `aot skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Aot Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Aot`, `Memory`, `Persona`, `Import` |
| Related skills | [`aot-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-aot-agent) |
...
# OpenClaw -> Aot Migration

Use this skill when a user wants to move their OpenClaw setup into Aot Agent with minimal manual cleanup.
...
```bash
aot claw migrate              # Full interactive migration
...
aot claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `aot setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_aot.py` to:

- import `SOUL.md` into the Aot home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Aot memory entries
- merge OpenClaw command approval patterns into Aot `command_allowlist`
- migrate Aot-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.aot/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Aot workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.aot/tts/`
- archive non-secret docs that do not have a direct Aot destination
...
- `scripts/openclaw_to_aot.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.aot/skills/migration/openclaw-migration/scripts/openclaw_to_aot.py`

Do not guess a shorter path like `~/.aot/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.aot/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Aot-compatible secrets, currently:
...
Aot CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Aot. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Aot workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Aot skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.aot/skills/migration/openclaw-migration/scripts/openclaw_to_aot.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.aot/skills/migration/openclaw-migration/scripts/openclaw_to_aot.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.aot/skills/migration/openclaw-migration/scripts/openclaw_to_aot.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.aot/skills/migration/openclaw-migration/scripts/openclaw_to_aot.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.aot/skills/migration/openclaw-migration/scripts/openclaw_to_aot.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.aot/skills/migration/openclaw-migration/scripts/openclaw_to_aot.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Aot unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Aot targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Aot destination. Unsupported auth blobs must still be reported as skipped.
...
- Aot persona state imported
- Aot memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.aot/skills/openclaw-imports/`
