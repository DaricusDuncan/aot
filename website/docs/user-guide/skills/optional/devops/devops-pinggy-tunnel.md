...
| | |
|---|---|
| Source | Optional — install with `aot skills install official/devops/pinggy-tunnel` |
| Path | `optional-skills/devops/pinggy-tunnel` |
| Version | `0.1.0` |
| Author | Teknium (teknium1), Aot Agent |
| License | MIT |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Aot' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
