...
| | |
|---|---|
| Source | Optional — install with `aot skills install official/devops/watchers` |
...
All three live in `$AOT_HOME/skills/devops/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `$AOT_HOME/watcher-state/`) for its state file, keyed by the `--name` argument.
...
```bash
python $AOT_HOME/skills/devops/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.aot/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $AOT_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name aot-issues --repo NousResearch/aot-agent --scope issues
```

Poll an arbitrary JSON API:

```bash
python $AOT_HOME/skills/devops/watchers/scripts/watch_http_json.py \
...
Every watcher writes `$AOT_HOME/watcher-state/<name>.json`. Inspect:

```bash
cat $AOT_HOME/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm $AOT_HOME/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `$AOT_HOME/watcher-state/` is always writable. Docker/Modal backends may not see arbitrary host paths.
