...
| | |
|---|---|
| Source | Optional — install with `aot skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Aot Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.aot/.env`:
...
```bash
CANVAS="python $AOT_HOME/skills/productivity/canvas/scripts/canvas_api.py"
