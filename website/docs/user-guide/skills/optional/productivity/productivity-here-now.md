...
| | |
|---|---|
| Source | Optional — install with `aot skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${AOT_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${AOT_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${AOT_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client aot
```
...
```bash
PUBLISH="${AOT_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client aot
```
...
```bash
DRIVE="${AOT_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `aot`)    |
