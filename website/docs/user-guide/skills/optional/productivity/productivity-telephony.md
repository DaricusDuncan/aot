---
title: "Telephony — Give Aot phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Aot phone capabilities without core tool changes"
---
...
Give Aot phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `aot skills install official/productivity/telephony` |
...
This optional skill gives Aot practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.aot/.env`
...
It does **not** turn Aot into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Aot memory
...
### 1) "I want Aot to own a real phone number"
Use **Twilio**.
...
- pairs well with Aot `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.aot/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
aot skills search telephony
aot skills install official/productivity/telephony
```
...
Then save credentials into Aot:
...
3. Buy it and save it into `~/.aot/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Aot calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Aot's existing `text_to_speech` support.

Use this when:
- you want the call to use Aot's configured TTS voice rather than Twilio `<Say>`
...
Recommended Aot TTS -> Twilio Play workflow:

1. Generate the audio with Aot `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Aot to create the message audio with `text_to_speech`
...
Important note:
- Aot TTS is great for prerecorded outbound messages
...
- Aot STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Aot memory.
...
- Do not store arbitrary third-party phone numbers in Aot memory.
...
3. persist that number to `~/.aot/.env`
