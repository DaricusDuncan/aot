...
description: "Configure and use Honcho memory with Aot -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Aot -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `aot skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Aot

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Aot profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/aot#running-honcho-locally-with-aot

### Verify

```bash
aot honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Aot profile's AI peer
...
Honcho models conversations as interactions between **peers**. Aot creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Aot instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Aot run |
| `global` | Single session across all directories |

Manual override: `aot honcho map my-project-name`
...
Each Aot profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block aot.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `aot.coder` host block in `honcho.json`
...
```bash
aot honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Aot when Honcho memory is active.
...
Config file: `$AOT_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `aot honcho setup`. Ensure `memory.provider: honcho` is in `~/.aot/config.yaml`.

### Memory not persisting across sessions
Check `aot honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `aot profile create <name> --clone`. For existing profiles: `aot honcho sync`.
...
| Command | Description |
|---------|-------------|
| `aot honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `aot honcho status` | Show resolved config, connection test, peer info for active profile |
| `aot honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `aot honcho disable` | Disable Honcho for the active profile |
| `aot honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `aot honcho peers` | Show peer identities across all profiles |
| `aot honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `aot honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `aot honcho sessions` | List known directory-to-session-name mappings |
| `aot honcho map <name>` | Map current working directory to a Honcho session name |
| `aot honcho identity` | Seed AI peer identity or show both peer representations |
| `aot honcho sync` | Create host blocks for all Aot profiles that don't have one yet |
| `aot honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Aot + Honcho |
| `aot memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `aot memory status` | Show active memory provider and config |
| `aot memory off` | Disable external memory provider |
