...
Aot Agent can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.

Enable checkpoints per-session with `--checkpoints`:

```bash
aot chat --checkpoints
```

Or enable globally in `~/.aot/config.yaml`:
...
This safety net is powered by an internal **Checkpoint Manager** that keeps a single shared shadow git repository under `~/.aot/checkpoints/store/` — your real project `.git` is never touched. Every project the agent works in shares the same store, so git's content-addressable object DB deduplicates across projects and across turns.
...
| Command | Description |
|---------|-------------|
| `aot checkpoints` | Show total size, project count, per-project breakdown |
| `aot checkpoints status` | Same as bare `checkpoints` |
| `aot checkpoints list` | Alias for `status` |
| `aot checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `aot checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `aot checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Aot detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.aot/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/aot/<project-hash>`).
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.aot/checkpoints/ at startup
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `aot checkpoints prune` manually.
...
Aot responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.aot/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/aot-agent                                        20       2h ago  live
...
Clear with: aot checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
aot checkpoints prune --retention-days 3 --max-size-mb 200
```
...
- **Directory scope** — Aot skips overly broad directories (root `/`, home `$HOME`).
...
~/.aot/checkpoints/
  ...
  │   ├── refs/aot/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `aot checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.aot/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.aot/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
aot checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `aot chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `aot checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Aot session in its own worktree/branch, with checkpoints as an extra layer.
