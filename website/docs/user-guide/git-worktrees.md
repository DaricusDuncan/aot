...
title: "Git Worktrees"
description: "Run multiple Aot agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Aot Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Aot so each session has a clean, isolated working directory.

## Why Use Worktrees with Aot?

Aot treats the **current working directory** as the project root:

- CLI: the directory where you run `aot` or `aot chat`
...
git worktree add ../repo-feature feature/aot-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/aot-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Aot there:

```bash
cd ../repo-feature

# Start Aot in the worktree
aot
```
...
git worktree add ../repo-experiment-a feature/aot-a
git worktree add ../repo-experiment-b feature/aot-b
```
...
- Works on its own branch (`feature/aot-a` vs `feature/aot-b`).
...
- Aot checkpoint data under `~/.aot/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Aot experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/aot-checkpoints-docs`, `feature/aot-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Aot from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `aot -w` (Automatic Worktree Mode)

Aot has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `aot/aot-<hash>`).
...
```bash
aot -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `aot -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Aot session its own clean checkout.
