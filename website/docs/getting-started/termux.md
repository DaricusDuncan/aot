...
description: "Run Aot Agent directly on an Android phone with Termux"
---

# Aot on Android with Termux

This is the tested path for running Aot Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Aot from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Aot now ships a Termux-aware installer path:
...
- links `aot` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `aot` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/aot" "$PREFIX/bin/aot"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `aot` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.aot/.env`.
...
### `aot doctor` says ripgrep or Node is missing
