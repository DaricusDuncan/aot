...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.aot/.env` at the end.
...
4. Fill in:
   - **Name:** `Aot Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `aot-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the Aot app
New-CsApplicationAccessPolicy `
  -Identity "Aot-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Aot meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Aot-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.aot/.env`:
...
Aot ships a Graph auth smoke-test. From your Aot install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.aot/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `aot gateway restart`.
