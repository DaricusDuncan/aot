---
sidebar_position: 7
title: "Use SOUL.md with Aot"
description: "How to use SOUL.md to shape Aot Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Aot

`SOUL.md` is the **primary identity** for your Aot instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Aot to feel like the same assistant every time you talk to it — or if you want to replace the Aot persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Aot should be
- what Aot should avoid stylistically
- how Aot should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Aot is and how Aot speaks
...
Aot now uses only the global SOUL file for the current instance:

```text
~/.aot/SOUL.md
```

If you run Aot with a custom home directory, it becomes:
...
Aot automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Aot does not overwrite it
- if the file exists but is empty, Aot adds nothing from it to the prompt

## How Aot uses it

When Aot starts a session, it reads `SOUL.md` from `AOT_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Aot feels.
...
Aot already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Aot should sound.

# Avoid
What Aot should not do.

# Defaults
How Aot should behave when ambiguity appears.
```
...
Then restart Aot or start a new session.
...
### I edited SOUL.md but Aot still sounds the same

Check:
- you edited `~/.aot/SOUL.md` or `$AOT_HOME/SOUL.md`
...
### Aot is ignoring parts of my SOUL.md
