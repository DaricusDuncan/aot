...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Aot Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Aot instantly when PRs are opened or updated.
:::
...
- **Aot Agent installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  aot gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.aot/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Aot can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/aot-agent --state open --limit 3
```
...
Still in the chat, ask Aot to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/aot-agent
```
...
A skill gives Aot consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.aot/skills/code-review
```

Create `~/.aot/skills/code-review/SKILL.md`:
...
Verify it loaded — start `aot` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Aot your team's standards:
...
2. Teach Aot your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
aot gateway status    # Is the gateway running?
aot cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-aot-plugin)** — wrap the review logic into a shareable plugin
