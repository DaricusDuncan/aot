...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Aot Agent — no API key required"
---

# MiniMax OAuth

Aot Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Aot automatically refreshes your session.
...
# → Aot opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.aot/auth.json` and are refreshed automatically before each session.
...
```bash
aot auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.aot/.env
```
...
```bash
aot auth add minimax-oauth --no-browser
```

Aot will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Aot implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Aot generates a PKCE verifier / challenge pair and a random state value.
...
4. Aot polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.aot/auth.json` under the `minimax-oauth` key.
...
```bash
aot config set model MiniMax-M2.7
aot config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.aot/config.yaml` will contain entries similar to:
...
```bash
aot --provider minimax-oauth    # canonical
aot --provider minimax-portal   # alias
aot --provider minimax-global   # alias
aot --provider minimax_oauth    # alias (underscore form)
```
...
```bash
AOT_INFERENCE_PROVIDER=minimax-oauth aot
```
...
Aot refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Aot marks the session as requiring re-login.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Aot marks the refresh token as dead and quarantines it locally so it doesn't keep replaying the doomed exchange. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `aot auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Aot raises a timeout error.

**Fix:** re-run `aot auth add minimax-oauth` (or `aot model`). The flow starts fresh.

### State mismatch (possible CSRF)

Aot detected that the `state` value returned by the authorization server does not match what it sent.
...
If `aot` cannot open a browser window, use `--no-browser`:

```bash
aot auth add minimax-oauth --no-browser
```

Aot prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `aot model` and select MiniMax (OAuth), or run `aot auth add minimax-oauth`.
...
- [aot doctor](../reference/cli-commands.md)
