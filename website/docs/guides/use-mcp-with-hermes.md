---
sidebar_position: 6
title: "Use MCP with Aot"
description: "A practical guide to connecting MCP servers to Aot Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Aot

This guide shows how to actually use MCP with Aot Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Aot tool
- you want Aot to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Aot to internal APIs, databases, or company systems without modifying Aot core

Do not use MCP when:
- a built-in Aot tool already solves the job well
...
- Aot remains the agent
- MCP servers contribute tools
- Aot discovers those tools at startup or reload time
...
If you installed Aot with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Aot banner/status should show MCP integration when configured
- ask Aot what tools it has available
...
## WSL2: bridge Aot in WSL to Windows Chrome

This is the practical setup when:

- Aot runs inside WSL2
...
In this setup, Aot does **not** connect to Chrome directly. Instead:

- Aot runs in WSL
- Aot starts a local stdio MCP server
...
```text
Aot (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Aot stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Aot core browser transport
...
```bash
aot mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
aot mcp test chrome-devtools-win
```

Then start a fresh Aot session or run:
...
Once loaded, Aot can use the MCP-prefixed browser tools directly. For example:
...
If Aot runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Aot from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Aot from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Aot:
...
So Aot will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Aot to reason over a bounded workspace.
...
Now Aot can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Aot core.
...
Because Aot now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
