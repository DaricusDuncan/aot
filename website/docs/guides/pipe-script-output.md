...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `aot send`."
---

# Pipe Script Output to Messaging Platforms

`aot send` is a small, scriptable CLI that pushes a message to any
messaging platform Aot is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | aot send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `aot
...
aot send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | aot send --to telegram:-1001234567890

# Send a file
aot send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
aot send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
aot send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
aot send --list

# Filter by platform
aot send --list telegram
```
...
Any platform Aot ships adapters for works as a target:
...
`aot send` resolves the message body in this order:

1. **Positional argument** — `aot send --to telegram "hi"`
2. **`--file PATH`** — `aot send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | aot send --to telegram`

When stdin is a TTY (no pipe), Aot does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  aot send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `aot send` reuses your Aot config, the same script works on
any host where Aot is installed — no need to export bot tokens into
...
`aot send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  aot send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | aot send \
...
```bash
./train.py --epochs 200 && \
  aot send --to telegram "training done" || \
  aot send --to telegram "training failed (exit $?)"
```
...
aot send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(aot send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `aot send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `aot send` calls
the platform's REST endpoint directly using credentials from
`~/.aot/.env` and `~/.aot/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `aot gateway start` and retry.
...
The listing is built from `~/.aot/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`aot gateway
...
| Approach | Multi-platform | Reuses Aot creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `aot send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`aot send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `aot send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `aot send` shares with cron delivery.
