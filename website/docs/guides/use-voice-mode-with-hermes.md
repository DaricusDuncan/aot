---
sidebar_position: 8
title: "Use Voice Mode with Aot"
description: "A practical guide to setting up and using Aot voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Aot
...
- you want Aot sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Aot.
...
## Step 1: make sure normal Aot works first
...
Aot supports both local and cloud speech stacks.
...
### If you use `aot setup`

If you choose NeuTTS in the setup wizard, Aot checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Aot transcribes and responds
...
- asking Aot to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Aot loop.

## Tuning CLI behavior

### Silence threshold

If Aot starts/stops too aggressively, tune:
...
Aot stays a normal chat bot, but can speak replies.
...
- you want Aot to function like a portable research or ops assistant
...
Aot joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Aot detects speech boundaries
- transcripts are posted in the associated text channel
- Aot responds in text and audio
...
1. get text Aot working
2. install `aot-agent[voice]`
