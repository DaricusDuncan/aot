...
This tutorial walks you through setting up a Telegram bot powered by Aot Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Aot Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.aot/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Aot itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Aot Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_aot_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Aot Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u aot-gateway -f
...
aot gateway start
aot gateway stop
tail -f ~/.aot/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `aot gateway install` to update the plist.
:::
...
```bash
aot gateway stop && aot gateway start
```
...
aot pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.aot/.env`:
...
Control how much detail the bot shows when using tools. In `~/.aot/config.yaml`:
...
Customize how the bot communicates by editing `~/.aot/SOUL.md`:

For a full guide, see [Use SOUL.md with Aot](/docs/guides/use-soul-with-aot).
...
```bash
# From the CLI
aot cron list          # View all scheduled jobs
aot cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u aot-gateway` (Linux) or `~/.aot/logs/gateway.log` (macOS) |
| Cron job output | `~/.aot/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.aot/cron/jobs.json` |
| Pairing data | `~/.aot/pairing/` |
| Session history | `~/.aot/sessions/` |
