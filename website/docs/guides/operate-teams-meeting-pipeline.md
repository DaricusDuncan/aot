...
```bash
aot teams-pipeline token-health
aot teams-pipeline token-health --force-refresh
```
...
```bash
aot teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
aot teams-pipeline maintain-subscriptions
aot teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Aot cron (recommended if you already run the Aot gateway)

Aot ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.aot/scripts/`. First create the script:

```bash
mkdir -p ~/.aot/scripts
cat > ~/.aot/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec aot teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.aot/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
aot cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/aot-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Aot Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=aot
EnvironmentFile=/etc/aot/env
ExecStart=/usr/local/bin/aot teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/aot-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Aot Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now aot-teams-pipeline-maintain.timer
systemctl list-timers aot-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/aot teams-pipeline maintain-subscriptions >> /var/log/aot/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.aot/.env` at the top of a wrapper script that crontab calls.
...
```bash
aot teams-pipeline subscriptions   # should show expirationDateTime advanced
aot teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
aot teams-pipeline list
aot teams-pipeline list --status failed
aot teams-pipeline show <job-id>
```

### Replay a stored job

```bash
aot teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
aot teams-pipeline fetch --meeting-id <meeting-id>
aot teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `aot teams-pipeline maintain-subscriptions --dry-run`
- inspect `aot teams-pipeline list --status failed`
...
- run `aot teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `aot teams-pipeline run`
...
- [ ] `aot teams-pipeline validate` returns an OK snapshot
- [ ] `aot teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Aot cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
