...
description: "Sign in with your SuperGrok subscription to use Grok models in Aot Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok Subscription)

Aot Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using your existing **SuperGrok subscription**. No `XAI_API_KEY` is required — log in once and Aot automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Aot — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Aot opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.aot/auth.json` and refreshed automatically before they expire.
...
```bash
aot auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, Aot detects the remote environment and prints the authorization URL instead of opening a browser.
...
aot auth add xai-oauth --no-browser
...
If you don't have a regular SSH client (e.g. you're running Aot inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — Aot skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
aot auth add xai-oauth --manual-paste
# Or via the model picker:
aot model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/NousResearch/aot-agent/issues/26923).

## How the Login Works

1. Aot opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Aot and the tokens are saved to `~/.aot/auth.json`.
4. From then on, Aot refreshes the access token in the background — you stay signed in until you `aot auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
aot config set model.default grok-4.3
aot config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.aot/config.yaml` will contain:
...
```bash
aot --provider xai-oauth        # canonical
aot --provider grok-oauth       # alias
aot --provider x-ai-oauth       # alias
aot --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `aot tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `aot tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
| `AOT_INFERENCE_PROVIDER` | Force the active provider at runtime, e.g. `AOT_INFERENCE_PROVIDER=xai-oauth aot`. |

## Troubleshooting

### Token expired — not re-logging in automatically

Aot refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Aot surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Aot marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `aot auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Aot raises a timeout error.

**Fix:** re-run `aot auth add xai-oauth` (or `aot model`). The flow starts fresh.

### State mismatch (possible CSRF)

Aot detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Aot prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `aot model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/aot-agent/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
aot config set model.provider xai
```
...
**Fix:** run `aot model` and pick the xAI Grok OAuth provider, or run `aot auth add xai-oauth`.
...
```bash
aot auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `aot auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `aot auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Aot is on a different machine than your browser
