...
Copy-paste recipes for common automation patterns. Each template uses Aot's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`aot webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `aot webhook subscribe`) |
...
```bash
aot cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/aot-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/aot-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
aot webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
aot cron create "0 9 * * 1" \
  "Scan the NousResearch/aot-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/aot-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (aot_cli/commands.py, aot_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (aot_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
aot cron create "0 6 * * *" \
  "Run a dependency security audit on the aot-agent project.

1. cd ~/.aot/aot-agent && source .venv/bin/activate
...
```bash
aot webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
aot webhook subscribe alert-triage \
...
```python title="~/.aot/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Aot-Monitor/1.0"})
...
aot cron create "every 30m" \
  ...
  --script ~/.aot/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
aot cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Aot Agent development." \
...
```bash
aot webhook subscribe github-issues \
  --events "issues" \
...
```bash
aot webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
aot webhook subscribe stripe-payments \
...
```bash
aot cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the aot-agent codebase.
