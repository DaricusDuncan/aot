...
2. **Aot spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.aot/cron/output/` and you can read them anytime.
:::
...
Aot will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Aot Agent
```
...
Before creating cron jobs, ensure Aot has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Aot what you want:
...
Aot will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Aot to delegate each topic to a sub-agent:
...
Aot will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
