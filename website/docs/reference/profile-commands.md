...
This page covers all commands related to [Aot profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `aot profile` without a subcommand shows help.
...
```bash
aot profile use <name>
```

Sets `<name>` as the active profile. All subsequent `aot` commands (without `-p`) will use this profile.
...
aot profile use default
```

## `aot profile create`

```bash
aot profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `aot profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `aot update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
aot profile create mybot

# Clone config only from current profile
aot profile create work --clone

# Clone everything from current profile
aot profile create backup --clone-all

# Clone config from a specific profile
aot profile create work2 --clone --clone-from work
```

## `aot profile describe`

```bash
aot profile describe [<name>] [options]
```
...
aot profile describe researcher

# Set it explicitly
aot profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
aot profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
aot profile describe --all --auto
```

## `aot profile delete`

```bash
aot profile delete <name> [options]
```
...
```bash
aot profile delete mybot
aot profile delete mybot --yes
```
...
```bash
aot profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Aot home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ aot profile show work
Profile: work
Path:    ~/.aot/profiles/work
...
```bash
aot profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Aot installation.
...
```bash
aot profile alias work
# Creates/updates ~/.local/bin/work

aot profile alias work --name mywork
# Creates ~/.local/bin/mywork

aot profile alias work --remove
...
```bash
aot profile rename <old-name> <new-name>
```
...
```bash
aot profile rename mybot assistant
# ~/.aot/profiles/mybot → ~/.aot/profiles/assistant
...
```bash
aot profile export <name> [options]
```
...
```bash
aot profile export work
# Creates work.tar.gz in the current directory

aot profile export work -o ./work-2026-03-29.tar.gz
```

## `aot profile import`

```bash
aot profile import <archive> [options]
```
...
```bash
aot profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

aot profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`aot profile export` / `import` are still the right commands for
...
```bash
aot profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `aot -p telemetry`). |
...
aot profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
aot profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
aot profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
aot profile install ./telemetry/
```

### `aot profile update`

```bash
aot profile update <name> [--force-config] [--yes]
```
...
```bash
aot profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Aot version, author, env var requirements, the source URL/path, and
...
`aot profile list` also shows the distribution name and version in a
`Distribution` column, and `aot profile show <name>` / `delete <name>`
...
aot profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
aot profile install https://github.com/your-org/internal-assistant.git
```
...
`aot_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Aot version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Aot can clone from.
3. Tell recipients to run `aot profile install <your-repo-url>`.
...
```bash
aot -p <name> <command> [options]
aot --profile <name> <command> [options]
```

Global flag to run any Aot command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
aot -p work chat -q "Check the server status"
aot --profile dev gateway start
aot -p personal skills list
aot -p work config edit
```

## `aot completion`

```bash
aot completion <shell>
```
...
- `aot profile <TAB>` — subcommands (list, use, create, etc.)
- `aot profile use <TAB>` — profile names
- `aot -p <TAB>` — profile names
