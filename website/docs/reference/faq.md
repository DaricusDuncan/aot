...
description: "Frequently asked questions and solutions to common issues with Aot Agent"
---
...
### What LLM providers work with Aot?

Aot Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — Nous Research's own inference endpoint
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `aot login anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `aot model` or by editing `~/.aot/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Aot Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Aot from inside it. The standard install command works perfectly in WSL2:

```bash
curl -fsSL https://raw.githubusercontent.com/NousResearch/aot-agent/main/scripts/install.sh | bash
```

### I run Aot in WSL2. What's the best way to control my normal Windows Chrome?
...
- let Aot use the resulting MCP browser tools

This is more reliable than trying to force Aot core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Aot](../guides/use-mcp-with-aot.md#wsl2-bridge-aot-in-wsl-to-windows-chrome)
...
Yes — Aot now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Aot Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.aot/`.

### Can I use it offline / with local models?

Yes. Run `aot model`, select **Custom endpoint**, and enter your server's URL:
...
Aot persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Aot — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Aot auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `AOT_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Aot Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Aot Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Aot programmatically:
...
#### `aot: command not found` after installation
...
**Cause:** Aot requires Python 3.11 or newer.
...
**Cause:** Aot builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Aot runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Aot auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.aot/config.yaml`:
...
**Solution:** Exit your session and use `aot model` from your terminal to add new providers:

```bash
# Exit the Aot chat session first (Ctrl+C or /quit)
...
After adding a new provider via `aot model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `aot model` (from terminal) |
| Enter/change API keys | `aot model` (from terminal) |
...
# Or set directly
aot config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.aot/.env` for conflicting entries.
:::
...
# Set a valid model
aot config set AOT_MODEL anthropic/claude-opus-4.7

# Or specify per-session
aot chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `aot chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Aot detected the wrong context length for your model.
...
aot chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Aot may have the wrong context length for your model. Check what it detected:
...
**Cause:** Aot detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Aot never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `aot chat`
...
cat ~/.aot/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `aot gateway setup`
- Check gateway logs: `cat ~/.aot/logs/gateway.log | tail -50`
...
Configure in `~/.aot/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "aot-agent[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `aot gateway start` fails
...
tmux new -s aot 'aot gateway run'
# Reattach later: tmux attach -t aot

# Option 3: Background via nohup
nohup aot gateway run > ~/.aot/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'aot gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `aot gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
aot gateway install    # Re-snapshots your current PATH
aot gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.aot.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `aot chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `aot chat -t "terminal"`
...
cd ~/.aot/aot-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.aot/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
aot config show | grep -A 12 mcp_servers

# Restart Aot or reload MCP after config changes
aot chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Aot](/docs/guides/use-mcp-with-aot)
...
:::warning
If an MCP server crashes mid-request, Aot will report a timeout. Check the server's own logs (not just Aot logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `aot profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `aot update`?

`aot update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `aot update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.aot/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Aot can route subagents to a different model automatically. Set this in `~/.aot/config.yaml`:
...
Now when you tell Aot "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Aot do this?

**Current limitation:** Aot profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Aot reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `aot skills config` settings don't seem to take effect.

**Solution:** Use `aot skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`aot gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Aot creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Aot to another machine
...
1. Install Aot Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/NousResearch/aot-agent/main/scripts/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   aot backup
   ```
   This creates a zip of your entire `~/.aot/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/aot-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/aot-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   aot import ~/aot-backup-<timestamp>.zip
   ```

4. On the new machine, run `aot setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
aot profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
aot profile import ./work-backup.tar.gz work
```
...
### `aot backup` vs `aot profile export`

| Feature | `aot backup` | `aot profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.aot` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='aot-agent' ~/.aot/ newmachine:~/.aot/
```

:::tip
`aot backup` produces a consistent snapshot even while Aot is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Aot installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Aot-specific issue — it's a shell config permissions problem.
...
aot config show | head -20

# Re-run model selection
aot model

# Or test with a known-good model
aot chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/aot-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Aot version (`aot --version`), and the full error message
