...
Aot fetches curated model lists for **OpenRouter** and **Nous Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `aot-agent` release.

When the manifest is unreachable (offline, network blocked, hosting failure), Aot silently falls back to the in-repo snapshot that ships with the CLI. The manifest never breaks the picker — worst case you see whatever list was bundled with your installed version.

## Live manifest URL

```
https://aot-agent.nousresearch.com/docs/api/model-catalog.json
```
...
- **`version`** — integer schema version. Future schemas bump this; Aot refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. Aot ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
...
| When | What happens |
|---|---|
| `/model` or `aot model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.aot/cache/model_catalog.json`.
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["nous"] in aot_cli/models.py).
