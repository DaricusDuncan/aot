#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Aot docs site.
...
Both publish at:
  https://aot-agent.nousresearch.com/docs/llms.txt
  https://aot-agent.nousresearch.com/docs/llms-full.txt
...
SITE_BASE = "https://aot-agent.nousresearch.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Aot as a Python Library", None),
        ("guides/use-mcp-with-aot", "Use MCP with Aot", None),
        ("guides/use-voice-mode-with-aot", "Use Voice Mode with Aot", None),
        ("guides/use-soul-with-aot", "Use SOUL.md with Aot", None),
        ("guides/build-a-aot-plugin", "Build a Aot Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Aot"),
...
def emit_llms_index() -> str:
    ...
    lines.append(
        ...
        "aot-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/NousResearch/aot-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Aot Agent — Full Documentation\n",
        (
            "This file is the entire Aot Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Aot, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://aot-agent.nousresearch.com/docs\n",
        "Short index: https://aot-agent.nousresearch.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
