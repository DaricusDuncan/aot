...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/aot-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    aot_meta = (fm.get("metadata") or {}).get("aot") or {}
    tags = aot_meta.get("tags") or []
    related = aot_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `aot skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Aot Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Aot ships with a large built-in skill library copied into `~/.aot/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Aot also syncs bundled skills on `aot update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.aot/skills/` tree, it is still shipped with Aot; restore it with `aot skills reset <name> --restore`.",
        "",
