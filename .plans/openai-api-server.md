# OpenAI-Compatible API Server for Aot Agent
...
SSE streaming. By exposing this endpoint, aot-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  aot-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.aot/.env`
2. Run `aot gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with aot-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns aot-agent as a model) |
...
Authorization: Bearer aot-api-key-here
...
But aot-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "aot-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured aot-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of aot-agent's core prompt
...
Once implemented, aot-agent works as a drop-in backend for:
