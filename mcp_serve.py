"""
Aot MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Aot-specific extra)

Usage:
    aot mcp serve
    aot mcp serve --verbose
...
logger = logging.getLogger("aot.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using AOT_HOME."""
    try:
        from aot_constants import get_aot_home
        return get_aot_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("AOT_HOME", Path.home() / ".aot")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from aot_constants import get_aot_home
        directory_file = get_aot_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("AOT_HOME", Path.home() / ".aot")
...
class EventBridge:
    ...
    This is the Aot equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from aot_constants import get_aot_home
            db_file = get_aot_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("AOT_HOME", Path.home() / ".aot")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Aot MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "aot",
        instructions=(
            "Aot Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Aot MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
