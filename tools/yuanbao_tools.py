...
提供以下工具函数，供 aot-agent 的 "aot-yuanbao" toolset 使用：
...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（AOT_SESSION_CHAT_ID）。
    ...
    target = (chat_id or "").strip() or get_session_env("AOT_SESSION_CHAT_ID", "")
    if not target:
        return {
            "success": False,
...
def _check_yuanbao():
    ...
    try:
        from gateway.session_context import get_session_env
        if get_session_env("AOT_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
