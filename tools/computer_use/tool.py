...
# regardless of approval level (e.g. logout kills the session Aot runs in).
_BLOCKED_KEY_COMBOS = {
...
def _get_backend() -> ComputerUseBackend:
    ...
    with _backend_lock:
        if _backend is None:
            backend_name = os.environ.get("AOT_COMPUTER_USE_BACKEND", "cua").lower()
            if backend_name in {"cua", "cua-driver", ""}:
                ...
                raise RuntimeError(f"Unknown AOT_COMPUTER_USE_BACKEND={backend_name!r}")
            _backend.start()
        return _backend
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "Run `aot tools` and enable Computer Use to install cua-driver.",
        })
