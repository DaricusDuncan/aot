"""
Lazy dependency installer for opt-in Aot Agent backends.

Many Aot features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``aot-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``aot tools`` or the manual pip command.
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("AOT_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``aot update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``aot_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``aot update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``aot update``. Never raises; lazy-install failures
