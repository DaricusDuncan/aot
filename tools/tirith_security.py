...
it is automatically downloaded from GitHub releases to $AOT_HOME/bin/tirith.
...
def _get_aot_home() -> str:
    """Return the Aot home directory, respecting AOT_HOME env var."""
    return str(get_aot_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_aot_home(), ".tirith-install-failed")
...
def _aot_bin_dir() -> str:
    """Return $AOT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_aot_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $AOT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_aot_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $AOT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $AOT_HOME/bin/tirith
    ...
    aot_bin = os.path.join(_aot_bin_dir(), "tirith")
    if os.path.isfile(aot_bin) and os.access(aot_bin, os.X_OK):
        _resolved_path = aot_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return aot_bin
