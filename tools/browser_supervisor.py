...
One ``CDPSupervisor`` runs per Aot ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "aot-dialog-bridge.invalid"
...
(() => {
  if (window.__aotDialogBridgeInstalled) return;
  window.__aotDialogBridgeInstalled = true;
  const ENDPOINT = "http://aot-dialog-bridge.invalid/";
