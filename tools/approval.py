...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("AOT_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("AOT_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set AOT_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind AOT_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds AOT_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("AOT_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $AOT_HOME.
...
DANGEROUS_PATTERNS = [
    ...
    (r'\baot\s+gateway\s+(stop|restart)\b', "stop/restart aot gateway (kills running agents)"),
    (r'\baot\s+update\b', "aot update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart aot-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart aot-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(aot|gateway|cli\.py)\b', "kill aot/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill aot` but not
    # `kill -9 $(pgrep -f aot)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("AOT_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("AOT_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("AOT_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("AOT_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("AOT_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("AOT_EXEC_ASK")
