...
def _global_allow_private_urls() -> bool:
    ...
    1. ``AOT_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("AOT_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``AOT_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
