...
This module provides the glue:
    - ``AotTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Aot Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses AOT_HOME so each profile gets its own OAuth tokens.
    Layout: ``AOT_HOME/mcp-tokens/``
    """
    try:
        from aot_constants import get_aot_home
        base = Path(get_aot_home())
    except ImportError:
        base = Path(os.environ.get("AOT_HOME", str(Path.home() / ".aot")))
...
# ---------------------------------------------------------------------------
# AotTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class AotTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        AOT_HOME/mcp-tokens/<server_name>.json         -- tokens
        AOT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        AOT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Aot records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Aot.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://aot-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Aot Agent")
    scope = cfg.get("scope")
