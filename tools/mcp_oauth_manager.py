...
`mcp_tool.py`, and `aot_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# AotMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_aot_provider_class() -> Optional[type]:
    ...
    class AotMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._aot_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`AotTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, AotTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import AotTokenStorage
                        if isinstance(storage, AotTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._aot_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import AotTokenStorage
            if not isinstance(storage, AotTokenStorage):
                return
    ...
    return AotMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_AOT_PROVIDER_CLS: Optional[type] = _make_aot_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`AotMCPOAuthProvider` directly using the helpers
        ...
        storage = AotTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``aot mcp remove <name>`` and (indirectly) by
        ``aot mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
