"""Base class for all Aot execution environment backends.
...
# AOT_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("AOT_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {AOT_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_aot_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__AOT_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Aot backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/aot-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/aot-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../aot-snap-*.sh: No such file or directory``
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"AOT_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via AOT_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __AOT_CWD_{session}__ marker from stdout output.
