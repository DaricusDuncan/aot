...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Aot'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``AOT_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_aot_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.aot"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_aot = (
            "/.aot"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.aot"
        )
        archive_member = remote_aot.lstrip("/")
        remote_tar = f"/tmp/.aot_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
