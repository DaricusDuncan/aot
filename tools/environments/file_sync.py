...
_FORCE_SYNC_ENV = "AOT_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.aot") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.aot to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``AOT_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, aot_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.aot/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="aot-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.aot/skills/a.md`` →
        ``~/.aot/skills/a.md``, a new remote file at
        ``/root/.aot/skills/b.md`` maps to ``~/.aot/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
