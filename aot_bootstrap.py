"""Windows UTF-8 bootstrap for Aot entry points.
...
should be imported at the very top of every Aot entry point
(``aot``, ``aot-agent``, ``aot-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import aot_bootstrap``
# (or ``from aot_bootstrap import apply_windows_utf8_bootstrap``) at
