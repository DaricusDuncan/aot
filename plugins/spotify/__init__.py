...
``_check_spotify_available()`` — when the user has not run ``aot auth
spotify``, the tools remain registered (so they appear in ``aot
...
The Spotify auth flow (``aot auth spotify``), CLI plumbing, and docs
