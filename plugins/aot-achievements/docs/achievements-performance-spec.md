# Aot Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: aot-achievements plugin
...
### `GET /api/plugins/aot-achievements/achievements`
...
### `POST /api/plugins/aot-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/aot-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/aot-achievements/overview`
...
- Persist plugin data under:
  - `~/.aot/plugins/aot-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.aot/plugins/aot-achievements/scan_checkpoint.json`
