...
Auto-tracks and cleans up ephemeral files created during Aot Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$AOT_HOME` and `/tmp/aot-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `AOT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `AOT_HOME` or `/tmp/aot-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$AOT_HOME/disk-cleanup/` is itself excluded
- `$AOT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
