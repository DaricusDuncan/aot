/**
 * Aot Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.aot/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__AOT_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "aot-kanban-dot-triage",
    todo: "aot-kanban-dot-todo",
    ready: "aot-kanban-dot-ready",
    running: "aot-kanban-dot-running",
    blocked: "aot-kanban-dot-blocked",
    done: "aot-kanban-dot-done",
    archived: "aot-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-aot-task";
  ...
  const DOCS_URL = "https://aot-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://aot-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "aot.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="aot-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "aot-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "aot-kanban-md",
  ...
  // standard `drop` event and our `aot-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("aot-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("aot-kanban-column--drop");
          if (target) target.classList.add("aot-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("aot-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("aot-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("aot-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "aot-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "aot-kanban-attention",
        "aot-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "aot-kanban-attention-bar" },
        h("span", { className: "aot-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "aot-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "aot-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "aot-kanban-attention-row",
                  "aot-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "aot-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "aot-kanban-attention-row-id" }, task.id),
                h("span", { className: "aot-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "aot-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "aot-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "aot-kanban-diag-action-btn",
      action.suggested ? "aot-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " aot-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".aot-kanban-drawer-comment-row input, .aot-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "aot-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("aot-kanban-diag", sevClass) },
      h("div", { className: "aot-kanban-diag-header" },
        h("span", { className: "aot-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "aot-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "aot-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "aot-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "aot-kanban-diag-data-row" },
                  h("span", { className: "aot-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "aot-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "aot-kanban-diag-data-row" },
                h("span", { className: "aot-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "aot-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "aot-kanban-diag-reassign-row" },
            h("span", { className: "aot-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "aot-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "aot-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "aot-kanban-diag-msg",
              msg.ok ? "aot-kanban-diag-msg--ok" : "aot-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "aot-kanban-section" },
      h("div", { className: "aot-kanban-section-head-row" },
        h("span", { className: "aot-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "aot-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "aot-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "aot-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "aot-kanban-docs-link",
      title: "Open Aot Kanban docs in a new tab",
      "aria-label": "Aot Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `aot kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "aot-kanban-msg-ok" : "aot-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "aot-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "aot-kanban-boardswitcher" },
      h("div", { className: "aot-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "aot-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "aot-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "aot-kanban-dialog-title" },
        ...
        h("div", { className: "aot-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Aot profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "aot-kanban-bulk" },
      h("span", { className: "aot-kanban-bulk-count" },
      ...
      h("div", { className: "aot-kanban-bulk-priority",
      ...
      h("div", { className: "aot-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Aot profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "aot-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("aot-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("aot-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "aot-kanban-trash",
        dragOver ? "aot-kanban-trash--drop" : "",
        props.draggingTaskId ? "aot-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "aot-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "aot-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".aot-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "aot-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("aot-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("aot-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "aot-kanban-column",
        dragOver ? "aot-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "aot-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "aot-kanban-col-check",
        ...
        h("span", { className: cn("aot-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "aot-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "aot-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "aot-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "aot-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "aot-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "aot-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "aot-kanban-lane" },
                  h("div", { className: "aot-kanban-lane-head" },
                    h("span", { className: "aot-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "aot-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "aot-kanban-card--stale-red";
    if (age >= tier.amber) return "aot-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".aot-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "aot-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "aot-kanban-card",
        props.selected ? "aot-kanban-card--selected" : "",
        props.failed ? "aot-kanban-card--failed" : "",
        props.draggingSource ? "aot-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "aot-kanban-card-content" },
          h("div", { className: "aot-kanban-card-row" },
            h("label", {
              className: "aot-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "aot-kanban-card-check",
            ...
            h("span", { className: "aot-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or aot kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "aot-kanban-warning-badge",
                    "aot-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            progress
              ? h("span", {
                  className: cn(
                    "aot-kanban-progress",
                    progress.done === progress.total ? "aot-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "aot-kanban-needs-assignee",
          ...
          h("div", { className: "aot-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "aot-kanban-card-row aot-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "aot-kanban-assignee",
                            title: `Assigned to Aot profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "aot-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "aot-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "aot-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "aot-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Aot profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Aot profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "aot-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "aot-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "aot-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "aot-kanban-drawer-close",
        ...
        data ? h("div", { className: "aot-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "aot-kanban-drawer-body" },
      h("div", { className: "aot-kanban-drawer-title" },
        h("span", { className: cn("aot-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
      ...
      t.result ? h("div", { className: "aot-kanban-section" },
        h("div", { className: "aot-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "aot-kanban-section" },
        h("div", { className: "aot-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "aot-kanban-comment" },
                h("div", { className: "aot-kanban-comment-head" },
                  h("span", { className: "aot-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "aot-kanban-comment-ago" },
      ...
      h("div", { className: "aot-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "aot-kanban-event",
              isDiag ? "aot-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "aot-kanban-event-header" },
                  h("span", { className: "aot-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "aot-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "aot-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "aot-kanban-event-header-plain" },
                  h("span", { className: "aot-kanban-event-kind" }, e.kind),
                  h("span", { className: "aot-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "aot-kanban-event-phantom-row" },
                  h("span", { className: "aot-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "aot-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "aot-kanban-event-payload" },
    ...
    return h("div", { className: "aot-kanban-section" },
      h("div", { className: "aot-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "aot-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `aot-kanban-run--${r.outcome || r.status || "ended"}`
          : "aot-kanban-run--active";
        return h("div", { key: r.id, className: cn("aot-kanban-run", outcomeClass) },
          h("div", { className: "aot-kanban-run-head" },
            h("span", { className: "aot-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "aot-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "aot-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "aot-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "aot-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "aot-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "aot-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "aot-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "aot-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "aot-kanban-pre aot-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "aot-kanban-meta-row" },
      h("span", { className: "aot-kanban-meta-label" }, props.label),
      h("span", { className: "aot-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "aot-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "aot-kanban-meta-row" },
        h("span", { className: "aot-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "aot-kanban-meta-value aot-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "aot-kanban-meta-row" },
        h("span", { className: "aot-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "aot-kanban-meta-value aot-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "aot-kanban-section" },
      h("div", { className: "aot-kanban-section-head-row" },
        h("span", { className: "aot-kanban-section-head" }, tx(t, "description", "Description")),
        editing
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "aot-kanban-section" },
      h("div", { className: "aot-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "aot-kanban-deps-row" },
        h("span", { className: "aot-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "aot-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "aot-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "aot-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "aot-kanban-dep-chip-x",
      ...
      h("div", { className: "aot-kanban-deps-row" },
        h("span", { className: "aot-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "aot-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "aot-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "aot-kanban-section" },
      ...
      h("div", { className: "aot-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "aot-kanban-home-sub aot-kanban-home-sub--on"
              : "aot-kanban-home-sub",
          }, label);
        })
      )
  ...
  if (window.__AOT_PLUGINS__ && typeof window.__AOT_PLUGINS__.register === "function") {
    window.__AOT_PLUGINS__.register("kanban", KanbanPage);
  }
})();
