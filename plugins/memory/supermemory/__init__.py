...
def _load_supermemory_config(aot_home: str) -> dict:
    config = _default_config()
    config_path = Path(aot_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, aot_home: str) -> None:
    config_path = Path(aot_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `aot memory setup`.
        # All other options are documented for $AOT_HOME/supermemory.json
    ...
    def save_config(self, values, aot_home):
        ...
        _save_supermemory_config(sanitized, aot_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from aot_constants import get_aot_home
        self._aot_home = kwargs.get("aot_home") or str(get_aot_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._aot_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "aot", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "aot_memory", "target": target, "type": "explicit_memory"},
