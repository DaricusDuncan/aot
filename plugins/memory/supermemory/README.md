...
Config file: `$AOT_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `aot` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `aot-{identity}` → `aot-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Aot profile:

```json
{
  "container_tag": "aot-{identity}"
}
```

For a profile named `coder`, this resolves to `aot-coder`. The default profile resolves to `aot-default`. Without `{identity}`, all profiles share the same container.
