...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/aot>
...
```bash
aot honcho setup    # full interactive wizard (cloud or local)
aot memory setup    # generic picker, also works
```

Or manually:
```bash
aot config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$AOT_HOME/honcho.json` | Profile-local (isolated Aot instances) |
| 2 | `~/.aot/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Aot profile: `aot` (default) or `aot.<profile>`.
...
| 4 | `per-session` strategy | Aot session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`aot-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`aot`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-aot-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening aot in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Aot session ID (timestamp + hex). Every `aot` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Aot profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`aot`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Aot profile: `aot` (default) or `aot.<profile>` (e.g. `aot -p coder` → host key `aot.coder`).
...
| `AOT_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `aot honcho setup` | Full interactive setup wizard |
| `aot honcho status` | Show resolved config for active profile |
| `aot honcho enable` / `disable` | Toggle Honcho for active profile |
| `aot honcho mode <mode>` | Change recall or observation mode |
| `aot honcho peer --user <name>` | Update user peer name |
| `aot honcho peer --ai <name>` | Update AI peer name |
| `aot honcho tokens --context <N>` | Set context token budget |
| `aot honcho tokens --dialectic <N>` | Set dialectic max chars |
| `aot honcho map <name>` | Map current directory to a session name |
| `aot honcho sync` | Create host blocks for all Aot profiles |
