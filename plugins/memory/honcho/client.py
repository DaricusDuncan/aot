...
Resolution order for config file:
  1. $AOT_HOME/honcho.json  (instance-local, enables isolated Aot instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Aot profile.

    Resolution order:
      1. AOT_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``aot.<profile>``
      3. Fallback: ``"aot"`` (default profile)
    """
    explicit = os.environ.get("AOT_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $AOT_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.aot/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_aot_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_aot_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Aot system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.aot
    ...
    def from_global_config(
        ...
        Resolution: $AOT_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Aot profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.aot block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Aot session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Aot session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Aot session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'aot honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run aot honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `aot honcho setup` to configure)."
        )
