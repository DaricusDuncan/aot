...
Handles: aot honcho setup | status | sessions | map | peer
"""
...
def cmd_sync(args) -> None:
    ...
    Scans all Aot profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'aot honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'aot honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `aot update` -- no output, no exceptions.
    """
    try:
        from aot_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Aot profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $AOT_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_aot_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Aot persistent cross-session memory.")
    ...
    aot_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'aot honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = aot_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        aot_host["peerName"] = new_peer

    current_ai = aot_host.get("aiPeer") or cfg.get("aiPeer", "aot")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        aot_host["aiPeer"] = new_ai

    current_workspace = aot_host.get("workspace") or cfg.get("workspace", "aot")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        aot_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = aot_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        aot_host["observationMode"] = new_obs
    else:
        aot_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(aot_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        aot_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        aot_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = aot_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        aot_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = aot_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        aot_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                aot_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(aot_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            aot_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        aot_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        aot_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        aot_host["dialecticReasoningLevel"] = new_reasoning
    else:
        aot_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = aot_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        aot_host["sessionStrategy"] = new_strat

    aot_host["enabled"] = True
    aot_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        aot_config = load_config()
        aot_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(aot_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: aot config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    aot honcho status     -- show full config")
    print("    aot honcho mode       -- change recall/observation mode")
    print("    aot honcho tokens     -- tune context and dialectic budgets")
    print("    aot honcho peer       -- update peer names")
    print("    aot honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Aot profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: aot honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'aot honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: aot honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        aot = hosts.get(_host_key(), {})
        user = aot.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = aot.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = aot.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = aot.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Aot' identity in Honcho. Seed with 'aot honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: aot honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: aot honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = aot.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = aot.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = aot.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Aot asks Honcho's AI peer a question")
        ...
        print("\n  Set with: aot honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'aot honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'aot honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    aot honcho identity --show        — show both peer representations")
        print("    aot honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Aot + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Aot\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Aot persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  aot honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'aot honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'aot honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  aot honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'aot'")
        print("  with Honcho configured and no prior session history.")
        print("  (Aot calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    aot honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'aot honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Aot, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'aot honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    aot honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  aot honcho identity <path/to/SOUL.md>")
    ...
    print("    Aot:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Aot:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Aot:   Honcho observes every message and updates representations")
    ...
    print("    Aot:   per-session by default — each run gets its own session")
    print("              Map a custom name:  aot honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. aot honcho setup              — configure API key (required)")
        print("  2. aot honcho migrate            — re-run this walkthrough")
    else:
        print("  1. aot honcho status             — verify Honcho connection")
        print("  2. aot                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. aot honcho identity --show    — verify AI peer representation")
        print("  4. aot honcho tokens             — tune context and dialectic budgets")
        print("  5. aot honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'aot memory setup'...\n")
        from aot_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``aot honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``aot honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to aot memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Aot Honcho",
    )
