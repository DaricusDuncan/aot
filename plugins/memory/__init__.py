...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with aot-agent)
2. User-installed providers: ``$AOT_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$AOT_HOME/plugins/`` or None if unavailable."""
    try:
        from aot_constants import get_aot_home
        d = get_aot_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$AOT_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_aot_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_aot_user_memory.{active_provider}.cli"
    try:
