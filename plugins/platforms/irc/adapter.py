"""
IRC Platform Adapter for Aot Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Aot agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "aot-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Aot Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Aot Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: aot_, aot_1, aot_2...
...
def check_requirements() -> bool:
    ...
    # aot setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `aot gateway setup` flow for the IRC platform.

    Lazy-imports ``aot_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Aot to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #aot — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.aot/.env")
    print_info("Restart the gateway for changes to take effect: aot gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "aot-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "aot-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Aot Agent (cron)")
