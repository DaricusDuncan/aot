"""SimpleX Chat platform adapter (Aot plugin).
...
This adapter ships as a Aot platform plugin under
``plugins/platforms/simplex/``. The Aot plugin loader scans the
...
discoverable and `aot setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Aot — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"aot-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``aot setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.aot/.env`` via ``aot_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("aot_cli.config not available; set SIMPLEX_* vars manually in ~/.aot/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `aot gateway status` without
