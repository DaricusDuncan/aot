"""
LINE Messaging API platform adapter for Aot Agent.
...
to Aot Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``AOT_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            aot_home = Path.home().joinpath(".aot").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``aot status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``aot status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``aot setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.aot/.env`` via ``aot_cli.config``.
    """
    print()
    ...
    try:
        from aot_cli.config import get_env_var, set_env_var
    except ImportError:
        print("aot_cli.config not available; set LINE_* vars manually in ~/.aot/.env")
        return
