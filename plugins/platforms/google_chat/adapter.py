...
# ``model_tools`` import time, so ``aot status``, ``aot chat``, etc.
...
# ``aot_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Aot is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a aot MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${AOT_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _aot_home = _get_aot_home()
        except (ModuleNotFoundError, ImportError):
            _aot_home = _Path.home() / ".aot"
        self._thread_count_store = _ThreadCountStore(
            _aot_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Aot is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("AOT_HOME", str(_Path.home() / ".aot"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${AOT_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a aot MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Aot is thinking…' marker message.
        ...
        "Aot is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Aot is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Aot is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Aot is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Aot is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``aot setup``.

    The setup wizard at ``aot_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.aot/.env`` so the next
    gateway restart picks them up.
    """
    from aot_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. aot-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.aot/.env")
    print_info("Restart the gateway: aot gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``aot cron`` running as a
    separate process from ``aot gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Aot plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'aot-agent[google_chat]'",
        ...
        # the "Aot is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Aot is thinking…' marker message "
