"""
Microsoft Teams platform adapter for Aot Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Aot"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        aot_action = data.get("aot_action", "")
        session_key = data.get("session_key", "")

        if not aot_action or not session_key:
        ...
        choice = choice_map.get(aot_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="aot_approve",
                    data={**btn_data_base, "aot_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="aot_approve",
                    data={**btn_data_base, "aot_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="aot_approve",
                    data={**btn_data_base, "aot_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="aot_approve",
                    data={**btn_data_base, "aot_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Aot\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.aot/.env")
