...
three virtual model IDs so the ``aot tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `aot tools` → Image "
                    "Generation → OpenAI to configure, or `aot setup` "
                    "to add the key."
                ),
