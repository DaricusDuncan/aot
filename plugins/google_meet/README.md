...
Let the aot agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where aot runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `aot meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `aot meet` CLI |
...
| `cli.py` | `aot meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$AOT_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `aot meet node {run,list,approve,remove,status,ping}` |
...
aot meet install                                      # pip + Chromium
aot meet setup                                        # preflight
aot meet auth                                         # optional
aot meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
aot meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.aot/.env
aot meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
aot meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
aot meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.aot/.env
aot meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, aot will **not** switch your system audio input automatically — the
...
aot plugins enable google_meet
aot meet node run --display-name my-mac --host 0.0.0.0 --port 18789
