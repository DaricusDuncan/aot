...
Launched by ``aot meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$AOT_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``aot meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_aot_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "aot-meet-node",
    ) -> None:
        self.host = host
        self.port = port
