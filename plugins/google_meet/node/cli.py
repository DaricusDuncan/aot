"""`aot meet node ...` subcommand tree.

Wired into the existing ``aot meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``aot meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="aot-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             aot meet node approve <name> ws://<host>:{args.port} {token}")
        try:
