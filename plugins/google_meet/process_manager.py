...
session-scoped state file under ``$AOT_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $AOT_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_aot_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this aot install, leave it first —
    ...
    env = os.environ.copy()
    env["AOT_MEET_URL"] = url
    env["AOT_MEET_OUT_DIR"] = str(out)
    env["AOT_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["AOT_MEET_HEADED"] = "1"
    if auth_state:
        env["AOT_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["AOT_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if AOT_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["AOT_MEET_MODE"] = mode
    if realtime_model:
        env["AOT_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["AOT_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["AOT_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["AOT_MEET_REALTIME_KEY"] = realtime_api_key
