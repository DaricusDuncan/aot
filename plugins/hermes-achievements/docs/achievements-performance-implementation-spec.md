# Aot Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.aot/plugins/aot-achievements/state.json` (existing unlock state)
  - `~/.aot/plugins/aot-achievements/scan_snapshot.json` (new)
  - `~/.aot/plugins/aot-achievements/scan_checkpoint.json` (new)
