...
AOT_HOME_DIR="${AOT_HOME:-$HOME/.aot}"
AOT_CFG="${AOT_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Aot config has twozero_td MCP entry ──
if [[ ! -f "$AOT_CFG" ]]; then
    echo -e " ${FAIL} Aot config not found at ${AOT_CFG}"
    manual_steps+=("Create ${AOT_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$AOT_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Aot config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Aot config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${AOT_CFG} manually"); }
    manual_steps+=("Restart Aot session to pick up config change")
fi
