#!/usr/bin/env bash
# GitHub environment detection helper for Aot Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif [ -f "$HOME/.aot/.env" ] && grep -q "^GITHUB_TOKEN=" "$HOME/.aot/.env" 2>/dev/null; then
    GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$HOME/.aot/.env" | head -1 | cut -d= -f2 | tr -d '\n\r')
