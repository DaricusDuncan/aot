...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("AOT_HOME", os.path.expanduser("~/.aot")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("AOT_HOME", Path.home() / ".aot")) / "skills" / "red-teaming" / "godmode"
...
AOT_HOME = Path(os.getenv("AOT_HOME", Path.home() / ".aot"))
CONFIG_PATH = AOT_HOME / "config.yaml"
PREFILL_PATH = AOT_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Aot models — already uncensored, just needs clean prompt
    "aot": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "aot" in model_lower or "nous" in model_lower:
        return "aot"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Aot config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.aot/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Aot for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Aot for changes to take effect.")
