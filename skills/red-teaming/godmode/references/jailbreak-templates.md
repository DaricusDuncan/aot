...
## 5. GODMODE FAST — Aot 4 405B (Zero Refusal)

**Model:** `nousresearch/aot-4-405b`
**Strategy:** Aot models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Aot Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.aot/config.yaml`:
