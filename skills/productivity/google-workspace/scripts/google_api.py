#!/usr/bin/env python3
"""Google Workspace API CLI for Aot Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Aot-facing JSON contract and falls back to the Python client
...
from _aot_home import get_aot_home
...
def _gws_binary() -> str | None:
    override = os.getenv("AOT_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Aot Agent")
