"""
Email platform adapter for the Aot gateway.

Allows users to interact with Aot by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "aot-agent" "version" "{_aot_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Aot Agent")
        ...
        msg_id = f"<aot-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
