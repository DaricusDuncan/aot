...
There are two ways to add a platform to the Aot gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.aot/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `aot gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `aot_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Aot core.
...
And add it to the `aot-gateway` composite:

```python
"aot-gateway": {
    "includes": [..., "aot-your-platform"]
}
```
...
## 12. Status Display (`aot_cli/status.py`)
...
## 13. Gateway Setup Wizard (`aot_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ aot_cli/ toolsets.py \
  --include="*.py" -l | sort -u
