...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_aot_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.aot/config.yaml (primary user-facing config)
    3. ~/.aot/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_aot_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``aot config set gateway.streaming.*``
