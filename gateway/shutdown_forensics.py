...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if aot_home_str:
            takeover_path = Path(aot_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(aot_home_str) / ".gateway-planned-stop.json"
...
def spawn_async_diagnostic(
    ...
    *,
    ...
    try:
        ...
        if timeout_cmd:
            cmd = [timeout_cmd, f"{timeout_seconds:.0f}", "bash", "-c", script]
        else:
            cmd = ["bash", "-c", script]
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded aot-agent but never re-ran ``aot setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../aot-gateway.service"
    ...
    # that's the common case for aot.
    ...
    # constant in aot_cli/gateway.py.
    headroom = 30.0
