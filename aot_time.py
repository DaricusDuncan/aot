"""
Timezone-aware clock for Aot.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``AOT_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.aot/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Aot never
...
from aot_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Aot requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("AOT_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
