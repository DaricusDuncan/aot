"""CLI subcommand: ``aot send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"aot send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"aot send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"aot send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"aot send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"aot send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `aot gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.aot/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_aot_env() -> None:
    """Populate ``os.environ`` from ``~/.aot/.env`` AND bridge top-level
    ...
    startup that ``aot send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.aot/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.aot/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``aot send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.aot/.env and ~/.aot/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "aot send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  aot send --to telegram \"hello\"\n"
            "  aot send --to discord:#ops --file report.md\n"
            "  aot send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "aot send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `aot send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Aot "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.aot/.env + ~/.aot/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  aot send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  aot send --list                  # all platforms\n"
            "  aot send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `aot send --list telegram`.",
    )
