"""aot hooks — inspect and manage shell-script hooks.

Usage::

    aot hooks list
    aot hooks test <event> [--for-tool X] [--payload-file F]
    aot hooks revoke <command>
    aot hooks doctor

Consent records live under ``~/.aot/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.aot/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``aot hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: aot hooks {list|test|revoke|doctor}")
        print("Run 'aot hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.aot/config.yaml.")
        print("See `aot hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `aot hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `aot hooks test` and `aot hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Aot wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `aot hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `aot hooks doctor` would execute
