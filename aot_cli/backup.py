"""
Backup and import commands for aot CLI.

`aot backup` creates a zip archive of the entire ~/.aot/ directory
(excluding the aot-agent repo and transient files).

`aot import` restores from a backup zip, overlaying onto the current
AOT_HOME root.
"""
...
from aot_constants import get_default_aot_root, get_aot_home, display_aot_home
...
_EXCLUDED_DIRS = {
    "aot-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to aot root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Aot home directory."""
    aot_root = get_default_aot_root()

    if not aot_root.is_dir():
        print(f"Error: Aot home directory not found at {aot_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"aot-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"aot-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_aot_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(aot_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(aot_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(aot_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside aot root
            try:
    ...
    print(f"\nRestore with: aot import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Aot backup.
    ...
    # Look for telltale files that a aot home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Aot backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.aot/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a aot dir name
        if prefix in {".aot", "aot"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_aot_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (aot_root / "config.yaml").exists()
        has_env = (aot_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Aot configuration.")
        ...
        for member in members:
            ...
            try:
                target.resolve().relative_to(aot_root.resolve())
            except ValueError:
        ...
        profiles_dir = aot_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # aot_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: aot profile list  (after installing aot)")

        # Guidance
        print()
        if not (aot_root / "aot-agent").is_dir():
            print("Note: The aot-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: aot update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  aot -p {pname} gateway install")

        print("Done. Your Aot configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and aot backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to AOT_HOME).
...
# — `aot update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(aot_home: Optional[Path] = None) -> Path:
    home = aot_home or get_aot_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    aot_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(aot_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(aot_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for aot backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_aot_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, aot_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``aot_root`` to ``out_path``.
...
def _pre_update_backup_dir(aot_home: Optional[Path] = None) -> Path:
    home = aot_home or get_aot_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of AOT_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<AOT_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``aot update``) should continue even if the backup fails.
    """
    aot_root = aot_home or get_default_aot_root()
    if not aot_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(aot_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, aot_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `aot claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of AOT_HOME under ``backups/`` before a
    ``aot claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``aot import <archive>``.  Writes to
    ``<AOT_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `aot import` and the
