"""aot webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.aot/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _aot_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: aot gateway run
"""
...
def webhook_command(args):
    """Entry point for 'aot webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: aot webhook {subscribe|list|remove|test}")
        print("Run 'aot webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (aot gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: aot webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from aot webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (aot gateway run)")
