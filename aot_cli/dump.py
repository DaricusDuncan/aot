"""
Dump command for aot CLI.

Outputs a compact, plain-text summary of the user's Aot setup
...
from aot_cli.config import get_aot_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``aot dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from aot_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(aot_home: Path) -> int:
...
def _cron_summary(aot_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = aot_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"aot_home:      {display_aot_home()}")
    ...
    toolsets = config.get("toolsets", ["aot-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(aot_home)}")
    lines.append(f"  skills:             {_count_skills(aot_home)}")
