"""
Gateway subcommand for aot CLI.

Handles: aot gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_aot_home is imported lazily at call sites to avoid ImportError
# when aot_constants is cached from a pre-update version during `aot update`.
from aot_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from aot_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "aot-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "aot CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``aot gateway status`` from falsely counting the ``aot`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``aot gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "aot_cli.main gateway",
        "aot_cli.main --profile",
        "aot_cli.main -p",
        "aot_cli/main.py gateway",
        "aot_cli/main.py --profile",
        "aot_cli/main.py -p",
        "aot gateway",
        "gateway/run.py",
    ]
    current_home = str(get_aot_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"AOT_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. AOT_HOME
        ...
        if "--profile " in command or " -p " in command:
            return False
        if "AOT_HOME=" in command and f"AOT_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``aot update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Aot profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Aot profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from aot_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "aot_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``aot update`` users
    # with no running gateway until they re-invoke ``aot gateway``
...
def _sync_aot_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``AOT_HOME``.

    Under ``sudo``, ``AOT_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_aot_home` falls back to ``/root/.aot`` — the wrong
    profile. The unit file pins ``AOT_HOME`` for the actual gateway
    ...
    unit_home = env.get("AOT_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("AOT_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["AOT_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}aot gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}aot gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from aot_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `aot gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``aot gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from aot_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (AOT_HOME-scoped).
...
from aot_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``aot gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``AOT_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("AOT_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "aot-gateway"
SERVICE_DESCRIPTION = "Aot Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current AOT_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (AOT_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from aot_constants import get_default_aot_root
    home = get_aot_home().resolve()
    default = get_default_aot_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary AOT_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(aot_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when AOT_HOME is a named profile.

    For ``~/.aot/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        aot_home: Optional explicit AOT_HOME path. Defaults to the current
            ``get_aot_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from aot_constants import get_default_aot_root
    home = Path(aot_home or str(get_aot_home())).resolve()
    default = get_default_aot_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this AOT_HOME.

    Default ``~/.aot`` returns ``aot-gateway`` (backward compatible).
    Profile ``~/.aot/profiles/coder`` returns ``aot-gateway-coder``.
    Any other AOT_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Aot installs that predate the
# aot-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (aot-gateway-*.service) and unrelated third-party
# "aot" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("aot.service",)
...
def _find_legacy_aot_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Aot gateway units.

    Detects unit files installed by older Aot versions that used a
    different service name (e.g. ``aot.service`` before the rename to
    ``aot-gateway.service``). When both a legacy unit and the current
    ``aot-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``aot-gateway-coder.service`` and unrelated third-party
      ``aot-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``aot.service`` running an unrelated
...
def has_legacy_aot_units() -> bool:
    """Return True when any legacy Aot gateway unit files exist."""
    return bool(_find_legacy_aot_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Aot gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_aot_units()
    if not legacy:
        return
    print_warning("Legacy Aot gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current aot-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    aot gateway migrate-legacy")


def remove_legacy_aot_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Aot gateway unit files.

    Iterates over whatever ``_find_legacy_aot_units()`` returns — which is
    ...
    legacy = _find_legacy_aot_units()
    if not legacy:
        print("No legacy Aot gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Aot gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: aot gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo aot gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    aot gateway uninstall")
    print_info("    sudo aot gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Aot can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo aot gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo aot gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo aot gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Aot often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.aot`` → ``ai.aot.gateway.plist`` (backward compatible).
    Profile ``~/.aot/profiles/coder`` → ``ai.aot.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.aot.gateway-{suffix}" if suffix else "ai.aot.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.aot/aot-agent  -> /home/alice/.aot/aot-agent
      /opt/aot                 -> /opt/aot  (kept as-is)
...
def _aot_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current AOT_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_aot_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.aot                    → /home/alice/.aot
      /root/.aot/profiles/coder     → /home/alice/.aot/profiles/coder
      /opt/custom-aot               → /opt/custom-aot  (kept as-is)
    """
    current_aot = get_aot_home().resolve()
    current_default = (Path.home() / ".aot").resolve()
    target_default = Path(target_home_dir) / ".aot"

    # Default ~/.aot → remap to target user's default
    if current_aot == current_default:
        return str(target_default)

    # Profile or subdir of ~/.aot → preserve the relative structure
    try:
        relative = current_aot.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.aot) — keep as-is
        return str(current_aot)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    aot_home = get_aot_home()
    aot_node = aot_home / "node" / "bin"
    if _is_dir(aot_node):
        candidates.append(str(aot_node))
    aot_nm = aot_home / "node_modules" / ".bin"
    if _is_dir(aot_nm):
        candidates.append(str(aot_nm))
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        aot_home = _aot_home_for_target_user(home_dir)
        profile_arg = _profile_arg(aot_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m aot_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
"""
    ...
    aot_home = str(get_aot_home().resolve())
    profile_arg = _profile_arg(aot_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only AOT_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp AOT_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../aot_test`` path into
    # ``Environment="AOT_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Aot install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo aot gateway uninstall --system")
    print_info("         aot gateway install")
    print_info("         aot gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (aot.service from pre-rename installs)
    # before installing the new aot-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}aot gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}aot gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}aot gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_aot_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `aot gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `aot gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}aot gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}aot gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}aot gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}aot gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.aot.gateway-{suffix}" if suffix else "ai.aot.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Aot install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  aot gateway status             # Check status")
    from aot_constants import display_aot_home as _dhh
...
def launchd_stop():
    ...
    # `aot gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Aot install")
    else:
        print("⚠ Service definition is stale relative to the current Aot install")
        print("  Run: aot gateway start")
    ...
    if deep:
        log_file = get_aot_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("AOT_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Aot gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'aot' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/aot/docker/entrypoint.sh before the Aot command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$AOT_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set AOT_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `aot gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set AOT_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `aot gateway restart` which already
    ...
    # `aot gateway start/restart` — leaving the gateway vulnerable to
    ...
    # AOT_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("AOT_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Aot will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @aot:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. aot) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Aot delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Aot agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Aot will use (e.g., aot@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Aot connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: aot pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Aot will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``aot setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run aot under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.aot/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_aot_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'aot pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: aot pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'aot gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from aot_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from aot_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from aot_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Aot will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Aot will store the returned account_id/token in ~/.aot/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `aot gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_aot_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `aot pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `aot pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"AotAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``aot_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from aot_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.aot/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.aot/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: aot gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: aot gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: aot gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo aot gateway install --system")
                    print_info("  Or run in foreground:  aot gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: aot gateway run")
                print_info("  For persistence:   tmux new -s aot 'aot gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from aot_constants import display_aot_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: aot gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup aot gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: aot gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'aot gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``aot gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: aot gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
            ...
            print()
            print("  aot gateway run                              # direct foreground")
            print("  tmux new -s aot 'aot gateway run'         # persistent via tmux")
            print("  nohup aot gateway run > ~/.aot/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: aot gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: aot gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: aot gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: aot gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: aot gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  aot gateway install")
                else:
                    print("To install as a service:")
                    print("  aot gateway install")
                    print("  sudo aot gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  aot gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup aot gateway run > ~/.aot/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s aot 'aot gateway run'         # persistent via tmux")
                    print("  nohup aot gateway run > ~/.aot/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  aot gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  aot gateway install  # Install as user service")
                    print("  sudo aot gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Aot gateway unit files from
        # pre-rename installs (e.g. aot.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_aot_units(interactive=not yes, dry_run=dry_run)
