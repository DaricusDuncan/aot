"""
MCP Server Management CLI — ``aot mcp`` subcommand.

Implements ``aot mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.aot/config.yaml under the ``mcp_servers`` key.
"""
...
from aot_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_aot_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from aot_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # aot_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  aot mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  aot mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  aot mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_aot_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: aot mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_aot_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `aot mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  aot mcp add <name> --url <endpoint>')
        _info('  aot mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `aot mcp remove` + `aot mcp add` to reconfigure auth.")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'aot mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``aot mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("aot mcp serve                              Run as MCP server")
        _info("aot mcp add <name> --url <endpoint>        Add an MCP server")
        _info("aot mcp add <name> --command <cmd>         Add a stdio server")
        _info("aot mcp add <name> --preset <preset>       Add from a known preset")
        _info("aot mcp remove <name>                      Remove a server")
        _info("aot mcp list                               List servers")
        _info("aot mcp test <name>                        Test connection")
        _info("aot mcp configure <name>                   Toggle tools")
        _info("aot mcp login <name>                       Re-authenticate OAuth")
        print()
