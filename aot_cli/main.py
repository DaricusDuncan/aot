#!/usr/bin/env python3
"""
Aot CLI - Main entry point.

Usage:
    aot                     # Interactive chat (default)
    aot chat                # Interactive chat
    aot gateway             # Run gateway in foreground
    aot gateway start       # Start gateway as service
    aot gateway stop        # Stop gateway service
    aot gateway status      # Show gateway status
    aot gateway install     # Install gateway service
    aot gateway uninstall   # Uninstall gateway service
    aot setup               # Interactive setup wizard
    aot logout              # Clear stored authentication
    aot status              # Show status of all components
    aot cron                # Manage cron jobs
    aot cron list           # List cron jobs
    aot cron status         # Check if cron scheduler is running
    aot doctor              # Check configuration and dependencies
    aot honcho setup                    # Configure Honcho AI memory integration
    aot honcho status                   # Show Honcho config and connection status
    aot honcho sessions                 # List directory → session name mappings
    aot honcho map <name>               # Map current directory to a session name
    aot honcho peer                     # Show peer names and dialectic settings
    aot honcho peer --user NAME         # Set user peer name
    aot honcho peer --ai NAME           # Set AI peer name
    aot honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    aot honcho mode                     # Show current memory mode
    aot honcho mode [hybrid|honcho|local]  # Set memory mode
    aot honcho tokens                   # Show token budget settings
    aot honcho tokens --context N       # Set session.context() token cap
    aot honcho tokens --dialectic N     # Set dialectic result char cap
    aot honcho identity                 # Show AI peer identity representation
    aot honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    aot honcho migrate                  # Step-by-step migration guide: OpenClaw native → Aot + Honcho
    aot version             Show version
    aot update              Update to latest version
    aot uninstall           Uninstall Aot Agent
    aot acp                 Run as an ACP server for editor integration
    aot sessions browse     Interactive session picker with search

    aot claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: aot_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``aot_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``aot update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``aot_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, aot crashes on import and the user can't run
# ``aot update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import aot_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to AOT_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (aot tools, aot setup, aot model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'aot {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any aot module import.
#
# Many modules cache AOT_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("AOT_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.aot/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set AOT_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors aot_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If AOT_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.aot/profiles/coder or
    # /opt/data/profiles/coder).  If AOT_HOME points to the aot root
    # instead (e.g. systemd hardcodes AOT_HOME=/root/.aot), we must
    # still read active_profile — the user may have switched profiles via
    # `aot profile use` and the gateway should honour that choice.
    # See issue #22502.
    aot_home_env = os.environ.get("AOT_HOME", "")
    if profile_name is None and aot_home_env:
        if Path(aot_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the aot root
    if profile_name is None:
        try:
            from aot_constants import get_default_aot_root

            active_path = get_default_aot_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set AOT_HOME
    if profile_name is not None:
        try:
            from aot_cli.profiles import resolve_profile_env

            aot_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent aot from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["AOT_HOME"] = aot_home
...
# Load .env from ~/.aot/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from aot_cli.config import get_aot_home
from aot_cli.env_loader import load_aot_dotenv

load_aot_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → AOT_REDACT_SECRETS env
# var BEFORE aot_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "AOT_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_aot_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["AOT_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `aot` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from aot_logging import setup_logging as _setup_logging
...
try:
    from aot_cli.config import load_config as _load_config_early
    from aot_constants import apply_ipv4_preference as _apply_ipv4
...
from aot_cli import __version__, __release_date__
from aot_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from aot_cli.config import get_env_path, get_aot_home, load_config
    from aot_cli.auth import get_auth_status

    # Determine whether Aot itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from aot_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_aot_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from aot_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_aot_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Aot has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Aot to use their tokens.
    if _has_aot_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from aot_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, aot_bin
        cli_args: the original CLI arguments (everything after 'aot')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    aot_bin = container_info["aot_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo aot {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo aot {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  aot --tui --resume {target}")
    if title:
        print(f'  aot --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @aot/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@aot" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``AOT_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("AOT_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    aot_home = os.environ.get("AOT_HOME") or str(Path.home() / ".aot")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "AOT_HOME": aot_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(aot_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(aot_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if aot_cli_dir is None:
        aot_cli_dir = Path(__file__).parent
    bundled = aot_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (AOT_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("AOT_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from aot_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("AOT_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with AOT_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset AOT_TUI_DIR (e.g. `unset AOT_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("AOT_QUIET"):
    ...
    if tui_dev:
        # Keep the local @aot/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @aot/ink resolves through
        # packages/aot-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "aot-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from aot_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="aot-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["AOT_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["AOT_PYTHON_SRC_ROOT"] = os.environ.get(
        "AOT_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("AOT_PYTHON", sys.executable)
    env.setdefault("AOT_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["AOT_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["AOT_MODEL"] = model
        env["AOT_INFERENCE_MODEL"] = model
    if provider:
        env["AOT_TUI_PROVIDER"] = provider
        env["AOT_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["AOT_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["AOT_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["AOT_TUI_SKILLS"] = value
    if query:
        env["AOT_TUI_QUERY"] = query
    if image:
        env["AOT_TUI_IMAGE"] = image
    if checkpoints:
        env["AOT_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["AOT_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["AOT_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["AOT_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["AOT_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["AOT_ACCEPT_HOOKS"] = "1"
    ...
    # AOT_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `aot --tui` try to
    ...
    # still set AOT_TUI_RESUME themselves.
    env.pop("AOT_TUI_RESUME", None)
    if resume_session_id:
        env["AOT_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `aot update` so
    ...
    if code == 42:
        from aot_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``AOT_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``aot kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``aot kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("AOT_KANBAN_BOARD"):
        return
    try:
        from aot_cli.kanban_db import get_current_board

        os.environ["AOT_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("AOT_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'aot sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Aot isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  aot setup")
        print()

        from aot_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'aot setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from aot_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["AOT_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.aot/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["AOT_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["AOT_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["AOT_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from aot_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from aot_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from aot_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Aot?")
        print()
    ...
    # has no creds.json.  Every subsequent `aot gateway` then paid a 30s
    ...
    session_dir = get_aot_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: aot gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `aot gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  aot gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Aot Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  aot gateway")
        ...
        print()
        print("  Or install as a service: aot gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'aot whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from aot_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from aot_cli.config import stamp_install_method
    from aot_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Aot post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``aot model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from aot_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from aot_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from aot_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("AOT_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'aot model' for "
                "available providers, or run 'aot doctor' to diagnose config "
                "issues."
            )
    ...
    from aot_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from aot_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.aot/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.aot/.env if the active provider is not 'custom'.
    ...
    """
    from aot_cli.config import get_env_value, save_env_value, load_config
...
#
# Aot uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `aot model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `aot model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from aot_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Aot only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``aot model`` flow, then route aux tasks to them here.
    """
    from aot_cli.config import load_config
    from aot_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from aot_cli.auth import _prompt_model_selection
    from aot_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from aot_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from aot_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.aot/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `aot setup` from scratch. OpenRouter
    ...
    from aot_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.aot/.env by hand.
    ...
    from aot_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from aot_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from aot_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from aot_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `aot auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from aot_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from aot_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``aot model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``aot model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``aot auth add xai-oauth``).  Fall
...
def _model_flow_custom(config):
    ...
    """
    from aot_cli.auth import _save_model_choice, deactivate_provider
    from aot_cli.config import get_env_value, load_config, save_config
    ...
    from aot_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `aot model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from aot_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Aot URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from aot_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from aot_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from aot_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Aot will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Aot will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from aot_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from aot_cli.config import save_env_value, load_config, save_config
    from aot_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from aot_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from aot_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Aot turns to `copilot --acp`.")
    print("  Aot currently starts its own ACP subprocess for each request.")
    print("  Aot uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set AOT_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``aot setup`` / ``aot model``.
    ...
    recover from a malformed paste without editing ``~/.aot/.env`` by hand.
    ...
    from aot_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from aot_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `aot setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so aot knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Aot typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Aot, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from aot_cli.models import fetch_lmstudio_models
        ...
        from aot_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from aot_constants import display_aot_home as _dhh_fn

            print(
                f"    Aot will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from aot_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'aot model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Aot CLI with a provider."""
    from aot_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from aot_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from aot_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from aot_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from aot_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from aot_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``aot slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: aot slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `aot slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from aot_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from aot_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from aot_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from aot_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from aot_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from aot_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from aot_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Aot home directory to a zip file."""
    if getattr(args, "quick", False):
        from aot_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from aot_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Aot backup from a zip file."""
    from aot_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Aot Agent v{__version__} ({__release_date__})")
    ...
    try:
        from aot_cli.banner import check_for_updates
        from aot_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from aot_cli.uninstall import run_uninstall
...
# Critical files that every ``aot`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``aot update`` again to roll forward. The post-pull syntax
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``aot`` startup; if any of them
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``aot update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``aot_cli/web_dist/`` (per vite.config.ts
    outDir: "../aot_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "aot_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``aot update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``aot web``).
    ...
    # build path usable on a stock `py -m aot_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (aot web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``aot dashboard`` processes other than ourselves.

    ``aot dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``aot update`` replaces files on disk, the running
    ...
    patterns = [
        "aot dashboard",
        "aot_cli.main dashboard",
        "aot_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "aot.*dashboard"`) keeps us consistent
            # with `aot_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `aot update`.
    ...
    print("  Preview now:  aot curator run --dry-run")
    print("  Pause it:     aot curator pause")
    print(
        "  Docs:         https://aot-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``aot update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``aot update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: aot curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``aot dashboard`` processes.

    Called at the end of ``aot update`` (default ``reason``) and also
    from ``aot dashboard --stop`` (which overrides ``reason``).  The
...
def _update_via_zip(args):
    """Update Aot Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/aot-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="aot-update-")
        zip_path = os.path.join(tmp_dir, f"aot-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to aot-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"aot-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "aot-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Aot behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes aot completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Aot couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Aot couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Aot behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `aot update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/aot-agent.git",
    "git@github.com:NousResearch/aot-agent.git",
    "https://github.com/NousResearch/aot-agent",
    "git@github.com:NousResearch/aot-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/aot-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_aot_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_aot_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Aot repository.")
        print("  This means you may miss updates from NousResearch/aot-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/aot-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/aot-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``aot update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from aot_constants import get_default_aot_root

    default_home = get_default_aot_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``aot update`` is still progressing even if pip/uv itself is silent.
    """
...
def _aot_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    return [
        scripts_dir / "aot.exe",
        scripts_dir / "aot-gateway.exe",
    ]


def _detect_concurrent_aot_instances(
    ...
    The Aot Desktop Electron app spawns ``aot.EXE`` as a backend child,
    so during ``aot update`` the user-invoked process and the desktop's
    ...
    shims (``aot.exe`` / ``aot-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID is excluded so the running
    ``aot update`` invocation never reports itself.
    ...
    for shim in _aot_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "aot.exe"
    lines = ["✗ Another aot.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Aot Desktop, exit any open `aot` REPLs, and")
    lines.append("  stop the gateway (`aot gateway stop`) before retrying.")
    lines.append("  Override with `aot update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_aot_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``aot.exe``.
    ...
    when ``aot update`` runs, ``aot.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``aot.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next aot invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Aot Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Aot Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _aot_exe_shims(scripts_dir):
        ...
        print(
            "    Close Aot Desktop, exit other `aot` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `aot update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_aot_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``aot.exe.old.*`` left by prior updates.

    Called early on every aot invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``aot update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `aot update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``aot.exe`` / ``aot-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_aot_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_aot_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.aot/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``aot update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.aot/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``aot update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``aot update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.aot/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``aot update --gateway``) the update is already
    ...
    try:
        ...
        # aot_cli.config.get_aot_home to simulate setup failure.
        from aot_cli.config import get_aot_home as _get_aot_home

        logs_dir = _get_aot_home() / "logs"
...
def _cmd_update_check():
    """Implement ``aot update --check``: fetch and report without installing."""
    from aot_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from aot_cli.config import recommended_update_command
        from aot_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``aot update`` without requiring a reinstall.  The
    ...
    ``aot`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/aot.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v aot'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/aot, code at /usr/local/lib/aot-agent).
    fhs_link = Path("/usr/local/bin/aot")
    ...
    path_comment = (
        "# Aot Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of AOT_HOME before running the update.
    ...
    AOT_HOME directories.  The ``--backup`` flag on ``aot update``
    ...
    try:
        from aot_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_aot_home so the user sees ~/.aot/...
    try:
        from aot_constants import get_aot_home, display_aot_home

        home = get_aot_home()
        try:
            display_path = f"{display_aot_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  aot import {out_path}")
...
def cmd_update(args):
    """Update Aot Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from aot_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Aot Agent")
        return
...
def _cmd_update_pip(args):
    """Update Aot via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "aot-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "aot-agent"]
    ...
    print("✓ Update complete! Restart aot to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Aot Agent...")
    print()

    # On Windows, abort early if another aot.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_aot_instances(scripts_dir)
            if concurrent:
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/aot-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # orphan merge-conflict markers in aot_cli/config.py bricked
        # every user who ran ``aot update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``aot
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``aot update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_aot_home added to aot_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload aot_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_aot_home() added since the last release.
        try:
            import importlib
            import aot_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit AOT_HOME so
        # it is not affected by sync_skills()'s module-level AOT_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's AOT_HOME env var points at the default or a named profile.
        try:
            from aot_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: aot config migrate")
            else:
                print()
                print("Skipped. Run 'aot config migrate' later to configure.")
        else:
        ...
        # check `aot curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``aot update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from aot_cli.tools_config import install_cua_driver
        ...
        # When running as ``aot update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_aot_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all aot-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. aot-gateway.service or aot-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: aot gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: aot -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Aot gateway unit files are still installed.
        # When both aot.service (from a pre-rename install) and the
        # current aot-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `aot update` surfaces the issue until the user migrates.
        try:
            from aot_cli.gateway import (
                has_legacy_aot_units,
                _find_legacy_aot_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_aot_units():
                print()
                print("⚠ Legacy Aot gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_aot_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (aot.service) fight the current")
                print("  aot-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    aot gateway migrate-legacy")
                print()
        ...
        print("  aot model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``aot -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from aot_constants import display_aot_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `aot profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_aot_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → aot -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.aot)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  aot profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     aot -p {name} chat")
                else:
        ...
        from aot_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from aot_constants import get_aot_home as _hh
        ...
        from aot_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `aot profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from aot_cli.profiles import profile_exists
        ...
        from aot_cli.profiles import rename_profile
        ...
        from aot_cli.profiles import export_profile
        ...
        from aot_cli.profiles import import_profile
        ...
        import tempfile
        from aot_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="aot_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  aot -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      aot -p {plan.manifest.name} chat")
        ...
        from aot_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `aot profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"aot -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from aot_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("aot_requires"):
            print(f"Requires:     Aot {data['aot_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from aot_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.aot_requires:
        print(f"  Requires: Aot {mf.aot_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``aot dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``aot dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No aot dashboard processes running.")
        return 0

    print(f"{len(pids)} aot dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No aot dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `aot update`.
    ...
    if "AOT_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["AOT_WEB_DIST"])
            if "AOT_WEB_DIST" in os.environ
            else PROJECT_ROOT / "aot_cli" / "web_dist"
        )
    ...
    from aot_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("AOT_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from aot_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Aot log files."""
    from aot_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from aot_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``aot -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``aot_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``aot -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``aot`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for aot CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from aot_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``aot.exe.old.*`` quarantine files left by previous
    # ``aot update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_aot_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from aot_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: aot-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from aot_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://aot-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `aot model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy aot.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Aot gateway unit files "
            "(e.g. aot.service) left over from older installs. Profile "
            "units (aot-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `aot proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Aot Agent with an interactive wizard. "
        "Run a specific section: aot setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'aot setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Aot.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $AOT_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Aot")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from aot_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Aot CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: aot-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Aot with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set AOT_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Aot Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.aot/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Aot profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.aot/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Aot profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from aot_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.aot/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.aot/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Aot Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `aot "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Aot setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Aot Agent. Use 'aot debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    aot debug share              Upload debug report and print URL
    aot debug share --lines 500  Include more log lines
    aot debug share --expire 30  Keep paste for 30 days
    aot debug share --local      Print report locally (no upload)
    aot debug share --no-redact  Disable upload-time secret redaction
    aot debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'aot debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Aot home directory to a zip file",
        description="Create a zip archive of your entire Aot configuration, "
        "skills, sessions, and data (excludes the aot-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/aot-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.aot/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo aot uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from aot_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Aot backup from a zip file",
        description="Extract a previously created Aot backup into your "
        "Aot home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Aot Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from aot_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.aot/skills/.bundled_manifest) "
            "so future 'aot update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from aot_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from aot_cli.skills_hub import skills_command
    ...
    from aot_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/aot-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `aot plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from aot_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``aot --help``, ``aot version``, ``aot logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from aot_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from aot_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_aot_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_aot_home()}/memories/\n")
        else:
            from aot_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'aot tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # aot tools disable <name...> [--platform cli]
    ...
    # aot tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from aot_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from aot_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `aot computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `aot tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: aot computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: aot computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Aot as an MCP server",
        description=(
            "Manage MCP server connections and run Aot as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'aot mcp add' to connect to a new server, or\n"
            "'aot mcp serve' to expose Aot conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Aot as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `aot mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from aot_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch aot --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Aot",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Aot",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.aot/ (by default a "
        "single restore-point archive is written to ~/.aot/backups/ "
        "before apply; restorable with 'aot import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from aot_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Aot Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'aot config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--force",
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another aot.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Aot Agent",
        description="Remove Aot Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Aot Agent as an ACP (Agent Client Protocol) server",
        description="Start Aot Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Aot ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Aot provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.aot/node/ "
    ...
    def cmd_acp(args):
        """Launch Aot Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Aot instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `aot update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `aot profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Aot profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Aot Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `aot --tui` via PTY/WebSocket). "
            "Alternatively set AOT_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `aot dashboard` cmdlines and SIGTERM them directly — the same
    # path `aot update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running aot dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running aot dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Aot log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    aot logs                    Show last 50 lines of agent.log
    aot logs -f                 Follow agent.log in real time
    aot logs errors             Show last 50 lines of errors.log
    aot logs gateway -n 100     Show last 100 lines of gateway.log
    aot logs --level WARNING    Only show WARNING and above
    aot logs --session abc123   Filter by session ID
    aot logs --component tools  Only show tool-related lines
    aot logs --since 1h         Lines from the last hour
    aot logs --since 30m -f     Follow, starting from 30 min ago
    aot logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``aot -c Pokemon Agent Dev`` → ``aot -c 'Pokemon Agent Dev'``
    ...
    from aot_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'aot -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (aot hooks list, cron
    ...
    if getattr(args, "oneshot", None):
        from aot_cli.oneshot import run_oneshot
