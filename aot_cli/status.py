"""
Status command for aot CLI.

Shows the status of all Aot Agent components.
"""
...
from aot_cli.auth import AuthError, resolve_provider
from aot_cli.colors import Colors, color
from aot_cli.config import get_env_path, get_env_value, get_aot_home, load_config
from aot_cli.models import provider_label
from aot_cli.nous_subscription import get_nous_subscription_features
from aot_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``aot config``'s
...
def show_status(args):
    """Show status of all Aot Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Aot Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: aot auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: aot model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: aot auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: aot auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: aot model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from aot_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'aot-agent[vercel]')"
    ...
    try:
        from aot_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_aot_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_aot_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'aot doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'aot setup' to configure", Colors.DIM))
    print()
