"""
Single source of truth for provider identity in Aot Agent.
...
2. **Aot overlays** — transport type, auth patterns, aggregator flags,
...
# -- Aot overlay ----------------------------------------------------------
# Aot-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class AotOverlay:
    """Aot-specific provider metadata layered on top of models.dev."""
...
AOT_OVERLAYS: Dict[str, AotOverlay] = {
    "openrouter": AotOverlay(
    ...
    "openai-codex": AotOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": AotOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": AotOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="AOT_QWEN_BASE_URL",
    ),
    "google-gemini-cli": AotOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
    ...
    "copilot-acp": AotOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": AotOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": AotOverlay(
    ...
    "alibaba-coding-plan": AotOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "aot", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Aot overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Aot overlay
    ...
    overlay = AOT_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + aot extra
    ...
    if overlay is not None:
        # Aot-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in AOT_OVERLAYS
    if provider == "bedrock":
