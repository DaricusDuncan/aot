"""`aot checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.aot/checkpoints/``.  Actions:

    aot checkpoints               # same as `status`
    aot checkpoints status        # total size, project count, breakdown
    aot checkpoints list          # per-project checkpoint counts + workdir
    aot checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    aot checkpoints clear [-f]    # nuke the entire base (asks first)
    aot checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    aot checkpoints
    aot checkpoints prune --retention-days 3 --max-size-mb 200
    aot checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: aot checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``aot checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `aot checkpoints` → status
