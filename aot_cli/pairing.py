...
Usage:
    aot pairing list              # Show all pending + approved users
    aot pairing approve <platform> <code>  # Approve a pairing code
    aot pairing revoke <platform> <user_id> # Revoke user access
    aot pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle aot pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: aot pairing {list|approve|revoke|clear-pending}")
        print("Run 'aot pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.aot/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'aot pairing list' to see pending codes.\n")
