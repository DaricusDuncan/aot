...
``<root>`` is the **shared Aot root** (the parent of any active
...
``aot -p <profile>`` joins the same board as the dispatcher that
...
the profile's Aot home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``AOT_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``AOT_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``aot kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.aot``. In Docker / custom
deployments where ``AOT_HOME`` points outside ``~/.aot`` (e.g.
``/opt/aot``), ``<root>`` is ``AOT_HOME``. Legacy env-var
overrides still work:

* ``AOT_KANBAN_DB`` — pin the database file path directly.
* ``AOT_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``AOT_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``AOT_KANBAN_DB``,
``AOT_KANBAN_WORKSPACES_ROOT``, and ``AOT_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/aot-kanban-v1-spec.pdf`` for the full
...
# ``AOT_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``AOT_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("AOT_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
# enough that kebab-case names like ``atm10-server`` or ``aot-agent``
...
def kanban_home() -> Path:
    """Return the shared Aot root that anchors the kanban board.

    Resolution order:

    1. ``AOT_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_aot_root()``, which already returns ``<root>``
       when ``AOT_HOME`` is ``<root>/profiles/<name>``, and returns
       ``AOT_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``AOT_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("AOT_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from aot_constants import get_default_aot_root
    return get_default_aot_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``aot kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``AOT_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``aot kanban boards
    ...
    """
    env = os.environ.get("AOT_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``aot kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``AOT_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("AOT_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``AOT_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("AOT_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``aot kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``AOT_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``AOT_SESSION_ID``. NULL
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``AOT_KANBAN_DB`` env → ``AOT_KANBAN_BOARD`` env →
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # See aot_state._WAL_INCOMPAT_MARKERS for detection logic.
            from aot_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``aot kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``AOT_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from aot_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``aot --skills ...`` alongside the built-in
    ...
    # `aot --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``aot kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `aot kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``aot kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``aot kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``aot kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Aot
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``aot kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Aot profile.
    ...
    if not rows:
        return False
    try:
        from aot_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Aot profile.
        # `_default_spawn` invokes ``aot -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Aot
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `aot kanban tail` — without
...
def _module_aot_argv() -> list[str]:
    """Return the interpreter-bound Aot CLI invocation."""
    # ``aot_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``aot`` package — there is no
    # ``aot`` package to import.
    return [sys.executable, "-m", "aot_cli.main"]


def _absolute_aot_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Aot shim."""
...
def _aot_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Aot executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_aot_argv()
    return [_absolute_aot_path(path)]


def _resolve_aot_argv() -> list[str]:
    """Resolve the ``aot`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$AOT_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("aot")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\aot.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m aot_cli.main`` — fallback for setups where
       Aot is launched from a venv and the ``aot`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_aot_bin`` for the same reason. Kept
    local (not imported from gateway) because ``aot_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("AOT_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _aot_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _aot_path_argv(resolved_env_bin)
        return _module_aot_argv()

    aot_bin = _safe_which_no_cwd("aot") if _IS_WINDOWS else shutil.which("aot")
    if aot_bin:
        return _aot_path_argv(aot_bin)
    return _module_aot_argv()


def _kanban_worker_skill_available(aot_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``aot -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset AOT_HOME means the worker falls back to the default root
    # home (``~/.aot``), which ships the bundled skill.
    base = _Path(aot_home) if aot_home else (_Path.home() / ".aot")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``aot -p <profile> chat -q ...`` subprocess.
    ...
    ``AOT_KANBAN_DB`` / ``AOT_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject AOT_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `aot -p <name>` the
    # _apply_profile_override() runs *before* aot_constants is imported.
    # If AOT_HOME is absent from the child's env, get_aot_home() falls
    # back to Path.home() / ".aot" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from aot_cli.profiles import resolve_profile_env
    try:
        env["AOT_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via AOT_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # AOT_HOME never had profiles created.
        pass
    if task.tenant:
        env["AOT_TENANT"] = task.tenant
    env["AOT_KANBAN_TASK"] = task.id
    env["AOT_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["AOT_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["AOT_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["AOT_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`aot -p <name>`
    # rewrites AOT_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_aot_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["AOT_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["AOT_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["AOT_KANBAN_BOARD"] = resolved_board
    # AOT_PROFILE is the author the kanban_comment tool defaults to.
    # `aot -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["AOT_PROFILE"] = profile_arg

    cmd = [
        *_resolve_aot_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped AOT_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("AOT_HOME")):
    ...
    # `aot kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`aot` executable not found on PATH. "
            "Install Aot Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``aot kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled AOT_PROFILE values like "aot-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Aot root exists

    Reads profile paths directly so this module has no import dependency on
    ``aot_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from aot_constants import get_default_aot_root
        default_root = get_default_aot_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``aot kanban assignees`` for the terminal.
