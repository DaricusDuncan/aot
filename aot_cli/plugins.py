...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with aot-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.aot/plugins/<name>/``
3. **Project plugins** – ``./.aot/plugins/<name>/`` (opt-in via
   ``AOT_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``aot_agent.plugins``
   entry-point group.
...
# ---------------------------------------------------------------------------
#
# Set ``AOT_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.aot/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("AOT_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When AOT_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Aot loggers.
    """
    ...
    logger.debug(
        "AOT_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "aot_agent.plugins"

_NS_PARENT = "aot_plugins"
...
class PluginManifest:
    ...
    #              in ~/.aot/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``aot plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``aot honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``aot <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.aot/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to aot_cli/. Two layouts are
        ...
        # 2. User plugins (~/.aot/plugins/)
        user_dir = get_aot_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.aot/plugins/)
        if _env_enabled("AOT_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".aot" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set AOT_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with aot and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Aot ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `aot plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``aot_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``aot_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``aot tools`` TUI so plugin-provided toolsets appear
