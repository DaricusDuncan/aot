...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Aot'
...
The actual runtime resolution happens in aot_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "aot"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Aot' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Aot for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from aot_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the aot-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "aot-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Aot tool callback are always-on production
            ...
            if "aot-tools" in mig_report.migrated:
                msg_lines.append(
                    "Aot tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Aot runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Aot tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Aot runtime.")
