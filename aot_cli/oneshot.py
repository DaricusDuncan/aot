...
configured for "cli" in `aot tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (AOT_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `aot chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    - AOT_INFERENCE_MODEL
    - AOT_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"aot -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "aot -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"aot -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "aot -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "aot -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to AOT_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            AOT_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    env_model_early = os.getenv("AOT_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "aot -z: --provider requires --model (or AOT_INFERENCE_MODEL). "
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``aot -z`` / oneshot mode.

    Oneshot bypasses ``AotCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when aot is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from aot_cli.config import load_config
    from aot_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("AOT_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from aot_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("AOT_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # workers spawned via `aot -p <profile> chat -q ...`) honour the
    ...
    agent = AIAgent(
        ...
        #                AOT_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via AOT_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via AOT_YOLO_MODE=1
