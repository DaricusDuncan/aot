...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or aot -c.",
    ...
    "Ctrl+Z suspends Aot to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "aot -c resumes your most recent CLI session. aot -c \"project name\" resumes by title.",
    "aot -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "aot -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "aot chat -t web,terminal enables only specific toolsets for a focused session.",
    "aot chat -s github-pr-workflow preloads a skill at launch.",
    "aot chat -q \"query\" runs a single non-interactive query and exits.",
    "aot chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "aot chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "aot --yolo bypasses all dangerous command approval prompts for the entire session.",
    "aot chat --source telegram tags the session for filtering in aot sessions list.",
    "aot -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "aot doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "aot dump outputs a compact setup summary — great for bug reports.",
    "aot config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "aot config edit opens config.yaml in your default editor.",
    "aot config check scans for missing or stale configuration options.",
    "aot sessions browse opens an interactive session picker with search.",
    "aot sessions stats shows session counts by platform and database size.",
    "aot sessions prune --older-than 30 cleans up old sessions.",
    "aot skills search react --source skills-sh searches the skills.sh public directory.",
    "aot skills check scans installed hub skills for upstream updates.",
    "aot skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "aot skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "aot mcp add github --command npx adds MCP servers from the command line.",
    "aot mcp serve runs Aot itself as an MCP server for other agents.",
    "aot auth add lets you add multiple API keys for credential pool rotation.",
    "aot completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "aot logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "aot backup creates a zip backup of your entire Aot home directory.",
    "aot profile create coder creates an isolated profile that becomes its own command.",
    "aot profile create work --clone copies your current config and keys to a new profile.",
    "aot update syncs new bundled skills to ALL profiles automatically.",
    "aot gateway install sets up Aot as a system service (systemd/launchd).",
    "aot memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "aot webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: aot tools disables unused tools, aot skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "aot models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as aot -w).",
    ...
    "AOT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Aot tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'aot profile create coder' creates the 'coder' command.",
    "aot profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Aot prints a resume command with session ID and stats.",
    "aot sessions export backup.jsonl exports all sessions for backup or analysis.",
    "aot -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "aot skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .aot/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: aot cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Aot runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "aot gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Aot via a local macOS server.",
    ...
    "Context files (.aot.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.aot/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Aot loads project context from .aot.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — aot tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.aot/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "aot chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "aot plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "aot acp runs Aot as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "AOT_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "aot login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "aot status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.aot/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Aot your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.aot/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Aot auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.aot/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at AOT_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "AOT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in AOT_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.aot/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'aot profile create ls' would be rejected.",
    "aot profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "aot claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "AOT_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via aot tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.aot/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.aot/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Aot auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Aot config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Aot is working.',
    ...
    '/reload reloads ~/.aot/.env into the running session — pick up new API keys without restarting.',
    ...
    "AOT_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.aot/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.aot/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'aot curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "aot curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'aot curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'aot auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'AOT_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "AOT_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'aot dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'aot dashboard --tui embeds the full Aot TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.aot/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.aot/dashboard-plugins/ — no npm build required.',
    ...
    'AOT_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'AOT_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'AOT_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'AOT_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.aot/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.aot/skills/ so drop-in skills appear without restarting the session.',
    ...
    'aot -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'aot chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'aot chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'aot chat --ignore-user-config skips ~/.aot/config.yaml — reproducible bug reports and CI runs.',
    "aot chat --source tool tags programmatic chats so they don't clutter aot sessions list.",
    'aot dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'aot sessions rename <ID> "new title" renames any past session; aot sessions delete <ID> removes one.',
    'aot import restores a session export or profile archive produced by sessions export or profile export.',
    'aot fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'aot pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'aot setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'aot status --deep runs the full health sweep across every component; plain aot status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'AOT_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'AOT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.aot/plugins/ — trust-gated by design.',
    "AOT_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'AOT_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'AOT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'AOT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'AOT_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'AOT_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'AOT_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'AOT_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'AOT_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'AOT_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'AOT_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Aot block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from aot sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.aot/dashboard-plugins/.',
]
