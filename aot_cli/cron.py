...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'aot cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: aot gateway install", Colors.DIM))
        print(color("                    sudo aot gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    aot gateway install    # Install as a user service")
        print("    sudo aot gateway install --system  # Linux servers: boot-time system service")
        print("    aot gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: aot cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
