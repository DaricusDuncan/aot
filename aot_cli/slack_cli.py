"""``aot slack ...`` CLI subcommands.

Today only ``aot slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ aot slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Aot. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Aot deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from aot_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Aot agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Aot in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``aot_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$AOT_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Aot")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Aot"
    description = getattr(args, "description", None) or "Your Aot agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("AOT_HOME") or str(Path.home() / ".aot")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Aot app\n"
