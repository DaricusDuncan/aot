"""
Aot Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.aot/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Aot PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing aot-agent or aot PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Aot Agent" comment and following line
                if '# Aot Agent' in line or '# aot-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('aot' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing aot
                if 'aot' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the aot wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "aot",
        Path("/usr/local/bin/aot"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains aot_cli reference)
                content = wrapper.read_text()
                if 'aot_cli' in content or 'aot-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``aot gateway run`` processes
    ...
    try:
        from aot_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from aot_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``AOT_HOME`` and ``AOT_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\aot\git\cmd``, ``%LOCALAPPDATA%\aot\git\bin``,
#      ``%LOCALAPPDATA%\aot\git\usr\bin``, ``%LOCALAPPDATA%\aot\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\aot\git\`` and Node to
#      ``%LOCALAPPDATA%\aot\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``aot dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\aot\gateway-service\`` and sometimes
...
def _aot_path_markers(aot_home: Path) -> list[str]:
    """Path-entry substrings that identify Aot-owned User-PATH entries."""
    root = str(aot_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare aot-agent install dir.
    markers = [root + "\\aot-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if AOT_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "aot".  We don't want to catch
    # unrelated entries like "caot-foo" or "ephermeral", so we look for
    # backslash-aot as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(aot_home: Path) -> list[str]:
    """Strip Aot-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _aot_path_markers(aot_home)
            kept: list[str] = []
...
def remove_aot_env_vars_windows() -> list[str]:
    """Delete AOT_HOME and AOT_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("AOT_HOME", "AOT_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(aot_home: Path) -> list[Path]:
    """Delete PortableGit and Node installs the Windows installer created under
    ``%LOCALAPPDATA%\\aot\\``.  Only called on full uninstall; they're
...
def _is_default_aot_home(aot_home: Path) -> bool:
    """Return True when ``aot_home`` points at the default (non-profile) root."""
    try:
        from aot_constants import get_default_aot_root
        return aot_home.resolve() == get_default_aot_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its AOT_HOME directory.

    We shell out to ``aot -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current AOT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m aot_cli.main` so we don't depend on a `aot`
    #    wrapper that may be half-removed mid-uninstall.
    aot_invocation = [_sys.executable, "-m", "aot_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                aot_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's AOT_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.aot/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.aot/ for future reinstall
    """
    project_root = get_project_root()
    aot_home = get_aot_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie AOT_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_aot_home(aot_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Aot Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {aot_home / 'config.yaml'}")
    print(f"  Secrets: {aot_home / '.env'}")
    print(f"  Data:    {aot_home / 'cron/'}, {aot_home / 'sessions/'}, {aot_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their AOT_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Aot data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Aot code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in aot_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\aot\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(aot_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Aot-owned PATH entries in User environment")

        log_info("Removing AOT_HOME / AOT_GIT_BASH_PATH User env vars...")
        removed_env = remove_aot_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Aot-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing aot command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.aot/, just remove the aot-agent subdir
            if aot_home in project_root.parents or project_root.parent == aot_home:
    ...
    #     under AOT_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(aot_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(aot_home)
    ...
    # 5. Optionally remove ~/.aot/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile AOT_HOME dirs live under
        ...
        try:
            if aot_home.exists():
                shutil.rmtree(aot_home)
                log_success(f"Removed {aot_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {aot_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {aot_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/aot-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/aot-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Aot Agent! ⚕")
    print()
