"""
Aot Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m aot_cli.main web          # Start on http://127.0.0.1:9119
    python -m aot_cli.main web --port 8080
"""
...
try:
    ...
    # running `aot dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["AOT_WEB_DIST"]) if "AOT_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Aot Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Aot-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``aot dashboard --tui``
# or AOT_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.aot/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_aot_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "aot-update": "aot-update.log",
}
...
def _spawn_aot_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``aot <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``aot_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "aot_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "AOT_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``aot gateway restart`` in the background."""
    try:
        proc = _spawn_aot_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/aot/update")
async def update_aot():
    """Kick off ``aot update`` in the background."""
    try:
        proc = _spawn_aot_action(["update"], "aot-update")
    except Exception as exc:
        _log.exception("Failed to spawn aot update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Aot supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in aot_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.aot/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``aot auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Aot resolves Anthropic creds in this order at runtime:
    1. ``~/.aot/.anthropic_oauth.json`` — Aot-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_aot_oauth_credentials,
            read_claude_code_credentials,
            _AOT_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_aot_oauth_credentials = None  # type: ignore
        _AOT_OAUTH_FILE = None  # type: ignore

    aot_creds = None
    if read_aot_oauth_credentials:
        try:
            aot_creds = read_aot_oauth_credentials()
        except Exception:
            aot_creds = None
    if aot_creds and aot_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "aot_pkce",
            "source_label": f"Aot PKCE ({_AOT_OAUTH_FILE})",
            "token_preview": _truncate_token(aot_creds.get("accessToken")),
            "expires_at": aot_creds.get("expiresAt"),
            "has_refresh_token": bool(aot_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Aot even
    when they also have a separate Aot-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "aot auth add anthropic",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "aot auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "aot auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "aot auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from aot_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("aot_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Aot-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _AOT_OAUTH_FILE
            if _AOT_OAUTH_FILE.exists():
                _AOT_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from aot_cli.auth import clear_provider_auth
...
#          → persists to ~/.aot/.anthropic_oauth.json AND credential pool
...
# Guarded so aot web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Aot file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``aot auth add anthropic``.
    """
    from agent.anthropic_adapter import _AOT_OAUTH_FILE
    payload = {
    ...
    _AOT_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _AOT_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from aot_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from aot_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_aot_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from aot_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, AOT_HOME)."""
    from aot_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process AOT_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_aot_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "aot setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from aot_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from aot_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from aot_cli.tools_config import (
...
#
# The endpoint spawns the same ``aot --tui`` binary the CLI uses, behind
...
try:
    from aot_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``aot --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``AOT_TUI_RESUME`` env var —
    matching what ``aot_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``AOT_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from aot_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("AOT_TUI_DISABLE_MOUSE", "1")
    env.setdefault("AOT_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["AOT_TUI_RESUME"] = resume

    if sidecar_url:
        env["AOT_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Aot inside WSL2 to use the dashboard's /chat "
...
# AOT_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/aot"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/aot/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /aot`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__AOT_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/aot``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__AOT_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__AOT_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__AOT_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/aot`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Aot backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Aot Teal",         "description": "Classic dark teal — the canonical Aot look"},
    {"name": "default-large", "label": "Aot Teal (Large)", "description": "Aot Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.aot/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.aot/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_aot_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.aot/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as aot_cli.plugins):
    1. User plugins:    ~/.aot/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.aot/plugins/  (only if AOT_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from aot_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_aot_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("AOT_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".aot" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from aot_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_aot_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from aot_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from aot_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from aot_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from aot_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"aot_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Aot Web UI → http://{host}:{port}")
