"""``aot logs`` — view and filter Aot log files.
...
Usage examples::
    ...
    aot logs                    # last 50 lines of agent.log
    aot logs -f                 # follow agent.log in real time
    aot logs errors             # last 50 lines of errors.log
    aot logs gateway -n 100    # last 100 lines of gateway.log
    aot logs --level WARNING    # only WARNING+ lines
    aot logs --session abc123   # filter by session ID substring
    aot logs --component tools  # only tool-related lines
    aot logs --since 1h         # lines from the last hour
    aot logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_aot_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Aot runs — try 'aot chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_aot_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_aot_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_aot_home()}/logs/")
        return

    print(f"Log files in {display_aot_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'aot chat' to generate logs)")
