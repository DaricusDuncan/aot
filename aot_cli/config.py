"""
Configuration management for Aot Agent.

Config files are stored in ~/.aot/ for easy access:
- ~/.aot/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.aot/.env         - API keys and secrets

This module provides:
- aot config          - Show current configuration
- aot config edit     - Open config in editor
- aot config set      - Set a specific value
- aot config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.aot/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``aot logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  aot config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `aot plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "AOT_LANGFUSE_ENV",
    "AOT_LANGFUSE_RELEASE",
    "AOT_LANGFUSE_SAMPLE_RATE",
    "AOT_LANGFUSE_MAX_CHARS",
    "AOT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from aot_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("AOT_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_aot_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Aot is running in package-manager-managed mode.

    Two signals: the AOT_MANAGED env var (set by the systemd service),
    or a .managed marker file in AOT_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Aot was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.aot/.install_method`` file (written by installers)
    2. AOT_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_aot_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.aot/.install_method."""
    stamp = get_aot_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade aot-agent"
    if method == "docker":
        return "docker pull nousresearch/aot-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade aot-agent"
        return "pip install --upgrade aot-agent"
    return "aot update"
...
def format_managed_message(action: str = "modify this Aot installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("AOT_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Aot installation is managed by NixOS "
            f"(AOT_MANAGED={env_hint}).\n"
            "Edit services.aot-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Aot installation is managed by Homebrew "
            f"(AOT_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade aot-agent"
        )

    return (
        f"Cannot {action}: this Aot installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Aot."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from AOT_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, aot_bin
    or None if container mode is not active, we're already inside the
    container, or AOT_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("AOT_DEV") == "1":
        return None
    ...
    container_mode_file = get_aot_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "aot-agent")
    exec_user = info.get("exec_user", "aot")
    aot_bin = info.get("aot_bin", "/data/current-package/bin/aot")
...
# Re-export from aot_constants — canonical definition lives there.
from aot_constants import get_aot_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_aot_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_aot_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the aot group can
    share state with the gateway service.

    The mode can be overridden via the AOT_HOME_MODE environment variable
    (e.g. AOT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse AOT_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("AOT_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Aot runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("AOT_CONTAINER") or os.environ.get("AOT_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set AOT_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into AOT_HOME if the user doesn't have one yet."""
...
def ensure_aot_home():
    """Ensure ~/.aot directory structure exists with secure permissions.
...
def _ensure_aot_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"AOT_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Aot-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Aot auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Aot sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Aot
        ...
        # bundled Aot image, which drops to the `aot` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Aot sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Aot should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``aot chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.aot/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: aot sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Aot truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``aot kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``aot kanban decompose`` and the kanban
        ...
        # ``aot profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `aot model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `aot --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `aot -c` muscle memory.  Default off so existing
        # users aren't surprised.  AOT_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/aot`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.aot/cache/piper-voices/
    ...
    # response. If not, Aot feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Aot auto-pauses the goal and
    ...
    # always goes to ~/.aot/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${AOT_SKILL_DIR} and ${AOT_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `aot curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.aot/skills/ into
        # ~/.aot/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `aot curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for aot-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Aot Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (AOT_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.aot/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or AOT_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `aot doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``aot_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Aot to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via AOT_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `aot -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `aot` launches with
        ...
        # decomposition is manual via `aot kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     aot-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.aot/logs/.
    ...
    # update model picker lists without shipping a aot-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://aot-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `aot model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.aot/state.db.
    ...
    # ``aot update`` behaviour.
    "updates": {
        # Run a full ``aot backup``-style zip of AOT_HOME before every
        # ``aot update``.  Backups land in ``<AOT_HOME>/backups/`` and
        # can be restored with ``aot import <path>``.  Off by default —
        # on large AOT_HOME directories the zip can add minutes to every
    ...
    # `aot tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'aot model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Aot auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @aot:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. AOT_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #aot)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: aot-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'aot-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Aot API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Aot API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Aot API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # AOT_TOOL_PROGRESS and AOT_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.aot.config`` entries, then checks
    ...
    try:
        ...
        # should never break `aot update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Aot versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``aot_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'aot setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Aot won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'aot doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("AOT_HOME", "~/.aot")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("AOT_TOOL_PROGRESS")
            old_mode = get_env_value("AOT_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from AOT_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from AOT_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("AOT_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from AOT_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$AOT_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_aot_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `aot plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `aot curator status` has no
    ...
    #   3. Creates `~/.aot/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_aot_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_aot_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: aot config set <key> <value>")
    ...
    # metadata.aot.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _aot_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.aot/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.aot/config.yaml.
    ...
    (which change ``AOT_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_aot_home()
...
#   openai-codex (OAuth — aot auth) — OpenAI Codex
#   nous         (OAuth — aot auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.aot/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.aot/.env.
    ...
    interactive menu render (`aot tools`, `aot setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `aot tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Aot env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.aot/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.aot/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.aot/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.aot/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Aot — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Aot vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.aot/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Aot Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  aot config edit     # Edit config file", Colors.DIM))
    print(color("  aot config set <key> <value>", Colors.DIM))
    print(color("  aot setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: aot config set <key> <value>")
            print()
            print("Examples:")
            print("  aot config set model anthropic/claude-sonnet-4")
            print("  aot config set terminal.backend docker")
            print("  aot config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'aot config migrate' to add them")
        ...
        print("  aot config           Show current configuration")
        print("  aot config edit      Open config in editor")
        print("  aot config set <key> <value>   Set a config value")
        print("  aot config check     Check for missing/outdated config")
        print("  aot config migrate   Update config with new options")
        print("  aot config path      Show config file path")
        print("  aot config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``aot config`` UI so users
