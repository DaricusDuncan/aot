"""Profile distributions — shareable, packaged Aot profiles via git.

A distribution is a Aot profile published as a git repository (or
...
* ``aot profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``aot skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``aot profile``, not a parallel tree):

    aot profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "aot-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            aot_requires=str(data.get("aot_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.aot_requires:
            out["aot_requires"] = self.aot_requires
        if self.author:
...
def check_aot_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Aot {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``aot profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Aot profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Aot distribution."
        )

    # Version check up-front so we fail fast
    check_aot_requires(manifest.aot_requires, aot_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.aot).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="aot_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `aot profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`aot profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="aot_dist_update_") as tmp:
        plan = plan_install(
