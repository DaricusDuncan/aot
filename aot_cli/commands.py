"""Slash command definitions and autocomplete for the Aot CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Aot config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Aot works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Aot is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.aot/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Aot Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`aot_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/aot`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``aot skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``aot skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``aot skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/aot <verb>`` subcommand.
    ...
    can still reach them via ``/aot <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/aot`` is always reserved as the first entry so the
    legacy ``/aot <subcommand>`` form keeps working for anything that
    ...
    # Reserve /aot as the catch-all top-level command.
    entries.append(("aot", "Talk to Aot or run a subcommand", "[subcommand] [args]"))
    seen.add("aot")
...
def slack_app_manifest(request_url: str = "https://aot-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /aot handler.

    Maps both canonical names and aliases so /aot bg do stuff works
    the same as /aot background do stuff.

    Plugin-registered slash commands are included so ``/aot <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from aot_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
