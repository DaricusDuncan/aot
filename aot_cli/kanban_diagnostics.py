...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``aot -p <profile> auth``). No HTTP side effect.
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``aot kanban specify``,
    ...
    if auto_decompose:
        ...
        detail_path = (
            ...
            "`aot kanban specify`, which uses auxiliary.triage_specifier."
        )
    ...
    actions = [
        DiagnosticAction(
            ...
            payload={
                "command": (
                    f"aot config set {primary_slot}.provider auto"
                )
            },
            suggested=True,
        ),
    ...
    if not fallback_explicit and not main_visible:
        actions.append(DiagnosticAction(
            ...
            payload={
                "command": (
                    f"aot config set {fallback_slot}.provider auto"
                )
            },
        ))
    if not auto_decompose:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Specify manually: aot kanban specify {task_id}",
            payload={"command": f"aot kanban specify {task_id}"},
        ))
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Verify profile: aot -p {assignee} doctor",
            payload={"command": f"aot -p {assignee} doctor"},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Fix profile auth: aot -p {assignee} auth",
            payload={"command": f"aot -p {assignee} auth"},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: aot kanban log {task_id}",
                payload={"command": f"aot kanban log {task_id}"},
                suggested=True,
            ))
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Dispatcher is misconfigured (wrong board, wrong AOT_HOME).
    ...
    works for Aot profiles, registered lanes, external workers, and
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "aot kanban diagnostics"},
        ),
    ]
...
def config_from_runtime_config(raw_config: Optional[dict]) -> dict:
    """Build diagnostics config from the full Aot runtime config.
