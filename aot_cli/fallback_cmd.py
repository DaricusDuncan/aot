"""
aot fallback — manage the fallback provider chain.
...
Subcommands:
  aot fallback [list]   Show the current fallback chain (default when no subcommand)
  aot fallback add      Pick provider + model via the same picker as `aot model`,
                           then append the selection to the chain
  aot fallback remove   Pick an entry to delete from the chain
  aot fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.aot/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from aot_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `aot model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  aot fallback add")
        print()
        return
    ...
    print("  Docs: https://aot-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `aot model`, then append the selection to the chain."""
    from aot_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `aot model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `aot fallback list` to view, or `aot fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``aot fallback [subcommand]``."""
