"""
Unified self-relaunch for Aot CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``aot sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``aot`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched aot."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_aot_bin() -> Optional[str]:
    """Find the aot entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("aot")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m aot_cli.main``.
    ...
    ``aot.exe`` on PATH or the ``sys.executable -m aot_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with aot.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_aot_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "aot_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh aot invocation.
    ...
    relaunch contract wants: "run aot again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``aot.exe`` (a Python console-script shim that wraps
    ``python -m aot_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "aot exited, then
    new aot started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``aot`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nAot relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run aot.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
