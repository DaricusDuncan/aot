...
on Japanese locales, etc.).  Aot's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``aot_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Aot is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``AOT_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("AOT_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Aot-managed Git install directories so
    ...
    # already-running shells don't see the change — which means aot
    ...
    # install without Aot-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Aot's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Aot's
    ``aot config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Aot (User env var in Windows Settings, or
    export in a PowerShell profile) and Aot picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Aot-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\aot\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``aot`` from right after install) retain their old PATH.

    Any subprocess Aot spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Aot-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "aot", "git", "cmd"),
        os.path.join(local_appdata, "aot", "git", "bin"),
        os.path.join(local_appdata, "aot", "git", "usr", "bin"),
        # Aot venv Scripts directory — host of the aot.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes aot, but harmless to include.
        os.path.join(local_appdata, "aot", "aot-agent", "venv", "Scripts"),
