"""Shell completion script generation for aot CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_aot_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Aot Agent bash completion
# Add to ~/.bashrc:
#   eval "$(aot completion bash)"

_aot_profiles() {{
    local profiles_dir="$HOME/.aot/profiles"
...
_aot_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_aot_profiles)" -- "$cur"))
        return
    fi
...
complete -F _aot_completion aot
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef aot
# Aot Agent zsh completion
# Add to ~/.zshrc:
...
_aot_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.aot/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.aot/profiles 2>/dev/null)}}")
    fi
...
_aot() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_aot_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Aot Agent fish completion",
        "# Add to your config:",
        "#   aot completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __aot_profiles",
        "    echo default",
        "    if test -d $HOME/.aot/profiles",
        "        ls $HOME/.aot/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c aot -f -s p -l profile"
        " -d 'Profile name' -xa '(__aot_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__aot_profiles)' -d 'Profile name'"
                )
