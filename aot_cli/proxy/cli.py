"""CLI handlers for the ``aot proxy`` subcommand."""
...
from aot_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "aot proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"aot login {adapter.name}")
        print(
    ...
    print(
        f"Starting Aot proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Aot proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: aot proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    ...
    print(
        "aot proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  aot proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
