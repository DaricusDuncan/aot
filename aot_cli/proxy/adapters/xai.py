...
# xAI's public API is OpenAI-compatible for the endpoints Aot commonly
# uses. The Responses endpoint is included because Aot' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Aot-managed OAuth credentials."""

    auth_hint = "aot auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`aot auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`aot auth reset xai-oauth` or re-authenticate with "
                    "`aot auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `aot auth add xai-oauth --type oauth`."
            )
