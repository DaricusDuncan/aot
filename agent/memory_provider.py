...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - aot_home (str): The active AOT_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.aot``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "aot").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'aot memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], aot_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'aot memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``aot_home`` is the active AOT_HOME directory path.
