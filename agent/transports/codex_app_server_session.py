...
Owns one Codex thread per Aot session. Drives `turn/start`, consumes
...
# Aot' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_AOT_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Aot session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _AOT_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("AOT_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="aot",
            client_title="Aot Agent",
            client_version=_get_aot_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Aot' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Aot' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own aot-tools callback we
            # auto-accept — the user already approved Aot' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Aot approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Aot returns 'once', 'session', 'always', or 'deny'.
...
def _get_aot_version() -> str:
    """Best-effort Aot version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("aot-agent")
