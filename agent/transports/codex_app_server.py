...
projection into Aot' display, approval bridging, transcript projection into
...
"codex_app_server"`. Aot' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("AOT_KANBAN_TASK"):
            kanban_db = spawn_env.get("AOT_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "AOT_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("AOT_HOME", os.path.expanduser("~/.aot")),
                        "kanban",
                    ),
                )
            )
