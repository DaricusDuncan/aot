"""Anthropic Messages API adapter for Aot Agent.

Translates between Aot's internal OpenAI-style message format and
...
# Aot effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _read_claude_code_credentials_from_keychain() -> Optional[Dict[str, Any]]:
    ...
    if not isinstance(raw, str):
        logger.debug("Keychain: credentials payload is not string output")
        return None
    raw = raw.strip()
    if not raw:
        return None
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Aot historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Aot)
    ...
    # 1. Aot-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Aot configs.
...
# ── Aot-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.aot/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_AOT_OAUTH_FILE = get_aot_home() / ".anthropic_oauth.json"
...
def run_aot_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Aot-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Aot with your Claude Pro/Max subscription.")
    print()
...
def read_aot_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Aot-managed OAuth credentials from ~/.aot/.anthropic_oauth.json."""
    if _AOT_OAUTH_FILE.exists():
        try:
            data = json.loads(_AOT_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Aot OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See aot-agent#13848.
            #
    ...
    # are preserved.  See aot-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Aot Agent", "Claude Code")
                text = text.replace("Aot agent", "Claude Code")
                text = text.replace("aot-agent", "claude-code")
    ...
    # silently hides reasoning text that Aot surfaces in its CLI. We
