...
Storage
-------
Bundles live in ``~/.aot/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``AOT_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``aot bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under AOT_HOME.

    Honors ``AOT_BUNDLES_DIR`` for tests; falls back to
    ``<AOT_HOME>/skill-bundles``.
    """
    override = os.environ.get("AOT_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_aot_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `aot bundles` CLI subcommand.
# ---------------------------------------------------------------------------
