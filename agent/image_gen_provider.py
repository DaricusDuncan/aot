...
as ``kind: backend``) or ``~/.aot/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$AOT_HOME/cache/images/``, creating parents as needed."""
    from aot_constants import get_aot_home

    path = get_aot_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$AOT_HOME/cache/images/``.
