...
Plugins built on Aot Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Aot
...
:class:`~aot_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Aot' main config
...
handles every provider, fallback chain, and per-task override Aot
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`aot_cli.plugins.PluginContext`
