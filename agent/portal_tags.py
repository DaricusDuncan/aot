...
Every Aot request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Aot Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=aot-agent",
        "client=aot-client-v<__version__>",
    ]

The version is sourced live from ``aot_cli.__version__`` so it auto-aligns
...
``aot_cli.__version__`` is the canonical source of truth.
"""
...
def _aot_version() -> str:
    """Return the current Aot release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``aot_cli`` cannot be imported (should
...
def aot_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=aot-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=aot-client-v{_aot_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=aot-agent", aot_client_tag()]
