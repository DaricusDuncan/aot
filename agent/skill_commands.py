...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``AOT_PLATFORM`` env var and
    ``AOT_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("AOT_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.aot/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.aot.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_aot_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.aot/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.aot/skills/`` and any ``skills.external_dirs`` so the
