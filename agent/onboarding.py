...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Aot starts and finds ``~/.openclaw/``.

    Points users at ``aot claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``aot claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Aot, run "
        "`aot claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `aot claw cleanup` (renames it to ~/.openclaw.pre-migration — "
