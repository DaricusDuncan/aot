...
appropriate.  All installs go to a Aot-owned bin staging dir,
``<AOT_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``aot lsp
  status``.
...
# ``<AOT_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def aot_lsp_bin_dir() -> Path:
    """Return the Aot-owned bin staging dir for LSP servers."""
    home = os.environ.get("AOT_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".aot")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = aot_lsp_bin_dir().parent  # <AOT_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a aot-owned target dir.
    ...
    console script.
    """
    pip_target = aot_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``aot lsp status`` CLI to give users a quick
