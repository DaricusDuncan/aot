...
from aot_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``AOT_PLATFORM`` or
            ``AOT_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``aot`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.aot/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``aot`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against AOT_HOME, not cwd
        if not p.is_absolute():
            p = (aot_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.aot/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(aot, dict):
        aot = {}
    return {
        "fallback_for_toolsets": aot.get("fallback_for_toolsets", []),
        "requires_toolsets": aot.get("requires_toolsets", []),
        "fallback_for_tools": aot.get("fallback_for_tools", []),
        "requires_tools": aot.get("requires_tools", []),
    }
