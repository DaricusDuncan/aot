...
from aot_constants import get_aot_home, get_skills_dir, is_wsl
...
_AOT_MD_NAMES = (".aot.md", "AOT.md")


def _find_aot_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.aot.md`` or ``AOT.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Aot Agent, an intelligent AI assistant created by Nous Research. "
...
AOT_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Aot Agent "
    "itself, load the `aot-agent` skill with skill_view(name='aot-agent') "
    "before answering. Docs: https://aot-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.aot/kanban.db`. Your task id is in "
    "`$AOT_KANBAN_TASK`; your workspace is `$AOT_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $AOT_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `aot kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Aot WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Aot itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Aot runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Aot itself is running. The host OS, home, and cwd "
                f"of the Aot process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Aot "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_aot_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.aot/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("AOT_PLATFORM")
        or get_session_env("AOT_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Aot Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `aot-agent` skill "
            "first. It has the actual commands (e.g. `aot config set …`, `aot tools`, "
            "`aot setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: aot setup, aot setup tools, aot setup terminal, aot status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from AOT_HOME and return its content, or None.
    ...
    """
    try:
        from aot_cli.config import ensure_aot_home
        ensure_aot_home()
    except Exception as e:
        logger.debug("Could not ensure AOT_HOME before loading SOUL.md: %s", e)

    soul_path = get_aot_home() / "SOUL.md"
...
def _load_aot_md(cwd_path: Path) -> str:
    """.aot.md / AOT.md — walk to git root."""
    aot_md_path = _find_aot_md(cwd_path)
    if not aot_md_path:
        return ""
    try:
        content = aot_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = aot_md_path.name
        try:
            rel = str(aot_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".aot.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", aot_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .aot.md / AOT.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from AOT_HOME is independent and always included when present.
    ...
    # SOUL.md from AOT_HOME only — skip when already loaded as identity
    if not skip_soul:
