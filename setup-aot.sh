...
# 5. Symlinks the 'aot' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/aot-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink aot into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up aot command..."

AOT_BIN="$SCRIPT_DIR/venv/bin/aot"
...
# ============================================================================
# Seed bundled skills into ~/.aot/skills/
# ============================================================================

AOT_SKILLS_DIR="${AOT_HOME:-$HOME/.aot}/skills"
mkdir -p "$AOT_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.aot/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$AOT_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  aot status        # Check configuration"
if is_termux; then
    echo "  aot gateway       # Run gateway in foreground"
else
    echo "  aot gateway install # Install gateway service (messaging + cron)"
fi
echo "  aot cron list     # View scheduled jobs"
echo "  aot doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m aot_cli.main setup
fi
