"""Centralized logging setup for Aot Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.aot/logs/`` (profile-aware via ``get_aot_home()``).
...
from aot_constants import get_config_path, get_aot_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_aot_session_injector", False):
    ...
    _session_record_factory._aot_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``aot logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "aot_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Aot logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    aot_home
        Override for the Aot home directory.  Falls back to
        ``get_aot_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_aot_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._aot_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the aot group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from aot_cli.config import is_managed
