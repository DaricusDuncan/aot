#!/usr/bin/env python3
"""
SQLite State Store for Aot Agent.
...
DEFAULT_DB_PATH = get_aot_home() / "state.db"
...
# aot_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``aot_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see aot_cli/kanban_db.py) would
...
class SessionDB:
    ...
    # With multiple aot processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``aot_state._set_last_init_error(None)`` explicitly.
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Aot, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Aot session.

        A Aot session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Aot session is already bound to any Telegram DM topic.
