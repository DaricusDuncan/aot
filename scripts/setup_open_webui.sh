...
# Bootstrap Open WebUI against Aot Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.aot/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-aot.sh
...
#   AOT_API_PORT=8642
#   AOT_API_HOST=127.0.0.1
#   AOT_API_MODEL_NAME='Aot Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Aot Agent WebUI}"
...
AOT_ENV_FILE="${AOT_ENV_FILE:-$HOME/.aot/.env}"
AOT_API_PORT="${AOT_API_PORT:-8642}"
AOT_API_HOST="${AOT_API_HOST:-127.0.0.1}"
AOT_API_CONNECT_HOST="${AOT_API_CONNECT_HOST:-127.0.0.1}"
AOT_API_MODEL_NAME="${AOT_API_MODEL_NAME:-Aot Agent}"
AOT_API_BASE_URL="http://${AOT_API_CONNECT_HOST}:${AOT_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-aot.sh"
LOG_DIR="$HOME/.aot/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$AOT_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$AOT_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.aot.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.aot</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.aot"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.aot"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-aot.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Aot Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-aot.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.aot/logs/openwebui.log
StandardError=append:%h/.aot/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-aot.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$AOT_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Aot API server is configured...'
  upsert_env API_SERVER_ENABLED true "$AOT_ENV_FILE"
  upsert_env API_SERVER_HOST "$AOT_API_HOST" "$AOT_ENV_FILE"
  upsert_env API_SERVER_PORT "$AOT_API_PORT" "$AOT_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$AOT_API_MODEL_NAME" "$AOT_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$AOT_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Aot gateway so API server settings take effect...'
  aot gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${AOT_API_CONNECT_HOST}:${AOT_API_PORT}/health" >/dev/null; then
    log 'Aot API server did not answer on the first check. Trying to start gateway in the background...'
    nohup aot gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${AOT_API_CONNECT_HOST}:${AOT_API_PORT}/health" >/dev/null
  ...
  log "Aot API endpoint: ${AOT_API_BASE_URL}"
