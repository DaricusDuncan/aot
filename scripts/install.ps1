# ============================================================================
# Aot Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$AotHome = "$env:LOCALAPPDATA\aot",
    [string]$InstallDir = "$env:LOCALAPPDATA\aot\aot-agent",
...
$RepoUrlSsh = "git@github.com:NousResearch/aot-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/aot-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Aot Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AotHome)) {
        New-Item -ItemType Directory -Force -Path $AotHome | Out-Null
    }
    $envFile = Join-Path $AotHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $AotHome "node"
...
function Install-Git {
    ...
    which Aot uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\aot\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Aot needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Aot copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\aot\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``AOT_GIT_BASH_PATH`` (User scope) so Aot can find it in a fresh
    ...
    # Download PortableGit into $AotHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AotHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "aot-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Aot features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Aot needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``AOT_GIT_BASH_PATH`` (User env scope) so Aot can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AotHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AotHome\git\usr\bin\bash.exe
    $candidates += "$AotHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AotHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("AOT_GIT_BASH_PATH", $candidate, "User")
            $env:AOT_GIT_BASH_PATH = $candidate
            Write-Info "Set AOT_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Aot may not find Git Bash."
    Write-Info "If needed, set AOT_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Test-Path $managedNode) {
        ...
        $env:Path = "$AotHome\node;$env:Path"
        Write-Success "Node.js $version found (Aot-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $AotHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AotHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\aot-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$AotHome\node") { Remove-Item -Recurse -Force "$AotHome\node" }
                Move-Item $extractedDir.FullName "$AotHome\node"
                ...
                $version = & "$AotHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AotHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running aot processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/aot-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/aot-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/aot-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\aot-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\aot-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `aot.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install aot-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and aot.exe broken with
    ...
    # here, not 30 seconds later when the user runs `aot`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `aot dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `aot dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `aot dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up aot command..."
    
    if ($NoVenv) {
        $aotBin = "$InstallDir"
    } else {
        $aotBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so aot is globally available
    # On Windows, the aot.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$aotBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$aotBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $aotBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set AOT_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\aot instead
    # of the Unix default ~/.aot
    $currentAotHome = [Environment]::GetEnvironmentVariable("AOT_HOME", "User")
    if (-not $currentAotHome -or $currentAotHome -ne $AotHome) {
        [Environment]::SetEnvironmentVariable("AOT_HOME", $AotHome, "User")
        Write-Success "Set AOT_HOME=$AotHome"
    }
    $env:AOT_HOME = $AotHome
    
    # Update current session
    $env:Path = "$aotBin;$env:Path"
    
    Write-Success "aot command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.aot directory structure
    New-Item -ItemType Directory -Force -Path "$AotHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AotHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AotHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AotHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AotHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AotHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AotHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AotHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AotHome\skills" | Out-Null
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.aot/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.aot/.env"
        }
    } else {
        Write-Info "~/.aot/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$AotHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.aot/config.yaml from template"
        }
    } else {
        Write-Info "~/.aot/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Aot's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.aot/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.aot/"
    
    # Seed bundled skills into ~/.aot/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.aot/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.aot/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$AotHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.aot/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'aot setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\aot-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\aot-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\aot-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.aot/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'aot setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m aot_cli.main setup
    } else {
        python -m aot_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $aotCmd = "$InstallDir\venv\Scripts\aot.exe"
    if (-not (Test-Path $aotCmd)) {
        $aotCmd = "aot"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AotHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'aot whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: aot gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AotHome\logs\gateway.log"
            Start-Process -FilePath $aotCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AotHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: aot gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: aot gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$AotHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AotHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AotHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AotHome\aot-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   aot              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   aot setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   aot config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   aot config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   aot gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   aot update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Aot repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Aot to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/aot-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
