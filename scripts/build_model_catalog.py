#!/usr/bin/env python3
"""Build the Aot Model Catalog — a centralized JSON manifest of curated models.
...
Aot CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Aot release.
...
# Ensure AOT_HOME is set for imports that touch it at module level.
os.environ.setdefault("AOT_HOME", os.path.join(os.path.expanduser("~"), ".aot"))

from aot_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "aot-agent repo",
            "docs": "https://aot-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
