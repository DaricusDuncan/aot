...
VERSION_FILE = REPO_ROOT / "aot_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+aot-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "aot@nousresearch.com": "NousResearch",
    ...
    "aot@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (AOT_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (AOT_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: aot-eval community link)
    ...
    "aotagent26@gmail.com": "aotagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+aotagent26@users.noreply.github.com": "aotagent26",
    ...
    "vigo@aot": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"aot-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "aot@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/aot-agent",
    ...
    lines.append(f"# Aot Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Aot Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Aot Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Aot Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Aot Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Aot Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Aot Agent v{new_version} ({calver_date})' "
