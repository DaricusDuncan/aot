 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.aot/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.aot', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.aot', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.aot', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.aot', 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '⚕ *Aot Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Aot Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Aot' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `aot_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
