#!/usr/bin/env python3
"""OpenClaw -> Aot migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Aot Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Aot.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Aot memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Aot memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Aot-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Aot-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Aot command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.aot/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.aot/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Aot .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Aot .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Aot .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Aot .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Aot .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Aot config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Aot config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Aot.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Aot MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Aot config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'aot cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Aot config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Aot session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Aot custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Aot config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Aot config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Aot config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Aot config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Aot in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Aot`` (prose), but lowercase matches
# like ``openclaw`` → ``aot`` (so filesystem paths like ``~/.openclaw``
# become ``~/.aot`` — the real Aot home — not the broken ``~/.Aot``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Aot'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Aot'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Aot'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Aot`` but maps ``openclaw`` → ``aot`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.aot/config.yaml`` (the real Aot home) instead of the broken
    ``~/.Aot/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Aot.
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-aot/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Aot config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-aot/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Aot session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Aot config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Aot
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Aot-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Aot-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: aot config set",
                        )
                    continue
        ...
        for oc_key, aot_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and aot_key not in secret_additions:
                secret_additions[aot_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, aot_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and aot_key not in secret_additions:
                        secret_additions[aot_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        aot_config = load_yaml_file(destination)
        current_model = aot_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = aot_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                aot_config["model"] = {"default": model_str}
            dump_yaml_file(destination, aot_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Aot still uses "edge"
        ...
        aot_config = load_yaml_file(destination)
        existing_tts = aot_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Aot destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Aot-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Aot-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        aot_cfg_path = self.target_root / "config.yaml"
        aot_cfg = load_yaml_file(aot_cfg_path)
        existing_mcp = aot_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Aot config")
                continue

            aot_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                aot_srv["command"] = srv["command"]
                if srv.get("args"):
                    aot_srv["args"] = srv["args"]
                if srv.get("env"):
                    aot_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    aot_srv["cwd"] = srv["cwd"]
            ...
            if srv.get("url"):
                aot_srv["url"] = srv["url"]
                if srv.get("headers"):
                    aot_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    aot_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                aot_srv["enabled"] = False
            if srv.get("timeout"):
                aot_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                aot_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                aot_srv["tools"] = {}
                if tools_cfg.get("include"):
                    aot_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    aot_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(aot_cfg_path)
            aot_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(aot_cfg_path, aot_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'aot cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = aot_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Aot reasoning_effort
        ...
        if compaction:
            compression = aot_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'aot gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("AOT_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        aot_cfg_path = self.target_root / "config.yaml"
        aot_cfg = load_yaml_file(aot_cfg_path)
        sr = aot_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        aot_cfg_path = self.target_root / "config.yaml"
        aot_cfg = load_yaml_file(aot_cfg_path)
        custom_providers = aot_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(aot_cfg_path)
            aot_cfg["custom_providers"] = custom_providers
            dump_yaml_file(aot_cfg_path, aot_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Aot config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            aot_cfg_path = self.target_root / "config.yaml"
            aot_cfg = load_yaml_file(aot_cfg_path)
            discord_aot = aot_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_aot["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_aot["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                aot_cfg["discord"] = discord_aot
                dump_yaml_file(aot_cfg_path, aot_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        aot_cfg_path = self.target_root / "config.yaml"
        aot_cfg = load_yaml_file(aot_cfg_path)
        browser_aot = aot_cfg.get("browser") or {}
        changed = False

        # Map fields that have Aot equivalents
        if browser.get("cdpUrl"):
            browser_aot["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_aot["headless"] = browser["headless"]
            changed = True

        if changed:
            aot_cfg["browser"] = browser_aot
            if self.execute:
                self.maybe_backup(aot_cfg_path)
                dump_yaml_file(aot_cfg_path, aot_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            aot_mode = mode_map.get(mode, "manual")
            aot_cfg.setdefault("approvals", {})["mode"] = aot_mode
            if self.execute:
                self.maybe_backup(aot_cfg_path)
                dump_yaml_file(aot_cfg_path, aot_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{aot_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Aot. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Aot Config Not Overwritten)",
                "",
                "These items already existed in your Aot config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Aot agent discovers these",
            "directories, it may read/write to them instead of the Aot state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `aot claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `aot claw cleanup` to fix it.",
            "",
            "## Aot-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `aot claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `aot setup` to configure any remaining settings",
            "- Run `aot mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `aot cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `aot cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `aot gateway install` if you need the gateway service",
            "- Review `~/.aot/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Aot Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("AOT_HOME") or str(Path.home() / ".aot"), help="Aot home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Aot targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Aot-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Aot Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.aot/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.aot/config.yaml")
        print("    2. Run: aot mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: aot cron")
