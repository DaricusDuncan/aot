...
Maps each role archetype to the Aot skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public aot-agent
...
## Aot skills relevant to video production

### Visual / rendering skills (`aot-agent/skills/creative/`)
...
### Design / pre-production skills (`aot-agent/skills/creative/`)
...
### Audio / media skills (`aot-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`aot-agent/skills/devops/`)
...
These are **not** Aot skills but external CLIs / APIs that profiles invoke.
...
## Built-in Aot tools for media review

These are native Aot tools — not invoked via terminal but through their own
...
key is present in `~/.aot/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Aot profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.aot/.env` → environment variable.
...
When a new Aot-public video skill ships:
