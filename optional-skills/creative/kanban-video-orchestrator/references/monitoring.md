...
aot kanban watch --tenant <project-slug>

# Snapshot of the board
aot kanban list --tenant <project-slug>
aot kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
aot kanban stats --tenant <project-slug>

# Visual dashboard (browser)
aot dashboard

# Inspect a specific task (includes comments + events)
aot kanban show <task-id>

# Follow a single task's event stream
aot kanban tail <task-id>
```

Verify available subcommands with `aot kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `aot kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `aot kanban block <id>` then `aot kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `aot kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `aot kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `aot profile list` (profile exists?), `aot status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(aot kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
aot kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
aot kanban block <task-id>
aot kanban archive <task-id>

# Diagnose what happened
aot kanban show <task-id>      # task body, comments, recent events
aot kanban tail <task-id>      # follow the live event stream
aot kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `aot kanban create` for the director
...
while true; do
    clear
    aot kanban list --tenant <slug>
    echo "---"
    aot kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `aot kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `aot profile list` and start
