#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `aot kanban list` and
...
def aot_available() -> bool:
    return shutil.which("aot") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed aot CLI."""
    try:
        out = subprocess.run(
            ["aot", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `aot kanban list`
    out = subprocess.run(
        ["aot", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not aot_available():
        print("ERROR: 'aot' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
