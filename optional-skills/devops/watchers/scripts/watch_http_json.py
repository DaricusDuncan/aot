...
Usage (via cron with --no-agent):
    ...
    aot cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$AOT_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Aot-Watcher/1.0"})
