...
Usage (via cron with --no-agent):
    ...
    aot cron create aot-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$AOT_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name aot-issues --repo NousResearch/aot-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.aot/.env to avoid the 60 req/hr
