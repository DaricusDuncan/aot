...
$AOT_OSINT_CACHE/icij/ (default: ~/.cache/aot-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("AOT_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "aot-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "aot-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $AOT_OSINT_CACHE/icij or ~/.cache/aot-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
