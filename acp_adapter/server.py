"""ACP agent server — exposes Aot Agent via the Agent Client Protocol."""
...
try:
    from aot_cli import __version__ as AOT_VERSION
except Exception:
    AOT_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Aot running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Aot cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Aot/OpenAI-compatible user content payload."""
...
class AotACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Aot AIAgent."""
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Aot maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from aot_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from aot_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Aot estimates ``used`` from the same
    ...
    async def _send_session_info_update(self, session_id: str) -> None:
        """Send ACP native session metadata after Aot changes it."""
        if not self._conn:
            return
        try:
        ...
        # aot_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["aot-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="aot-agent", version=AOT_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Aot' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Aot setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Aot
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Aot on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Aot' local kawaii waiting/status
        ...
        # not the executor's. Also set AOT_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue AOT_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind AOT_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("AOT_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("AOT_SESSION_ID")
            os.environ["AOT_SESSION_ID"] = session_id
            try:
                ...
                # Restore AOT_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("AOT_INTERACTIVE", None)
                else:
                    os.environ["AOT_INTERACTIVE"] = previous_interactive
                # Restore AOT_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("AOT_SESSION_ID", None)
                else:
                    os.environ["AOT_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (AOT_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Aot Agent v{AOT_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Aot has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
