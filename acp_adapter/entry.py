"""CLI entry point for the aot-agent ACP adapter.

Loads environment variables from ``~/.aot/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from AOT_HOME (default ``~/.aot``)."""
    from aot_cli.env_loader import load_aot_dotenv

    aot_home = get_aot_home()
    loaded = load_aot_dotenv(aot_home=aot_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", aot_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="aot-acp",
        description="Run Aot Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Aot version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import AotACPAgent  # noqa: F401

    print("Aot ACP check OK")


def _run_setup() -> None:
    from aot_cli.main import main as aot_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "aot", "model"]
        aot_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``aot postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting aot-agent ACP adapter")
    ...
    import acp
    from .server import AotACPAgent
