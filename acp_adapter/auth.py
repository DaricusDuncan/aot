"""ACP auth helpers — detect and advertise Aot authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "aot-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Aot runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Aot can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Aot.
    ...
    not have Aot provider credentials configured yet, so Aot always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Aot using the currently configured "
