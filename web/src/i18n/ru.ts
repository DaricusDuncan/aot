...
export const ru: Translations = {
  ...
  status: {
    ...
    stopped: "Остановлено",
    updateAot: "Обновить Aot",
    updatingAot: "Обновление Aot…",
  ...
  pluginsPage: {
    ...
    headline:
      "Поиск, установка, включение и обновление плагинов Aot (аналог `aot plugins`).",
    ...
    removeConfirm: "Удалить этот плагин из ~/.aot/plugins/?",
    removeHint: "Удалять можно только плагины, установленные пользователем в ~/.aot/plugins.",
  ...
  skills: {
    ...
    filters: "Фильтры",
    noSkills: "Навыки не найдены. Навыки загружаются из ~/.aot/skills/",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Коллекционные значки Aot, полученные на основе реальной истории сессий. Известные, но ещё не полученные достижения отображаются как «Обнаруженные»; «Секретные» достижения остаются скрытыми до появления первого подходящего поведения.",
      scan_subtitle:
        "Анализ истории сессий Aot. Первое сканирование может занять 5–10 секунд при большой истории.",
    },
    actions: {
    ...
    stats: {
      ...
      latest: "Последнее",
      latest_hint_empty: "запускайте Aot чаще",
      none_yet: "Пока нет",
    },
    state: {
      unlocked: "Разблокировано",
    ...
    guide: {
      ...
      secret_body:
        "Секретные достижения скрывают свой точный триггер. Как только Aot обнаруживает связанный сигнал, карточка становится «Обнаруженной» и показывает требование.",
      scan_status_header: "Статус сканирования",
      scan_status_body:
        "Aot сканирует локальную историю один раз, затем карточки появятся автоматически. Если это занимает несколько секунд — ничего не зависло.",
