...
export class GatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    const resolved = token ?? window.__AOT_SESSION_TOKEN__ ?? "";
    if (!resolved) {
      this.setState("error");
      throw new Error(
        "Session token not available — page must be served by the Aot dashboard",
      );
    }

    const scheme = location.protocol === "https:" ? "wss:" : "ws:";
    const ws = new WebSocket(
      `${scheme}//${location.host}${AOT_BASE_PATH}/api/ws?token=${encodeURIComponent(resolved)}`,
    );
    this.ws = ws;
