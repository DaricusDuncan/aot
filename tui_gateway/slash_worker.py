"""Persistent slash-command worker — one AotCLI per TUI session.
...
def _run(cli: AotCLI, command: str) -> str:
...
def main():
    ...
    os.environ["AOT_SESSION_KEY"] = args.session_key
    os.environ["AOT_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = AotCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
