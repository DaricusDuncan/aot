...
load_aot_dotenv(
    aot_home=_aot_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.aot/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_aot_home, "logs", "tui_gateway_crash.log")
...
_cfg_path = None
try:
    _slash_timeout = float(os.environ.get("AOT_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("AOT_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent AotCLI subprocess for slash commands."""
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `aot` shows the prompt before constructing AIAgent; the TUI used
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["AOT_GATEWAY_SESSION"] = "1"
...
def resolve_skin() -> dict:
    try:
        from aot_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("AOT_MODEL", "")
        or os.environ.get("AOT_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("AOT_TUI_PROVIDER", "").strip()
    ...
    try:
        from aot_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("AOT_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("AOT_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("AOT_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] AOT_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from aot_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown AOT_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in AOT_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid AOT_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid AOT_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from aot_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from aot_cli.config import get_compatible_custom_providers, load_config
    ...
    os.environ["AOT_MODEL"] = result.new_model
    os.environ["AOT_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # AOT_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["AOT_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["AOT_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from aot_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from aot_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from aot_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from aot_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("AOT_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("AOT_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("AOT_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("AOT_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("AOT_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("AOT_IGNORE_RULES")),
        **_agent_cbs(sid),
    )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `AOT_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"aot_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
...
#
# Layout:  $AOT_HOME/spawn-trees/<session_id>/<timestamp>.json
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from aot_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from aot_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: aot_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from aot_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _aot_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from aot_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_aot_home), "display": display_aot_home()})
    if key == "full":
...
def _(rid, params: dict) -> dict:
    try:
        from aot_cli.main import _has_any_provider_configured
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.aot/.env`` into the gateway process via
    ``aot_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `aot` is interactive — use `/aot chat -q …` or run `aot` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`aot setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`aot gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`aot sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`aot config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m aot_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "aot_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from aot_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from aot_cli.config import is_managed, save_env_value
        from aot_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `aot model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.aot/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from aot_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from aot_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("AOT_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("AOT_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["AOT_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from aot_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["AOT_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from aot_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from aot_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from aot_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from aot_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("AOT_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("AOT_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_aot_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from aot_cli.skills_hub import do_install
        ...
        if action == "browse":
            from aot_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from aot_cli.skills_hub import inspect_skill
