...
# aot modules.  aot_cli sets AOT_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("AOT_PYTHON_SRC_ROOT", "")
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `AOT_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("AOT_TUI_SIDECAR_URL")
...
# tested; override via ``AOT_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("AOT_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``AOT_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``aot --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
